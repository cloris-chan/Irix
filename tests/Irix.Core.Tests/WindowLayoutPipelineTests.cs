...
public sealed class WindowLayoutPipelineTests
{
    ...
    [Fact]
    public async Task StyleTransitionRuntimeCoordinator_starts_and_retargets_from_runtime_decisions()
    {
        ...
        var startRuntime = new SingleStyleTransitionRuntimeAdapter(StyleTransitionRuntimeDecision.Start(
            new NodeKey(22),
        ...
        var start = await coordinator.ApplyNextAsync(
            startRuntime,
            compositor,
        ...
        Assert.Equal(StyleTransitionCompileStatus.CompiledCompositionDeclaration, start.CompileStatus);
        Assert.True(start.HasDeclaration);
        ...
        var retargetRuntime = new SingleStyleTransitionRuntimeAdapter(StyleTransitionRuntimeDecision.Retarget(
            new NodeKey(22),
            BuildCompositionStyleProperties(opacity: 0.75, translateX: 12, translateY: 8),
        ...
        var retarget = await coordinator.ApplyNextAsync(
            retargetRuntime,
            compositor,
            new FixedStyleTransitionRetainedSnapshotProvider(snapshot),
            cancellationToken);
        ...
        Assert.Equal(StyleTransitionCompileStatus.CompiledCompositionDeclaration, retarget.CompileStatus);
        Assert.True(retarget.HasDeclaration);
        Assert.Equal(retarget, retargetRuntime.LastResult);
        Assert.Equal(2, compositor.StartCount);
        Assert.Equal(0.5f, compositor.LastDeclaration.Opacity.To);
        Assert.Equal(24f, compositor.LastDeclaration.Transform.TranslateX.To);
        Assert.Equal(16f, compositor.LastDeclaration.Transform.TranslateY.To);
    }
    ...
    public async Task StyleTransitionRuntimeCoordinator_cancel_and_commit_clear_presentation_without_compiling()
    {
        ...
        var compositor = new RecordingStyleTransitionCompositorAdapter();
        var coordinator = new StyleTransitionRuntimeCoordinator();
        var cancelRuntime = new SingleStyleTransitionRuntimeAdapter(StyleTransitionRuntimeDecision.Cancel(new NodeKey(22)));

        var canceled = await coordinator.ApplyNextAsync(
            cancelRuntime,
            compositor,
            new FixedStyleTransitionRetainedSnapshotProvider(snapshot),
            cancellationToken);

        Assert.Equal(StyleTransitionRuntimeResultKind.Canceled, canceled.Kind);
        Assert.Equal(StyleTransitionCompileStatus.None, canceled.CompileStatus);
        Assert.False(canceled.HasDeclaration);
        ...
        Assert.Equal(canceled, cancelRuntime.LastResult);

        var commitRuntime = new SingleStyleTransitionRuntimeAdapter(StyleTransitionRuntimeDecision.Commit(new NodeKey(22)));

        var committed = await coordinator.ApplyNextAsync(
            commitRuntime,
            compositor,
            new FixedStyleTransitionRetainedSnapshotProvider(snapshot),
            cancellationToken);
    ...
    [Fact]
    public async Task StyleTransitionRuntimeCoordinator_falls_back_before_changing_presentation_owner()
    {
        var cancellationToken = TestContext.Current.CancellationToken;
        var snapshot = CreateStyleTransitionRetainedSnapshot();
        var missingSnapshotCompositor = new RecordingStyleTransitionCompositorAdapter();
        var coordinator = new StyleTransitionRuntimeCoordinator();
        var missingSnapshotRuntime = new SingleStyleTransitionRuntimeAdapter(StyleTransitionRuntimeDecision.Start(
            new NodeKey(22),
            BuildCompositionStyleProperties(opacity: 1, translateX: 0, translateY: 0),
            BuildCompositionStyleProperties(opacity: 0.5, translateX: 12, translateY: 8),
            CompositionTimestamp.Zero,
            CompositionDuration.FromStopwatchTicks(50)));

        var missingSnapshot = await coordinator.ApplyNextAsync(
            missingSnapshotRuntime,
            missingSnapshotCompositor,
            new FixedStyleTransitionRetainedSnapshotProvider(null),
            cancellationToken);

        Assert.Equal(StyleTransitionRuntimeResultKind.Fallback, missingSnapshot.Kind);
        Assert.Equal(StyleTransitionRuntimeFallbackReason.MissingRetainedSnapshot, missingSnapshot.FallbackReason);
        Assert.Equal(StyleTransitionCompileStatus.None, missingSnapshot.CompileStatus);
        Assert.Equal(0, missingSnapshotCompositor.StartCount);
        Assert.Equal(0, missingSnapshotCompositor.CancelCount);
        ...
        var drawOwned = await coordinator.ApplyNextAsync(
            drawOwnedRuntime,
            drawOwnedCompositor,
            new FixedStyleTransitionRetainedSnapshotProvider(snapshot),
            cancellationToken);

        Assert.Equal(StyleTransitionRuntimeResultKind.Fallback, drawOwned.Kind);
        Assert.Equal(StyleTransitionRuntimeFallbackReason.CompileRejected, drawOwned.FallbackReason);
        Assert.Equal(StyleTransitionCompileStatus.RequiresDrawUpdate, drawOwned.CompileStatus);
        Assert.True(drawOwned.DeltaPlan.RequiresDrawUpdate);
        Assert.False(drawOwned.HasDeclaration);
        Assert.Equal(0, drawOwnedCompositor.StartCount);
        Assert.Equal(0, drawOwnedCompositor.CancelCount);
        Assert.Equal(drawOwned, drawOwnedRuntime.LastResult);
    }

    [Fact]
    public void StyleTransitionDecisionBatchPreflight_preserves_runtime_order_and_does_not_install_compositor_owners()
    {
        var snapshot = CreateMultiTargetStyleTransitionRetainedSnapshot();
        var compositor = new RecordingStyleTransitionCompositorAdapter();
        var acceptedIncrement = new StyleTransitionDecisionBatchEntry(
        ...
        var rejectedDecrement = new StyleTransitionDecisionBatchEntry(
        ...
        var batch = StyleTransitionDecisionBatch.Create([acceptedIncrement, rejectedDecrement]);

        var result = StyleTransitionDecisionBatchPreflight.Validate(
            batch,
            new FixedStyleTransitionRetainedSnapshotProvider(snapshot));
        ...
        var ownerResults = result.OwnerResults.ToArray();
        Assert.Collection(
            ownerResults,
            first =>
            {
                Assert.Equal(StyleTransitionOwnerValidationKind.Accepted, first.Kind);
                Assert.Equal(acceptedIncrement.OwnerKey, first.OwnerKey);
                Assert.Equal(new NodeKey(22), first.TargetKey);
                Assert.Equal(StyleTransitionRuntimeDecisionKind.Start, first.DecisionKind);
                Assert.Equal(StyleTransitionOwnerRejectionReason.None, first.RejectionReason);
                Assert.Equal(StyleTransitionCompileStatus.CompiledCompositionDeclaration, first.CompileStatus);
                Assert.True(first.HasDeclaration);
            },
            second =>
            {
                Assert.Equal(StyleTransitionOwnerValidationKind.Rejected, second.Kind);
                Assert.Equal(rejectedDecrement.OwnerKey, second.OwnerKey);
                Assert.Equal(new NodeKey(23), second.TargetKey);
                Assert.Equal(StyleTransitionRuntimeDecisionKind.Start, second.DecisionKind);
                Assert.Equal(StyleTransitionOwnerRejectionReason.CompileRejected, second.RejectionReason);
                Assert.Equal(StyleTransitionCompileStatus.RequiresDrawUpdate, second.CompileStatus);
                Assert.False(second.HasDeclaration);
            });
    }

    [Fact]
    public void StyleTransitionDecisionBatchPreflight_rejects_missing_snapshot_or_target_before_compositor_install()
    {
        var missingSnapshotEntry = new StyleTransitionDecisionBatchEntry(
            StyleTransitionOwnerKey.ControlState(ActionIdRegistry.Increment, new NodeKey(22)),
            StyleTransitionRuntimeDecision.Start(
                new NodeKey(22),
                BuildCompositionStyleProperties(opacity: 1, translateX: 0, translateY: 0),
                BuildCompositionStyleProperties(opacity: 0.5, translateX: 4, translateY: 4),
                CompositionTimestamp.FromStopwatchTicks(100),
                CompositionDuration.FromStopwatchTicks(50)));
        var missingTargetEntry = new StyleTransitionDecisionBatchEntry(
            StyleTransitionOwnerKey.ControlState(ActionIdRegistry.Reset, new NodeKey(99)),
            StyleTransitionRuntimeDecision.Start(
                new NodeKey(99),
                BuildCompositionStyleProperties(opacity: 1, translateX: 0, translateY: 0),
                BuildCompositionStyleProperties(opacity: 0.5, translateX: 4, translateY: 4),
                CompositionTimestamp.FromStopwatchTicks(100),
                CompositionDuration.FromStopwatchTicks(50)));
        var missingSnapshotBatch = StyleTransitionDecisionBatch.Create([missingSnapshotEntry]);
        var missingTargetBatch = StyleTransitionDecisionBatch.Create([missingTargetEntry]);
        var compositor = new RecordingStyleTransitionCompositorAdapter();

        var missingSnapshot = StyleTransitionDecisionBatchPreflight.Validate(
            missingSnapshotBatch,
            new FixedStyleTransitionRetainedSnapshotProvider(null));
        var missingTarget = StyleTransitionDecisionBatchPreflight.Validate(
            missingTargetBatch,
            new FixedStyleTransitionRetainedSnapshotProvider(CreateMultiTargetStyleTransitionRetainedSnapshot()));

        Assert.Equal(StyleTransitionBatchResultKind.Rejected, missingSnapshot.Kind);
        Assert.Equal(0, missingSnapshot.AcceptedCount);
        Assert.Equal(1, missingSnapshot.RejectedCount);
        Assert.False(missingSnapshot.PresentationStateChanged);
        Assert.Equal(StyleTransitionOwnerRejectionReason.MissingRetainedSnapshot, missingSnapshot.OwnerResults[0].RejectionReason);

        Assert.Equal(StyleTransitionBatchResultKind.Rejected, missingTarget.Kind);
        Assert.Equal(0, missingTarget.AcceptedCount);
        Assert.Equal(1, missingTarget.RejectedCount);
        Assert.False(missingTarget.PresentationStateChanged);
        Assert.Equal(StyleTransitionOwnerRejectionReason.MissingRetainedTarget, missingTarget.OwnerResults[0].RejectionReason);
        Assert.Equal(0, compositor.StartCount);
        Assert.Equal(0, compositor.CancelCount);
    }

    [Fact]
    public void StyleTransitionRuntimeCoordinator_validate_batch_reports_per_owner_results_against_one_snapshot_without_compositor_install()
    {
        var snapshot = CreateMultiTargetStyleTransitionRetainedSnapshot();
        ...
        var increment = new StyleTransitionDecisionBatchEntry(
            ...
            StyleTransitionRuntimeDecision.Start(
                ...
                CompositionAnimationEasing.SineOut,
                CompositionAnimationRepeatMode.Once,
                new CompositionAnimationInstanceId(21)));
        ...
        Assert.Equal(StyleTransitionBatchResultKind.Accepted, result.Kind);
        ...
        Assert.Collection(
            ...
            second =>
            {
                ...
                Assert.Equal(StyleTransitionOwnerValidationKind.Accepted, second.Kind);
    ...
    [Fact]
    public async Task StyleTransitionRuntimeCoordinator_batch_clear_clears_active_presentation_and_owner_table_when_all_ready()
    {
        var cancellationToken = TestContext.Current.CancellationToken;
        var pipeline = new RenderPipeline();
        using var backend = new StyleTransitionCompositionBackend();
        ...
        var firstOwner = StyleTransitionOwnerKey.ControlState(ActionIdRegistry.Increment, new NodeKey(22));
        var secondOwner = StyleTransitionOwnerKey.ControlState(ActionIdRegistry.Decrement, new NodeKey(23));
        var startBatch = StyleTransitionDecisionBatch.Create([
            new StyleTransitionDecisionBatchEntry(
                firstOwner,
                StyleTransitionRuntimeDecision.Start(
                    new NodeKey(22),
                    ...
                    CompositionAnimationEasing.SineOut,
                    CompositionAnimationRepeatMode.Once,
                    new CompositionAnimationInstanceId(131))),
            new StyleTransitionDecisionBatchEntry(
                secondOwner,
                StyleTransitionRuntimeDecision.Start(
                    new NodeKey(23),
                    ...
                    BuildCompositionStyleProperties(opacity: 0.85, translateX: -8, translateY: 2),
                    CompositionTimestamp.FromStopwatchTicks(100),
                    CompositionDuration.FromStopwatchTicks(75),
                    CompositionAnimationEasing.SineOut,
                    CompositionAnimationRepeatMode.Once,
                    new CompositionAnimationInstanceId(132)))
        ]);
        var activation = coordinator.ActivateBatchPresentation(
            startBatch,
            new DrawingBackendStyleTransitionPresentationActivationCompositorAdapter(compositor),
            new FixedStyleTransitionRetainedSnapshotProvider(snapshot),
            tracker,
            cancellationToken);
        var clearBatch = StyleTransitionDecisionBatch.Create([
            new StyleTransitionDecisionBatchEntry(firstOwner, StyleTransitionRuntimeDecision.Cancel(new NodeKey(22))),
            new StyleTransitionDecisionBatchEntry(secondOwner, StyleTransitionRuntimeDecision.Commit(new NodeKey(23)))
        ]);

        Assert.Equal(StyleTransitionBatchPresentationActivationKind.Activated, activation.Kind);
        Assert.NotNull(compositor.CompositionAnimationPresentationPlan);

        var cleared = coordinator.ClearBatchPresentation(
            clearBatch,
            new DrawingBackendStyleTransitionPresentationClearCompositorAdapter(compositor),
            new FixedStyleTransitionRetainedSnapshotProvider(snapshot),
            tracker,
            cancellationToken);

        Assert.Equal(StyleTransitionBatchPresentationClearKind.Cleared, cleared.Kind);
        Assert.Equal(StyleTransitionBatchPresentationClearReason.None, cleared.Reason);
        Assert.True(cleared.IsCleared);
        Assert.True(cleared.PresentationStateChanged);
        Assert.Equal(StyleTransitionBatchRuntimePreflightKind.Ready, cleared.RuntimePreflight.Kind);
        Assert.True(cleared.RuntimePreflight.RequiresTrackedOwnerClear);
        Assert.Equal(2, cleared.ClearedOwnerCount);
        Assert.Equal(StyleTransitionCompletionResultKind.Cleared, cleared.LastTrackerResult.Kind);
        Assert.Equal(secondOwner, cleared.LastTrackerResult.OwnerKey);
        Assert.False(tracker.HasActiveTransition);
        Assert.Equal(0, tracker.ActiveOwnerCount);
        Assert.Null(compositor.CompositionAnimationPresentationPlan);
        Assert.Null(compositor.CompositionAnimationPlan);
    ...
    [Fact]
    public async Task StyleTransitionRuntimeCoordinator_batch_clear_removes_one_owner_and_preserves_remaining_presentation()
    {
        var cancellationToken = TestContext.Current.CancellationToken;
        var pipeline = new RenderPipeline();
        using var backend = new StyleTransitionCompositionBackend();
        ...
        var startBatch = StyleTransitionDecisionBatch.Create([
            new StyleTransitionDecisionBatchEntry(
                ...
                StyleTransitionRuntimeDecision.Start(
                    ...
                    CompositionAnimationEasing.SineOut,
                    CompositionAnimationRepeatMode.Once,
                    new CompositionAnimationInstanceId(135))),
            new StyleTransitionDecisionBatchEntry(
                secondOwner,
                StyleTransitionRuntimeDecision.Start(
                    new NodeKey(23),
                    ...
                    CompositionAnimationEasing.SineOut,
                    CompositionAnimationRepeatMode.Once,
                    new CompositionAnimationInstanceId(136)))
        ]);
        var activation = coordinator.ActivateBatchPresentation(
            startBatch,
            new DrawingBackendStyleTransitionPresentationActivationCompositorAdapter(compositor),
        ...
        Assert.True(cleared.IsCleared);
        Assert.True(cleared.PresentationStateChanged);
        Assert.Equal(1, cleared.ClearedOwnerCount);
        Assert.Equal(StyleTransitionCompletionResultKind.Cleared, cleared.LastTrackerResult.Kind);
        Assert.Equal(firstOwner, cleared.LastTrackerResult.OwnerKey);
        Assert.Equal(1, tracker.ActiveOwnerCount);
        ...
        Assert.Equal(new CompositionAnimationInstanceId(136), remainingInstance);
        Assert.NotNull(compositor.CompositionAnimationPresentationPlan);
        ...
        var tick = await compositor.RenderCompositionAnimationPresentationTickAtAsync(CompositionTimestamp.FromStopwatchTicks(120), cancellationToken);

        Assert.Equal(1, tick.LayerCount);
        Assert.Null(compositor.CompositionAnimationPlan);
    }

    [Fact]
    public async Task StyleTransitionRuntimeCoordinator_batch_clear_blocks_mixed_clear_without_presentation_or_tracker_mutation()
    {
        var cancellationToken = TestContext.Current.CancellationToken;
        var pipeline = new RenderPipeline();
        using var backend = new StyleTransitionCompositionBackend();
        ...
        var firstOwner = StyleTransitionOwnerKey.ControlState(ActionIdRegistry.Increment, new NodeKey(22));
        var secondOwner = StyleTransitionOwnerKey.ControlState(ActionIdRegistry.Decrement, new NodeKey(23));
        var missingOwner = StyleTransitionOwnerKey.ControlState(ActionIdRegistry.Reset, new NodeKey(23));
        var startBatch = StyleTransitionDecisionBatch.Create([
            new StyleTransitionDecisionBatchEntry(
                firstOwner,
                StyleTransitionRuntimeDecision.Start(
                    ...
                    CompositionAnimationEasing.SineOut,
                    CompositionAnimationRepeatMode.Once,
                    new CompositionAnimationInstanceId(141))),
            new StyleTransitionDecisionBatchEntry(
                secondOwner,
                StyleTransitionRuntimeDecision.Start(
                    new NodeKey(23),
                    ...
                    CompositionAnimationEasing.SineOut,
                    CompositionAnimationRepeatMode.Once,
                    new CompositionAnimationInstanceId(142)))
        ]);
        var activation = coordinator.ActivateBatchPresentation(
            startBatch,
            new DrawingBackendStyleTransitionPresentationActivationCompositorAdapter(compositor),
            ...
            tracker,
            cancellationToken);
        var blockedBatch = StyleTransitionDecisionBatch.Create([
            new StyleTransitionDecisionBatchEntry(firstOwner, StyleTransitionRuntimeDecision.Cancel(new NodeKey(22))),
            new StyleTransitionDecisionBatchEntry(missingOwner, StyleTransitionRuntimeDecision.Commit(new NodeKey(23)))
        ]);

        var blocked = coordinator.ClearBatchPresentation(
            blockedBatch,
            new DrawingBackendStyleTransitionPresentationClearCompositorAdapter(compositor),
            new FixedStyleTransitionRetainedSnapshotProvider(snapshot),
            tracker,
        ...
        Assert.Equal(StyleTransitionBatchPresentationActivationKind.Activated, activation.Kind);
        Assert.Equal(StyleTransitionBatchPresentationClearKind.Blocked, blocked.Kind);
        Assert.Equal(StyleTransitionBatchPresentationClearReason.RuntimePreflightNotReady, blocked.Reason);
        Assert.Equal(StyleTransitionBatchRuntimePreflightKind.Mixed, blocked.RuntimePreflight.Kind);
        Assert.False(blocked.PresentationStateChanged);
        Assert.Equal(0, blocked.ClearedOwnerCount);
        Assert.Equal(2, tracker.ActiveOwnerCount);
        Assert.True(tracker.TryGetActiveTransition(firstOwner, out var firstTarget, out var firstInstance));
        Assert.True(tracker.TryGetActiveTransition(secondOwner, out var secondTarget, out var secondInstance));
        Assert.Equal(new NodeKey(22), firstTarget);
        Assert.Equal(new CompositionAnimationInstanceId(141), firstInstance);
        Assert.Equal(new NodeKey(23), secondTarget);
        Assert.Equal(new CompositionAnimationInstanceId(142), secondInstance);
        Assert.NotNull(compositor.CompositionAnimationPresentationPlan);
    }
    ...
    public async Task StyleTransitionCompletionPump_presentation_tick_clears_completed_targets_and_plan_after_last_owner()
    {
        ...
        var batch = StyleTransitionDecisionBatch.Create([
            new StyleTransitionDecisionBatchEntry(
                ...
                StyleTransitionRuntimeDecision.Start(
                    ...
                    CompositionAnimationEasing.SineOut,
                    CompositionAnimationRepeatMode.Once,
                    new CompositionAnimationInstanceId(151))),
            new StyleTransitionDecisionBatchEntry(
                secondOwner,
                StyleTransitionRuntimeDecision.Start(
                    new NodeKey(23),
                    ...
                    CompositionAnimationEasing.SineOut,
                    CompositionAnimationRepeatMode.Once,
                    new CompositionAnimationInstanceId(152)))
        ]);
        var activation = coordinator.ActivateBatchPresentation(
            batch,
            new DrawingBackendStyleTransitionPresentationActivationCompositorAdapter(compositor),
        ...
        var firstTick = await pump.TickPresentationAndDrainAtAsync(CompositionTimestamp.FromStopwatchTicks(100), cancellationToken);
        var firstCompletion = await pump.TickPresentationAndDrainAtAsync(CompositionTimestamp.FromStopwatchTicks(150), cancellationToken);
        var afterFirstCompletion = pump.CaptureDiagnosticSnapshot();

        Assert.Equal(StyleTransitionBatchPresentationActivationKind.Activated, activation.Kind);
        Assert.Equal(StyleTransitionCompletionPumpResultKind.TickRendered, firstTick.Kind);
        Assert.Equal(0, firstTick.DrainedEvents);
        Assert.Equal(StyleTransitionCompletionPumpResultKind.CompletionCommitted, firstCompletion.Kind);
        Assert.True(firstCompletion.HasCommit);
        Assert.Equal(StyleTransitionRuntimeResultKind.Committed, firstCompletion.CommitResult.Kind);
        Assert.Equal(new NodeKey(22), firstCompletion.CommitResult.TargetKey);
        Assert.Equal(1, firstCompletion.DrainedEvents);
        Assert.Equal(1, tracker.ActiveOwnerCount);
        Assert.False(tracker.TryGetActiveTransition(firstOwner, out _, out _));
        Assert.True(tracker.TryGetActiveTransition(secondOwner, out var remainingTarget, out var remainingInstance));
        Assert.Equal(new NodeKey(23), remainingTarget);
        ...
        var secondCompletion = await pump.TickPresentationAndDrainAtAsync(CompositionTimestamp.FromStopwatchTicks(200), cancellationToken);
        var finalSnapshot = pump.CaptureDiagnosticSnapshot();

        Assert.Equal(StyleTransitionCompletionPumpResultKind.CompletionCommitted, secondCompletion.Kind);
        Assert.True(secondCompletion.HasCommit);
        Assert.Equal(StyleTransitionRuntimeResultKind.Committed, secondCompletion.CommitResult.Kind);
        Assert.Equal(new NodeKey(23), secondCompletion.CommitResult.TargetKey);
        Assert.Equal(1, secondCompletion.DrainedEvents);
        Assert.False(tracker.HasActiveTransition);
        Assert.Null(compositor.CompositionAnimationPresentationPlan);
        ...
        Assert.Equal(3, backend.ExecuteCompositionCount);
    }
    ...
    public async Task StyleTransitionRuntimeCoordinator_batch_validation_preserves_single_owner_apply_path()
    {
        ...
        var decision = StyleTransitionRuntimeDecision.Start(
            ...
            CompositionAnimationEasing.SineOut,
            CompositionAnimationRepeatMode.Once,
            new CompositionAnimationInstanceId(31));
        var batch = StyleTransitionDecisionBatch.Create([
            new StyleTransitionDecisionBatchEntry(
                StyleTransitionOwnerKey.ControlState(ActionIdRegistry.Increment, new NodeKey(22)),
                decision)
        ...
        var validation = coordinator.ValidateBatch(
            batch,
            new FixedStyleTransitionRetainedSnapshotProvider(snapshot),
            TestContext.Current.CancellationToken);

        Assert.Equal(StyleTransitionBatchResultKind.Accepted, validation.Kind);
        Assert.Equal(1, validation.AcceptedCount);
        Assert.False(validation.PresentationStateChanged);
        Assert.Equal(0, compositor.StartCount);
        Assert.Equal(0, compositor.CancelCount);

        var applied = await StyleTransitionRuntimeCoordinator.ApplyDecisionAsync(
            decision,
            compositor,
            new FixedStyleTransitionRetainedSnapshotProvider(snapshot),
            cancellationToken);

        Assert.Equal(StyleTransitionRuntimeResultKind.Started, applied.Kind);
        Assert.Equal(StyleTransitionCompileStatus.CompiledCompositionDeclaration, applied.CompileStatus);
        Assert.True(applied.HasDeclaration);
        Assert.Equal(1, compositor.StartCount);
        Assert.Equal(0, compositor.CancelCount);
        Assert.Equal(new NodeKey(22), compositor.LastDeclaration.TargetKey);
        Assert.Equal(new CompositionAnimationInstanceId(31), compositor.LastDeclaration.InstanceId);
    }

    [Fact]
    public void CounterStyleTransitionBridge_routes_multi_target_delta_to_transition_batch()
    {
        ...
        Assert.Collection(
            ...
            first =>
            {
                ...
                Assert.Equal(new CompositionAnimationInstanceId(1002), first.Decision.InstanceId);
            },
            second =>
            {
    ...
    [Fact]
    public void CounterStyleTransitionBridge_keeps_legacy_single_decision_api_narrow_while_batch_api_accepts_multi_target()
    {
        var previous = new OwnershipSnapshot(
            ActionIdRegistry.Increment,
            default,
        ...
        var mapped = CounterStyleTransitionBridge.TryCreateDecision(
            ...
            CompositionTimestamp.FromStopwatchTicks(100),
            out var decision);
        var batched = CounterStyleTransitionBridge.TryCreateDecisionBatch(
            previous,
            next,
            CompositionTimestamp.FromStopwatchTicks(100),
            out var batch);
    ...
    [Fact]
    public void CounterStyleTransitionBridge_lifecycle_routes_active_scroll_to_normal_dispatch_but_multi_target_to_batch()
    {
        var previous = new OwnershipSnapshot(
            ActionIdRegistry.Increment,
            default,
        ...
        Assert.Equal(CounterStyleTransitionLifecycleAction.ApplyTransitionBatch, multiTarget.Action);
        Assert.Equal(CounterStyleTransitionLifecycleReason.MultiTargetControlStateDelta, multiTarget.Reason);
        Assert.False(multiTarget.RequiresNormalDispatch);
        Assert.False(multiTarget.RequiresStyleTransitionAbort);
        Assert.Equal(CounterStyleTransitionLifecyclePresentationPolicy.Preserve, multiTarget.PresentationPolicy);
        Assert.False(multiTarget.HasTransitionDecision);
        Assert.True(multiTarget.HasTransitionBatch);
        Assert.Equal(default, multiTarget.Decision);
        Assert.Equal(2, multiTarget.Batch.Count);
    }
    ...
    public async Task CounterStyleTransitionRuntimeBridge_multi_target_batch_dispatch_activates_presentation_set_and_tracks_owners()
    {
        ...
        var previous = new OwnershipSnapshot(
            ...
            HoverChangeCount: 1,
            IsPointerPressed: false);
        await runtime.DispatchAndWaitAsync(new CounterMessage.InputVisualStateChanged(previous), cancellationToken);
        ...
        var decrementOwner = StyleTransitionOwnerKey.ControlState(ActionIdRegistry.Decrement, new NodeKey(7));
        Assert.True(tracker.TryGetActiveTransition(incrementOwner, out var incrementTarget, out var incrementInstance));
        Assert.True(tracker.TryGetActiveTransition(decrementOwner, out var decrementTarget, out var decrementInstance));
        Assert.Equal(new NodeKey(6), incrementTarget);
        Assert.Equal(new CompositionAnimationInstanceId(1002), incrementInstance);
        Assert.Equal(new NodeKey(7), decrementTarget);
        Assert.Equal(new CompositionAnimationInstanceId(2002), decrementInstance);
    ...
    [Fact]
    public async Task CounterStyleTransitionRuntimeBridge_active_scroll_normal_dispatch_aborts_active_style_transition()
    {
        var cancellationToken = TestContext.Current.CancellationToken;
        var window = new FakeWindow(new ScreenRegion(0, new PixelRectangle(0, 0, 960, 540)));
        var translator = new WindowDrawCommandTranslator(window);
        ...
        var hoverDecrementAndLeaveIncrement = new OwnershipSnapshot(
            ...
            HoverChangeCount: 2,
            IsPointerPressed: false);
        var activeScrollLifecycle = CounterStyleTransitionBridge.EvaluateInputTransition(
            runtime.CurrentModel.InputOwnership,
            hoverDecrementAndLeaveIncrement,
            hasActiveScrollPresentation: true,
            CompositionTimestamp.FromStopwatchTicks(150));

        Assert.Equal(CounterStyleTransitionLifecycleReason.ActiveScrollPresentation, activeScrollLifecycle.Reason);
        Assert.True(activeScrollLifecycle.RequiresNormalDispatch);
        Assert.True(activeScrollLifecycle.RequiresStyleTransitionAbort);

        var abortResult = Program.AbortStyleTransitionPresentationForRuntime(
            activeScrollLifecycle,
            tracker,
            new DrawingBackendStyleTransitionCompositorAdapter(compositor),
            cancellationToken);
        await runtime.DispatchAndWaitAsync(
        ...
        Assert.Null(compositor.CompositionAnimationPlan);
        var currentOwnership = runtime.CurrentModel.InputOwnership;
        Assert.Equal(hoverDecrementAndLeaveIncrement.HoveredTarget, currentOwnership.HoveredTarget);
        Assert.Equal(hoverDecrementAndLeaveIncrement.FocusedTarget, currentOwnership.FocusedTarget);
        Assert.Equal(hoverDecrementAndLeaveIncrement.PressedTarget, currentOwnership.PressedTarget);
        Assert.Equal(hoverDecrementAndLeaveIncrement.CapturedTarget, currentOwnership.CapturedTarget);
        Assert.Equal(hoverDecrementAndLeaveIncrement.LastHoverEnteredTarget, currentOwnership.LastHoverEnteredTarget);
    ...
    [Fact]
    public async Task CounterStyleTransitionRuntimeBridge_blocked_multi_target_batch_aborts_stale_presentation_after_dispatch()
    {
        var cancellationToken = TestContext.Current.CancellationToken;
        var window = new FakeWindow(new ScreenRegion(0, new PixelRectangle(0, 0, 960, 540)));
        var translator = new WindowDrawCommandTranslator(window);
        ...
        var lifecycle = CounterStyleTransitionBridge.EvaluateInputTransition(
            ...
            CompositionTimestamp.FromStopwatchTicks(150));
        var blockedBatch = StyleTransitionDecisionBatch.Create([
            lifecycle.Batch.Entries[0],
            new StyleTransitionDecisionBatchEntry(
                StyleTransitionOwnerKey.ControlState(ActionIdRegistry.Decrement, new NodeKey(99)),
                StyleTransitionRuntimeDecision.Start(
                    new NodeKey(99),
                    BuildCompositionStyleProperties(opacity: 1, translateX: 0, translateY: 0),
                    BuildCompositionStyleProperties(opacity: 0.98, translateX: 0, translateY: 0),
                    CompositionTimestamp.FromStopwatchTicks(150),
                    CompositionDuration.FromMilliseconds(120),
                    CompositionAnimationEasing.SineOut,
                    CompositionAnimationRepeatMode.Once,
                    new CompositionAnimationInstanceId(2002)))
        ]);
    ...
    private RenderPipelineRetainedInputSnapshot CreateStyleTransitionRetainedSnapshot()
    {
        var pipeline = new RenderPipeline();
        var viewport = new PixelRectangle(0, 0, 960, 540);
        var root = VirtualNodeFactory.ScrollContainer(new NodeKey(1),
    ...
    private RenderPipelineRetainedInputSnapshot CreateMultiTargetStyleTransitionRetainedSnapshot()
    {
        var pipeline = new RenderPipeline();
        var viewport = new PixelRectangle(0, 0, 960, 540);
