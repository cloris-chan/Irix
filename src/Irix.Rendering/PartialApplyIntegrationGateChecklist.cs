...
internal static class PartialApplyIntegrationGateChecklist
{
    ...
    [
        ...
        new(
            ...
            "Retained-frame segment ownership tests cover default-off no segmented owner, enabled full apply, accepted partial, fallback full, empty batch, malformed dirty ranges, hit target projection failure, disposal, and TryReadFrame survival without making the owner a render source.",
            "None; production RetainedRenderFrame does not own multiple retained snapshots across command ranges.",
        ...
        new(
            ...
            "Retained-frame segment ownership stores secondary hit target metadata, uses HitTargetMetadataProjector for accepted partial rehearsal, falls back on projection failure, and does not change production hit-test runtime; compositor shadow probe covers hit-test no-change including clipped target rejection.",
        ...
        new(
            ...
            "Production-off no-change tests prove retained-frame segment ownership and the feed do not alter DrawingBackendCompositor counters, backend Execute calls, dirty range propagation, hit-test results, or diagnostics text in default-off or enabled-secondary modes; enabled tests prove rendering still executes the normal retained frame.",
        ...
        new(
            ...
            "Production-off tests cover retained-frame segment ownership default-off and enabled-secondary equivalence, accepted partial rehearsal, owner-side hit target projection, projection failure fallback, four-piece atomicity, fallback, rebuild, empty batch, malformed dirty ranges, dispose, compositor probe hardening, dirty-range isolation, throw path, clipped hit-test, and diagnostics text no-change.",
