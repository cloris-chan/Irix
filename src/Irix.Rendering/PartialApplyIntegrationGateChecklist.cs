...
internal static class PartialApplyIntegrationGateChecklist
{
    ...
    [
        new(
            ...
            "Default-off RetainedRenderFrameSegmentOwnership sits beside the production RetainedRenderFrame, can opt in a segmented owner that follows normal batches, and RetainedRenderFrameHandoffHarness executes RuntimeOwner.ReadSegments through the per-segment adapter while TryReadFrame remains unchanged.",
        ...
        new(
            ...
            "Production-off no-change tests prove retained-frame segment ownership, the feed, and the default-off/enabled/missing-owner handoff harness do not alter DrawingBackendCompositor counters, backend Execute calls, dirty range propagation, hit-test results, or diagnostics text; enabled harness uses segment-local dirty ranges and internal handoff counter semantics only.",
