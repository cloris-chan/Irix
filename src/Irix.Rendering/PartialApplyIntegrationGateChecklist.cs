...
internal static class PartialApplyIntegrationGateChecklist
{
    ...
    [
        new(
            ...
            "Default-off RetainedRenderFrameSegmentOwnership sits beside the production RetainedRenderFrame, can opt in a segmented owner that follows normal batches, and DrawingBackendCompositorHandoffSeam invokes RetainedRenderFrameHandoffHarness as a production-adjacent candidate while TryReadFrame remains unchanged.",
        ...
        new(
            ...
            "Production-owner feed result reports Disabled, ShadowAppliedPartial, ShadowFallbackFull, fallback-applied, and owner-state-preserved-before-fallback flags; DrawingBackendCompositorHandoffSeam maps candidate outcomes to Disabled, MissingOwner, Executed, FallbackFull, and Rejected for tests/internal callers only.",
        ...
        new(
            ...
            "Production-adjacent no-change tests prove retained-frame segment ownership, the feed, and DrawingBackendCompositorHandoffSeam do not alter DrawingBackendCompositor counters, backend Execute calls, dirty range propagation, hit-test results, or diagnostics text when disabled or enabled; enabled seam uses segment-local dirty ranges and internal handoff counter semantics only.",
        ...
        new(
            ...
            "Production-adjacent tests cover retained-frame segment ownership and handoff seam default-off lazy allocation, missing-owner, enabled-secondary equivalence, internal result reporting, candidate render-source backend call order, accepted partial rehearsal, rejected partial fallback, owner-side hit target lookup, projection failure fallback, multiple FrameDrawingResources lifecycle exact-once, repeated replacement, handoff counter semantics, segment-local dirty-range handoff, dirty-range mismatch routing, four-piece atomicity, fallback, rebuild, empty batch, malformed dirty ranges, dispose, compositor probe hardening, dirty-range isolation, throw path, clipped/no-hit hit-test, and diagnostics text no-change.",
