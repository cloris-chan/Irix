...
internal static class PartialApplyIntegrationGateChecklist
{
    ...
    [
        new(
            ...
            "Default-off SegmentedRetainedFrameProductionOwnerFeed can opt in a runtime owner that follows normal RenderPipeline.Build batches and exposes segmented reads plus owner-side hit target metadata while rendering still uses RetainedRenderFrame.",
        ...
        new(
            ...
            "Production-owner feed tests cover default-off no allocation, enabled full apply, accepted partial, fallback full, rebuild, empty batch, malformed dirty ranges, hit target projection failure, and runtime-owner disposal without making the owner a render source.",
        ...
        new(
            ...
            "Production-off feed stores secondary hit target metadata, uses HitTargetMetadataProjector for accepted partial rehearsal, falls back on projection failure, and does not change hit-test runtime; compositor shadow probe covers hit-test no-change including clipped target rejection.",
        ...
        new(
            ...
            "Production-owner feed accepted partial rehearsal advances the runtime owner's retained root metadata and hit target metadata while RenderPipeline's production retained root baseline remains unchanged.",
        ...
        new(
            ...
            "Production-off tests cover default-off equivalence, enabled-secondary equivalence, accepted partial rehearsal, owner-side hit target projection, projection failure fallback, four-piece atomicity, fallback, rebuild, empty batch, malformed dirty ranges, compositor probe hardening, dirty-range isolation, throw path, clipped hit-test, and diagnostics text no-change.",
