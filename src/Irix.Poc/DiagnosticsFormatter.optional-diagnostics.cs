...
internal static class DiagnosticsFormatter
{
    internal static string[] BuildBackendDeviceDiagnosticLines(BackendClipTextDiagnosticSnapshot snapshot)
    {
        return [
            ...
            BuildMaterialOutputDiagnosticLine(snapshot.MaterialOutput)
        ];
    }
    ...
    {
        return $"style-transition-completion-pump status isRunning={snapshot.IsRunning} hasActiveTransition={snapshot.HasActiveTransition} activeTarget={FormatNodeKey(snapshot.ActiveTargetKey)} activeInstance={FormatInstanceId(snapshot.ActiveInstanceId)} activeOwnerCount={snapshot.ActiveOwnerCount} activeOwnerKind={snapshot.ActiveOwnerKind} tickMode={snapshot.TickMode} lastResult={snapshot.LastResult.Kind} lastDrainedEvents={snapshot.LastResult.DrainedEvents} lastCommitResult={snapshot.LastResult.CommitResult.Kind} lastCommitTarget={FormatNodeKey(snapshot.LastResult.CommitResult.TargetKey)} trackerResult={snapshot.TrackerResult.Kind} trackerTarget={FormatNodeKey(snapshot.TrackerResult.TargetKey)} trackerInstance={FormatInstanceId(snapshot.TrackerResult.InstanceId)} tickCount={snapshot.TickCount} commitCount={snapshot.CommitCount} drainedEvents={snapshot.DrainedEventCount} hasError={snapshot.HasError} error={snapshot.LastErrorKind ?? "-"}";
    }

    internal static string BuildStyleTransitionBatchActivationDiagnosticLine(StyleTransitionBatchActivationDiagnosticSnapshot snapshot)
    {
        return $"style-transition-batch-activation status activationKind={snapshot.ActivationKind} activationReason={snapshot.ActivationReason} runtimePreflight={snapshot.RuntimePreflightKind} runtimeReady={snapshot.RuntimeReadyCount} runtimeBlocked={snapshot.RuntimeBlockedCount} presentationPreflight={snapshot.PresentationPreflightKind} presentationAccepted={snapshot.PresentationAcceptedCount} presentationRejected={snapshot.PresentationRejectedCount} declarationCount={snapshot.DeclarationCount} trackedOwnerCount={snapshot.TrackedOwnerCount} presentationStateChanged={snapshot.PresentationStateChanged} cleanupResult={snapshot.CleanupKind} cleanupTarget={FormatNodeKey(snapshot.CleanupTargetKey)} cleanupApplied={snapshot.CleanupApplied} activeAfterCleanup={snapshot.HasActiveTransitionAfterCleanup} activeOwnerCountAfterCleanup={snapshot.ActiveOwnerCountAfterCleanup} presentationPlanAfterCleanup={snapshot.HasPresentationPlanAfterCleanup}";
    }
