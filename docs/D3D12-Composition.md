...
- A `--diagnose-scroll-presentation-interaction` diagnostic that runs the real Counter app/coordinator/compositor/backend path for active pointer interaction, chained wheel retarget, `EnsureRunning` rapid-wheel coalescing, and clamped top/bottom boundary wheel input.
