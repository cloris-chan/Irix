...
- Typed composition clock values: `CompositionTimestamp` and `CompositionDuration` carry high-precision Irix `CompositionClock` ticks and keep frame indexes, present counters, and refresh-rate ticks out of animation progress. `DrawingBackendCompositor`, `CompositorLoop`, and Poc `StyleTransitionCompletionPump` use an internal `ICompositionClockSource` for compositor-owned ticks, delayed tick sampling, and automatic marker-completion ticks while D3D12 `Present` remains pacing, not animation timeline authority.
