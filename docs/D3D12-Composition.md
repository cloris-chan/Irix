...
- Typed composition clock values: `CompositionTimestamp` and `CompositionDuration` carry high-precision Irix `CompositionClock` ticks and keep frame indexes, present counters, and refresh-rate ticks out of animation progress. `CompositorLoop` scroll presentation scheduling uses an internal `ICompositionClockSource` for delayed tick sampling while D3D12 `Present` remains pacing, not animation timeline authority. Poc `StyleTransitionCompletionPump` samples that same clock source for automatic marker-completion ticks.
