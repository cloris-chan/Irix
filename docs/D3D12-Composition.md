...
- `DrawingBackendCompositor` private execution preflight for compositor-only ticks and retained scroll refresh: active plan ownership stays in the typed tick/refresh paths, while backend capability checks, retained-frame availability, and skipped-execution diagnostics are recorded through one internal path.
...
- Typed composition clock values: `CompositionTimestamp` and `CompositionDuration` carry high-precision Irix `CompositionClock` ticks, use saturating timestamp arithmetic for extreme samples, and keep frame indexes, present counters, and refresh-rate ticks out of animation progress. `DrawingBackendCompositor`, `CompositorLoop`, and Poc `StyleTransitionCompletionPump` plus `ScrollPresentationCoordinator` use an internal `ICompositionClockSource` for compositor-owned ticks, delayed tick sampling, automatic marker-completion ticks, and scroll retarget segment starts while D3D12 `Present` remains pacing, not animation timeline authority.
...
Before any compositor-only tick or retained scroll-presentation refresh reaches `ICompositionDrawingBackend.ExecuteComposition`, `DrawingBackendCompositor` performs one private execution preflight: the backend must implement the requested composition capability, a retained command/resource frame must be readable, and the typed plan must still fit that retained frame. The preflight records the same skip reason and capability metadata used by optional diagnostics, but it does not add a public diagnostics contract, CLI surface, scheduler, or backend ownership channel.

Color output is intentionally narrow in the current D3D12 path. Draw commands retain canonical Irix color internally, then D3D12 maps through the current internal `ColorOutputMapping.SdrSrgb` stage into SDR/sRGB R8G8B8A8-style payloads. The backend does not yet own HDR swapchain selection, system SDR brightness, tone mapping, or Rec.2100 HLG/PQ output. The long-term Irix color contract is defined separately in [Color-Pipeline.md](Color-Pipeline.md): canonical color is linear BT.2020, and final mapping to sRGB or HDR backend payloads belongs to a future compositor/backend output mapping context. The HDR output mapping preflight in [Color-Pipeline.md](Color-Pipeline.md) is the gate for changing this backend; until that preflight is implemented as code, D3D12 swapchain creation, resize, rectangle PSO render-target formats, glyph atlas render-target formats, and diagnostic execution stay on the current SDR/sRGB R8G8B8A8 path.
...
- active presentation-set transform/opacity tick skip when no presentation plan set is active
- active presentation-set transform/opacity tick skip when the backend lacks `MultiLayer`
- active presentation-set transform/opacity execution clears the skip reason to `None`
