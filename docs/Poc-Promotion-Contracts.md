...
| `AppDispatchIntent`, `IAppMessageDispatchMapper`, `IControlFeedbackDispatchMapper`, and `CounterAppMessageDispatchMapper` | `Irix.Poc` app message adapter | Groups routed input results, ownership snapshots, max-scroll feedback, and scroll-presentation interrupt decisions into a Poc-owned intent before converting them into `CounterMessage` dispatch values. Runtime and marker dispatch use a separate Poc dispatch sink, and wheel raw input dispatches through a Poc scroll presentation sink; broader framework runtime ownership still needs a non-Counter action/message contract. |
