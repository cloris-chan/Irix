...
Current compositor-adjacent handoff seam: `DrawingBackendCompositorHandoffSeam`. It wraps an existing `DrawingBackendCompositor` instance without changing that class. Disabled mode never creates a `RetainedRenderFrameHandoffHarness` and renders only through the current compositor path. Enabled mode renders the current compositor path first, then invokes the harness as a candidate render source using the current compositor dirty ranges. The result vocabulary is internal only: `Disabled`, `MissingOwner`, `Executed`, `FallbackFull`, and `Rejected`. The seam can prove candidate backend call order, segment-local dirty ranges, owner-side hit-test, and throw handling beside the compositor, but it still does not select segmented reads as the production render source.
...
|------|------------|--------------------------|-----------------------------|
| Resource resolver ownership | Upgradeable from production-adjacent evidence, but not satisfied. | The retained-frame seam plus `DrawingBackendCompositorHandoffSeam` prove real batches can execute segmented reads as a candidate render source beside a real `RetainedRenderFrame` and compositor render. | The production render source still comes from `RetainedRenderFrame.TryReadFrame`; no production-owned selector consumes segmented reads. |
...
| Fallback reporting | Upgradeable to production-adjacent evidence, but not satisfied. | The seam returns internal `Disabled`, `MissingOwner`, `Executed`, `FallbackFull`, and `Rejected` without diagnostics formatter changes. | Production callers and compositor still do not consume local partial/fallback/rejected result vocabulary, and CLI diagnostics must stay unchanged. |
| Compositor ownership | Closest to production runtime evidence, but still not satisfied. | Default-off lazy allocation, missing-owner, enabled, empty, throw, dirty-range mismatch, counter, hit-test, and diagnostics no-change sentinels prove candidate execution beside the compositor. | `DrawingBackendCompositor` still owns one `RetainedRenderFrame`, never selects segmented reads, and has no default-off render-source selector in the main class. |
| Regression coverage | Upgradeable only after an actual default-off render-source path exists. | Focused production-adjacent coverage includes accepted partial, rejected partial fallback, fallback, rebuild, empty reads, missing owner, malformed ranges, exact-once resource lifecycle, repeated replacement, owner-side clipped/no-hit lookup, handoff counter semantics, segment-local dirty-range handoff, dirty-range mismatch routing, throw path, probe hardening, diagnostics no-change, and full no-change suite coverage. | A true production render-source path still needs disabled/enabled cross-suite equivalence before any gate can be satisfied. |

Closest to production runtime evidence today: resource resolver ownership, fallback reporting, hit target metadata projection, and compositor ownership. They are still not satisfied because the production compositor class does not select segmented reads and production hit-test/counter ownership remains unchanged.
...
Implemented prototype seam: `DrawingBackendCompositorHandoffSeam` with `DrawingBackendCompositorHandoffOptions`. It is not a formal `DrawingBackendCompositor` option and does not change the compositor constructor or main render path. Disabled mode leaves the candidate backend factory untouched, so no candidate harness/backend allocation occurs.
...
- The implemented seam is adjacent to `DrawingBackendCompositor`; the next round must decide whether to move the selector into the compositor while preserving the same rollback and no-change guarantees.
