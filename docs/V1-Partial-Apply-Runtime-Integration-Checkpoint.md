...
| Compositor ownership | Dry-run sentinel tests keep planner/projector/root patcher/segment reader flow outside compositor mutation. | `DrawingBackendCompositorShadowProbe` executes shadow segmented reads through the adapter outside `DrawingBackendCompositor` and verifies execute order, resolver ownership, and hit-test no-change. | The default-off `DrawingBackendCompositor` handoff selector proves lazy allocation/no-op, same-frame ownership freshness, and enabled candidate execution without changing compositor counters, backend execute calls, dirty ranges, hit-test results, or diagnostics text; enabled selector uses segment-local dirty ranges and internal handoff counters only. | `DrawingBackendCompositor` still owns one retained frame and selects the existing retained-frame read as the production render source. |
| Regression coverage | Focused preflight tests and existing layout/compositor tests cover no-change behavior. | Shadow owner, runtime owner seam, compositor shadow probe, diagnostic harness, result vocabulary, disabled no-change sentinel, opt-in end-to-end, fallback, rejection, and adapter tests cover the current shadow path. | Production-adjacent tests cover retained-frame segment ownership, main-compositor selector default-off lazy allocation, same-frame freshness guard, missing-owner, stale-owner, fallback, rejected, empty-read fallback, enabled-secondary equivalence, candidate render-source backend call order, internal result reporting, accepted partial, owner-side hit target lookup, projection failure fallback, multiple `FrameDrawingResources` lifecycle exact-once, repeated replacement, handoff counter semantics, segment-local dirty-range handoff, dirty-range mismatch routing, four-piece atomicity, rebuild, malformed dirty ranges, dispose, empty segments, throw path, clipped/no-hit hit-test, dirty-range isolation, and diagnostics text no-change. | Runtime partial apply has no production render-source hookup to cover yet. |
...
Current main-compositor handoff selector: `DrawingBackendCompositor` has an internal default-off constructor overload with `DrawingBackendCompositorHandoffOptions`. Disabled mode never creates a `RetainedRenderFrameHandoffHarness` and renders only through the current retained-frame path. Enabled mode renders the current retained-frame path first, then invokes the harness as a candidate render source only when `RetainedRenderFrameSegmentOwnership.LastResult` is fresh for the same batch and represents an accepted partial owner with non-empty reads. Stale owner, missing owner, rejected owner, fallback full, and empty reads all stay on the existing retained-frame path. The result vocabulary is internal only: `Disabled`, `MissingOwner`, `Executed`, `FallbackFull`, and `Rejected`. The selector can prove candidate backend call order, segment-local dirty ranges, owner-side hit-test, and throw handling inside the same compositor instance, but it still does not select segmented reads as the production render source.
...
|------|------------|--------------------------|-----------------------------|
| Resource resolver ownership | Upgradeable from production-adjacent evidence, but not satisfied. | The retained-frame seam plus `DrawingBackendCompositor` default-off handoff selector prove real batches can execute fresh accepted segmented reads as a candidate render source beside a real `RetainedRenderFrame` and compositor render. | The production render source still comes from `RetainedRenderFrame.TryReadFrame`; no production-owned selector chooses segmented reads. |
...
| Compositor ownership | Closest to production runtime evidence, but still not satisfied. | Default-off lazy allocation, same-frame freshness guard, missing-owner, stale-owner, fallback, rejected, empty, enabled fresh partial, throw, dirty-range routing, counter, hit-test, and diagnostics no-change sentinels prove candidate execution inside the same compositor instance. | `DrawingBackendCompositor` still owns one `RetainedRenderFrame` and never selects segmented reads as the production render source. |
| Regression coverage | Upgradeable only after an actual selected render-source path exists. | Focused production-adjacent coverage includes accepted partial, rejected owner, fallback, rebuild, empty reads, stale owner, missing owner, malformed ranges, exact-once resource lifecycle, repeated replacement, owner-side clipped/no-hit lookup, handoff counter semantics, segment-local dirty-range handoff, dirty-range mismatch routing, throw path, probe hardening, diagnostics no-change, and full no-change suite coverage. | A true production render-source path still needs selected-source cross-suite equivalence before any gate can be satisfied. |

Closest to production runtime evidence today: resource resolver ownership, fallback reporting, hit target metadata projection, and compositor ownership. They are still not satisfied because the production compositor class does not select segmented reads as its render source and production hit-test/counter ownership remains unchanged.
...
- Production compositor still never selects segmented reads as the production render source.
...
- The next round must decide whether the existing default-off selector may choose segmented reads as the selected render source while preserving the same rollback and no-change guarantees.
