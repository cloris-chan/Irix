...
> Checkpoint only. This document does not enable partial apply, does not change `RenderPipeline.Build`, does not change `RetainedRenderFrame`, does not change `IDrawingBackend.Execute`, does not change default compositor behavior, and does not make any gate satisfied.

## 1. Current Evidence Layers

| Gate | Preflight evidence exists | Shadow runtime evidence exists | Production-off runtime evidence exists | Production runtime evidence / still missing |
|------|---------------------------|--------------------------------|-----------------------------------------|------------------------------------------|
| Resource resolver ownership | `RetainedResourceSegmentTable`, `SegmentedRetainedFrameReader`, and segmented backend adapter prototype prove command-range resolver ownership. | `SegmentedRetainedFrameDiagnosticHarness` and `SegmentedRetainedFrameRuntimeOwner` feed real batches into `SegmentedRetainedFrameOwner`; `DrawingBackendCompositorShadowProbe` verifies segmented resolver ownership outside the compositor. | `RetainedRenderFrameSegmentOwnership` can sit beside the production `RetainedRenderFrame`; the default-off `DrawingBackendCompositor` handoff selector can invoke `RetainedRenderFrameHandoffHarness` against fresh accepted `RuntimeOwner.ReadSegments()` while `TryReadFrame` remains unchanged. | Production evidence now exists for one selected fresh accepted segmented frame through the real backend; production retained frame still does not expose segmented ownership through `TryReadFrame`. |
...
| Fallback reporting | `RetainedPartialApplyPlanner` reports local `AppliedPartial`, `FallbackFull`, and `Rejected` reasons. | Local shadow result vocabulary reports `Disabled`, `ShadowAppliedPartial`, `ShadowFallbackFull`, and `ShadowRejected` for tests/internal diagnostics only. | `DrawingBackendCompositor.LastHandoffResult` reports `Disabled`, `MissingOwner`, `Executed`, `FallbackFull`, and `Rejected` for tests/internal callers only. | No public runtime/compositor result propagation exists. |
| Compositor ownership | Dry-run sentinel tests keep planner/projector/root patcher/segment reader flow outside compositor mutation. | `DrawingBackendCompositorShadowProbe` executes shadow segmented reads through the adapter outside `DrawingBackendCompositor` and verifies execute order, resolver ownership, and hit-test no-change. | The default-off `DrawingBackendCompositor` handoff selector proves lazy allocation/no-op, same-frame ownership freshness, and enabled selected execution without changing diagnostics text; enabled selector uses segment-local dirty ranges and internal handoff counters only. | Production evidence now exists for selected segmented execution inside the main compositor, including real-backend segment calls and owner-side hit targets after success; broader ownership/lifecycle gates still block full hookup. |
| Regression coverage | Focused preflight tests and existing layout/compositor tests cover no-change behavior. | Shadow owner, runtime owner seam, compositor shadow probe, diagnostic harness, result vocabulary, disabled no-change sentinel, opt-in end-to-end, fallback, rejection, and adapter tests cover the current shadow path. | Production-adjacent tests cover retained-frame segment ownership, main-compositor selector default-off lazy allocation, same-frame freshness guard, missing-owner, stale-owner, fallback, rejected, empty-read fallback, enabled-secondary equivalence, selected render-source backend call order, internal result reporting, accepted partial, owner-side hit target lookup, projection failure fallback, multiple `FrameDrawingResources` lifecycle exact-once, repeated replacement, handoff counter semantics, segment-local dirty-range handoff, dirty-range mismatch routing, four-piece atomicity, rebuild, malformed dirty ranges, dispose, empty segments, throw path, clipped/no-hit hit-test, dirty-range isolation, and diagnostics text no-change. | Runtime coverage has first selected-source evidence but not enough to satisfy every gate. |

`PartialApplyIntegrationGateChecklist.CanHookUpPartialApply` must remain false until production runtime hookup evidence exists for every gate and the corresponding gate is explicitly marked satisfied. The first selected-source evidence is limited to resource resolver and compositor ownership.
...
Current main-compositor handoff selector: `DrawingBackendCompositor` has an internal default-off constructor overload with `DrawingBackendCompositorHandoffOptions`. Disabled mode never creates a `RetainedRenderFrameHandoffHarness` and renders only through the current retained-frame path. Enabled mode keeps the retained frame updated for rollback, then selects segmented reads as the actual backend render source only when `RetainedRenderFrameSegmentOwnership.LastResult` is fresh for the same batch and represents an accepted partial owner with non-empty reads. Stale owner, missing owner, rejected owner, fallback full, and empty reads all stay on the existing retained-frame path. The result vocabulary is internal only: `Disabled`, `MissingOwner`, `Executed`, `FallbackFull`, and `Rejected`. The selected path uses the real compositor backend, routes segment-local dirty ranges, commits owner-side hit targets only after selected execution succeeds, and leaves CLI diagnostics unchanged.
...
|------|------------|--------------------------|-----------------------------|
| Resource resolver ownership | Has first production runtime evidence, but not satisfied. | The retained-frame seam plus `DrawingBackendCompositor` default-off handoff selector prove real batches can execute fresh accepted segmented reads as the selected render source through the real backend. | The selected source still depends on secondary ownership, and broader resource dispose/lifecycle gates are not satisfied. |
...
| Fallback reporting | Upgradeable to production-adjacent evidence, but not satisfied. | `DrawingBackendCompositor.LastHandoffResult` returns internal `Disabled`, `MissingOwner`, `Executed`, `FallbackFull`, and `Rejected` without diagnostics formatter changes. | Production callers still do not consume local partial/fallback/rejected result vocabulary, and CLI diagnostics must stay unchanged. |
| Compositor ownership | Has first production runtime evidence, but not satisfied. | Default-off lazy allocation, same-frame freshness guard, missing-owner, stale-owner, fallback, rejected, empty, enabled selected partial, throw rollback, dirty-range routing, selected-source counters, owner-side hit-test, and diagnostics no-change sentinels prove selected execution inside the same compositor instance. | The selector is still internal/default-off, and remaining gates for resource lifecycle, command stability, retained-root update, and hit-target projection are not satisfied. |
| Regression coverage | Upgradeable, but still not satisfied. | Focused coverage includes selected accepted partial, rejected owner, fallback, rebuild, empty reads, stale owner, missing owner, malformed ranges, exact-once resource lifecycle, repeated replacement, owner-side clipped/no-hit lookup, handoff counter semantics, segment-local dirty-range handoff, dirty-range mismatch routing, throw path, probe hardening, diagnostics no-change, and full no-change suite coverage. | Selected-source coverage must stay green with the broader no-change suite before any gate is marked satisfied. |

Production runtime evidence now starts with resource resolver ownership and compositor ownership. They are still not satisfied because the selector is internal/default-off and the resource lifecycle, command stability, retained-root update, and metadata projection gates remain unsatisfied.
...
RenderPipeline existing full Build path
  ...
  -> default-off compositor selector may choose fresh accepted segmented reads for this frame
```
...
- Accepted partials may update the segmented owner state; the default-off compositor selector may use those reads as the selected source only when same-frame freshness and accepted-owner guards pass.
...
- Compositor tests prove disabled rendering still comes from `DrawingBackendCompositor`'s existing retained frame and enabled selected rendering can roll back on missing/stale/rejected/fallback/empty owner states.
...
Implemented production-off feed: `SegmentedRetainedFrameProductionOwnerFeed` follows the minimal access point above, but only as secondary state behind a default-off option. It does not modify `RenderPipeline.Build` and does not replace `RetainedRenderFrame`; the compositor selector consumes it only when explicitly enabled and fresh.
...
1. Keep `DrawingBackendCompositor` default behavior rendering from the existing `RetainedRenderFrame`; selected segmented rendering remains internal/default-off until every gate has production runtime proof.
...
3. Broaden counter semantics and segment-local dirty-range rules beyond the first selected-source path before any gate is satisfied.
4. Broaden owner-side hit-test lookup beyond selected-source tests only after production no-change equivalence remains green.
...
Rollback is to disable the option and stop constructing the segmented owner. No rollback step may touch `IDrawingBackend`, D3D12, or diagnostics formatting.
...
The option is owned by the compositor, not by `IDrawingBackend` or D3D12. Its default value is disabled. A disabled instance must take the exact current path: apply/read `RetainedRenderFrame`, set production dirty ranges once, call one backend `Execute`, use current compositor hit targets, and emit the same counters/diagnostics behavior.

Implemented default-off selector: `DrawingBackendCompositor` has an internal constructor overload with `DrawingBackendCompositorHandoffOptions` and an internal `LastHandoffResult`. Disabled mode leaves the candidate backend factory untouched, so no candidate harness/backend allocation occurs.
...
if option disabled:
  ...
  render existing RetainedRenderFrame path
else if ownership.LastResult is rejected or fallback:
  render existing RetainedRenderFrame path
...
| Counters | Exactly current compositor counters. | Existing compositor counters remain unchanged; candidate counters stay internal and apply only to the fresh executed candidate. |
| Hit-test | Current compositor hit targets. | Current compositor hit targets remain unchanged; owner-side hit target snapshot is observable only through internal candidate tests. |
...
- Selected segmented reads are now available only behind the internal default-off selector.
...
- The next round must decide whether selected-source coverage is broad enough to begin satisfying individual gates without expanding diagnostics or enabling StyleOnly fast-path.
