...
| 5 | Style transition scheduler or multi-owner design | Public style/transition authoring, Poc runtime ownership, first Counter app/control transition integration, and lifecycle preflight are written. Next slice should choose either a narrow transform/opacity completion scheduler or a multi-owner boundary for concurrent control-state deltas; keep renderer processing categories, theme/cascade, and public authoring names out of this path. |
| 6 | Color IR preflight | Add the canonical internal linear BT.2020 color value and conversion tests while keeping the current renderer on SDR/sRGB output. Do not implement HDR swapchains, Rec.2100 HLG/PQ, per-monitor color mapping, or tone mapping in this slice. |
