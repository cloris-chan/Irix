...
| D3D12 composition execution | [D3D12-Composition.md](D3D12-Composition.md) covers animation declarations/plans, scroll presentation declarations/plans, compositor-owned ticks, hit-test remapping, marker events, runtime marker dispatch, multi-layer frame execution, retained nested/mixed-clip decomposition, D3D12 layer content cache diagnostics including display-scale invalidation, backend capabilities, skip diagnostics, typed scroll runtime cancellation diagnostics, and visible diagnostic commands. | Keep ticks independent of UI rebuild; expand direct-path coverage only when a concrete behavior needs it. |
