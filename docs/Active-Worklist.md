...
| 2 | Main-app lifecycle matrix | Extend only narrow coverage around concrete bugs in resize, DPI, max-scroll feedback, render invalidation, queued compositor ticks, and active hit-test routing. Render invalidation stale delayed tick cancellation has focused compositor-loop coverage; keep lifecycle scenarios diagnostic-visible with stable presented-origin continuity and no generic fallback compositor. |
