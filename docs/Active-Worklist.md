...
- Style, animation, and GPU composition contracts exist. The active implementation direction is D3D12-backed transform/opacity, fixed-clip scroll presentation, retained target resolution, marker event publication, multi-layer composition execution, active hit-test remapping, main-app scroll presentation production, main-app interaction hardening, reason-typed lifecycle invalidation/cancellation, and D3D12 layer content caching.
...
| Scroll compositor | Fixed-clip scroll presentation is implemented for single-layer and decomposed nested/mixed-clip retained targets. Logical scroll remains app/control runtime; presented scroll is compositor-owned, and cancellation reasons plus main-app resize/DPI/max-scroll cancellation paths are diagnostic-visible. | Continue hardening interruption, lifecycle cancellation, and hit testing without a generic fallback compositor. |
| D3D12 composition execution | [D3D12-Composition.md](D3D12-Composition.md) covers animation declarations/plans, scroll presentation declarations/plans, compositor-owned ticks, hit-test remapping, marker events, runtime marker dispatch, multi-layer frame execution, retained nested/mixed-clip decomposition, D3D12 layer content cache diagnostics including display-scale and resource-resolver invalidation, backend capabilities, skip diagnostics, typed scroll runtime cancellation diagnostics, and visible diagnostic commands. | Keep ticks independent of UI rebuild; expand direct-path coverage only when a concrete behavior needs it. |
