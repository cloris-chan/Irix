...
| 4 | D3D12 direct composition hardening | Keep transform/opacity, fixed-clip scroll, nested/mixed-clip decomposition, marker dispatch, layer content cache, and skip diagnostics on the D3D12-first path. Recent boundary coverage pins fixed-clip and nested hit-test remapping plus fixed-clip cache-hit state. Add more tests only for concrete behavior boundaries, especially marker/skip interactions or main-app integration regressions. |
