...
| 5 | Style transition lifecycle design | Public style/transition authoring, Poc runtime ownership, and first Counter app/control transition integration are written. Next slice should design a narrow lifecycle/scheduler or multi-owner boundary for completion/commit, active scroll coexistence, and multi-target state deltas; keep renderer processing categories, theme/cascade, and public authoring names out of this path. |
