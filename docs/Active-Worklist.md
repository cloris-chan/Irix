...
- The scroll/input runtime owner boundary is written in `Poc-Promotion-Contracts.md`: logical scroll and input ownership belong to app/control runtime; layout feedback, compositor sampling, hit-test service access, input action mapping, and app message dispatch each require explicit adapter contracts before code moves. The first Poc-owned feedback sink, max-scroll feedback dispatch mapper, hit-test service, input action mapper, input-route app-message dispatch mapper, wheel-input scroll presentation dispatch sink, and app runtime dispatch sink cuts exist.
...
| 3 | Scroll/input ownership contract | Boundary is written in [Poc-Promotion-Contracts.md](Poc-Promotion-Contracts.md). The first Poc-owned scroll presentation adapter cut exists for runtime dispatch, compositor sampling/start, retained-snapshot access, control feedback delivery, max-scroll feedback message mapping, wheel-input scroll presentation dispatch, and app runtime dispatch; the input route now consumes Poc-owned hit-test service, input action mapper, and app-message dispatch mapper shapes for resolved action activation. Next code moves must continue broadening app-message-dispatch contracts rather than moving Counter/Poc runtime types wholesale. |
