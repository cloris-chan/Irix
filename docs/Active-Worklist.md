...
| 3 | Scroll/input ownership contract | Boundary is written in [Poc-Promotion-Contracts.md](Poc-Promotion-Contracts.md). The first Poc-owned scroll presentation adapter cut exists for runtime dispatch, compositor sampling/start, and retained-snapshot access; next code moves must continue introducing explicit feedback-sink, hit-test-service, and app-message-dispatch contracts rather than moving Counter/Poc runtime types wholesale. |
