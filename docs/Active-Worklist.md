...
|-------|------|------------|
| 1 | Scroll presentation continuity | Preserve the compositor-thread sample-and-cancel retarget invariant: same-target boundary input must not cancel/restart, and replacement segments must start from the latest compositor-presented value. Keep `CompositorLoop_sample_and_cancel_scroll_presentation_returns_latest_presented_value` and boundary coordinator tests green; add a focused low-FPS/queued-tick regression if this class of flicker returns. |
| 2 | Main-app lifecycle matrix | Extend only narrow coverage around concrete bugs in resize, DPI, max-scroll feedback, render invalidation, queued compositor ticks, and active hit-test routing. Acceptance is diagnostic-visible cancellation reason, stable presented-origin continuity, and no generic fallback compositor. |
| 3 | Scroll/input ownership contract | Before moving `ScrollController`, `ScrollState`, `ScrollFramePump`, input routing, or control projection out of `Irix.Poc`, write the framework owner boundary for logical state, feedback, compositor sampling, hit-test service access, and app message dispatch. |
