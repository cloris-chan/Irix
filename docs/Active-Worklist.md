...
- The scroll/input runtime owner boundary is written in `Poc-Promotion-Contracts.md`: logical scroll and input ownership belong to app/control runtime; layout feedback, compositor sampling, hit-test service access, input action mapping, and app message dispatch each require explicit adapter contracts before code moves. The first Poc-owned input action mapper cut exists for resolved `ActionId` activation.
...
| 3 | Scroll/input ownership contract | Boundary is written in [Poc-Promotion-Contracts.md](Poc-Promotion-Contracts.md). The first Poc-owned scroll presentation adapter cut exists for runtime dispatch, compositor sampling/start, and retained-snapshot access, and `CounterInputRouter` now consumes a Poc-owned input action mapper for resolved action activation; next code moves must continue introducing explicit feedback-sink, hit-test-service, and app-message-dispatch contracts rather than moving Counter/Poc runtime types wholesale. |
...
| Input/control projection | `InputOwnershipState`, `ControlVisualState*`, and `ActionHitTestResolver` remain Counter/Poc runtime projection. `CounterInputRouter` now has a Poc-owned input action mapper boundary for resolved `ActionId` activation. The owner boundary still requires a renderer-neutral hit-test service and a broader app-message dispatch mapper before extraction. | Do not promote input/control code without replacing Counter-specific action identity and single-pointer assumptions. |
