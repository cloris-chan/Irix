...
| 5 | Style transition app integration | Public style/transition authoring and Poc runtime ownership preflights are written. Next slice should wire a concrete app/control semantic transition decision through the coordinator, or defer actual public UI API until app integration proves the ownership shape; keep renderer processing categories, theme/cascade, and scheduler concerns out of public authoring. |
...
| Task | Current state | Acceptance |
|------|---------------|------------|
| Style system | [Style-System.md](Style-System.md) defines internal layout, visual, text shaping, composition, and control-state classification. Compact typed internal visual color properties now feed draw-command color payloads, composite metadata has its own mask, and pure opacity/translation deltas can precompile to existing transform/opacity declarations. Public authoring preflight now defines semantic future terms and forbidden internal processing names; the Poc runtime coordinator proves the first internal semantic decision boundary. | Keep public UI style semantic; implement against internal metadata and invalidation rules without adding generic compatibility style layers, public processing-layer authoring names, theme/cascade, or a scheduler before app integration is explicit. |
