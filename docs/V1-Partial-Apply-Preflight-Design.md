...
|------|--------------------|-------------------------|----------------------------|-------------------------------|----------------|
| Resource resolver ownership | Segmented reader and per-segment adapter prove old/new resolver ownership by command segment. | `SegmentedRetainedFrameDiagnosticHarness` opt-in feeds real `RenderPipeline.Build` batches into shadow owner and exposes segmented reads. | Production retained frame still exposes a single resolver boundary. | Backend contract and D3D12 execution stay unchanged. | Runtime retained frame must expose segmented resolver reads before hookup. |
| Resource dispose policy | Segment table covers ownership lifecycle rules. | Shadow owner and diagnostic harness cover partial accept, explicit full fallback, disabled mode, invalidate, dispose, replaced old ranges, and malformed reads. | Production retained frame does not own multiple retained snapshots. | Existing retained frame resource ownership tests keep same-frame behavior sealed. | Production retained frame must release retained snapshots exactly once across every path. |
...
The opt-in segmented retained-frame diagnostic harness can also consume real `RenderPipeline.Build` output plus an explicit retained root. This remains a test/diagnostic harness: it is not called by production `RenderPipeline.Build`, is not owned by `DrawingBackendCompositor`, does not replace the current `RetainedRenderFrame` read path, and its local shadow result vocabulary is not sent to CLI diagnostics formatters.
