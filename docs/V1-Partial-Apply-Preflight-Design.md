...
|------|--------------------|-------------------------|---------------------------------|----------------------------|-------------------------------|----------------|
| Resource resolver ownership | Segmented reader and per-segment adapter prove old/new resolver ownership by command segment. | Diagnostic harness and runtime owner seam feed real batches into shadow owner; compositor shadow probe verifies segmented resolver ownership outside compositor. | `RetainedRenderFrameSegmentOwnership` can opt in secondary segmented reads plus owner-side hit target metadata beside a real `RetainedRenderFrame` while `TryReadFrame` remains unchanged. | Production retained frame still exposes a single resolver boundary. | Backend contract and D3D12 execution stay unchanged. | Runtime retained frame must expose segmented resolver reads before hookup. |
| Resource dispose policy | Segment table covers ownership lifecycle rules. | Shadow owner, runtime owner seam, and diagnostic harness cover partial accept, explicit full fallback, rebuild, disabled mode, invalidate, dispose, replaced old ranges, and malformed reads. | Retained-frame segment ownership covers disabled no-owner mode plus enabled full apply, accepted partial, fallback full, rebuild, empty batch, malformed dirty ranges, hit target projection failure, disposal, and `TryReadFrame` survival. | Production retained frame does not own multiple retained snapshots. | Existing retained frame resource ownership tests keep same-frame behavior sealed. | Production retained frame must release retained snapshots exactly once across every path. |
...
| Regression coverage | Preflight tests cover planner, projector, root patch, segment table, and segmented reader scaffolds. | Shadow owner, runtime owner seam, compositor shadow probe, diagnostic harness, result vocabulary, disabled no-change sentinel, opt-in end-to-end, fallback, rejection, and adapter tests cover the current shadow path. | Production-off tests cover retained-frame segment ownership default-off equivalence, enabled-secondary equivalence, partial rehearsal, owner-side hit target projection, projection failure fallback, four-piece atomicity, fallback, rebuild, empty batch, malformed dirty ranges, dispose, empty segments, throw path, clipped hit-test, dirty-range isolation, and diagnostics text no-change. | No partial apply runtime hookup exists to cover. | Focused and full test suites act as the no-change regression guard. | Planner, retained frame, compositor, diagnostics, and hit-test coverage must remain green. |
