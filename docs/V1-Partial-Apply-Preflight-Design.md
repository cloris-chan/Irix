...
| `RetainedRenderFrameHandoffHarness` | Internal / tests only | Default-off candidate render-source harness; enabled mode executes `RuntimeOwner.ReadSegments()` through the per-segment adapter, uses owner-side hit targets, segment-local dirty ranges, and internal handoff counters without mutating production compositor state. |
...
|------|--------------------|-------------------------|---------------------------------|----------------------------|-------------------------------|----------------|
| Resource resolver ownership | Segmented reader and per-segment adapter prove old/new resolver ownership by command segment. | Diagnostic harness and runtime owner seam feed real batches into shadow owner; compositor shadow probe verifies segmented resolver ownership outside compositor. | `RetainedRenderFrameSegmentOwnership` plus `RetainedRenderFrameHandoffHarness` can execute secondary segmented reads as a candidate render source while `TryReadFrame` remains unchanged. | Production retained frame still exposes a single resolver boundary. | Backend contract and D3D12 execution stay unchanged. | Runtime retained frame must expose segmented resolver reads before hookup. |
