...
| Runtime hookup | Internal/default-off selected render-source path is V1 core complete; public/default-on hookup remains postponed. Runtime checkpoint: [V1-Partial-Apply-Runtime-Integration-Checkpoint.md](V1-Partial-Apply-Runtime-Integration-Checkpoint.md). |
...
| `DrawingBackendCompositor` handoff selector | Internal / tests only | Default-off constructor overload on the main compositor; disabled mode does not allocate a handoff harness, enabled mode keeps the retained frame updated for rollback and selects segmented reads as the backend render source only for a fresh accepted partial owner, while stale/missing/rejected/fallback/empty-read cases stay on the retained-frame path and report `Disabled` / `MissingOwner` / `Executed` / `FallbackFull` / `Rejected` internally. |
...
This scaffold is still default-off and internal. It does not change `RetainedRenderFrame.TryReadFrame`, `IDrawingBackend.Execute`, or D3D12, and the main compositor still uses the existing retained-frame render source unless the internal selector is explicitly constructed for tests and same-frame ownership freshness passes.
...
Current shadow owner proves the first runtime checkpoint target without replacing the existing retained frame: command buffer + resource segment table + retained root metadata can be owned together, full-applied, partially rehearsed, fallback-reported, invalidated, and disposed while retaining/releasing multiple snapshots exactly once. Current retained-frame segment ownership extends this proof beside a real `RetainedRenderFrame` with multiple `FrameDrawingResources` snapshots, failed partial owner-state preservation, repeated replacement, invalidate, dispose, selected render-source evidence, and retained-frame no-change checks.
...
Every V1 core gate below is now satisfied for the internal/default-off selected segmented render-source path. Evidence remains split so future public/default-on promotion cannot confuse preflight proof with production runtime proof.

| Gate | V1 core status | Production runtime evidence |
|------|----------------|-----------------------------|
| Resource resolver ownership | Satisfied | Fresh accepted `RuntimeOwner.ReadSegments()` can execute as the selected source through the real compositor backend; disabled/missing/stale/rejected/fallback paths still use `RetainedRenderFrame.TryReadFrame`. |
| Resource dispose policy | Satisfied | Selected path keeps snapshots owned by `SegmentedRetainedFrameRuntimeOwner`; accepted partial, reported fallback without owner mutation, empty frame, repeated replacement, backend throw, and dispose retain/release exactly once. |
| Command range stability | Satisfied | `DrawingBackendCompositor` validates owner freshness, batch resources, command owner, command count, `FrameId`, strict dirty ranges, and contiguous segment coverage before selected execution. |
| Hit target metadata projection | Satisfied | `TryGetActionIdAt` uses owner-side hit targets only after selected segmented execution succeeds; fallback/projection failure/stale/rejected/backend throw paths keep retained-frame hit targets. |
| Retained root update | Satisfied | Accepted partials atomically advance command segments, resource snapshots, retained root metadata, and hit target metadata; failed partials report fallback while preserving the previous owner state. |
| Fallback reporting | Satisfied | `LastHandoffResult` reports `Disabled`, `MissingOwner`, `Executed`, `FallbackFull`, and `Rejected` plus internal reasons for stale owner, malformed segment coverage, dirty range mismatch, and backend throw before commit. |
| Compositor ownership | Satisfied | The compositor owns selected-source choice behind an internal option, routes segment-local dirty ranges, commits owner hit targets only after success, and leaves default behavior equivalent. |
| Regression coverage | Satisfied | Tests cover default-off equivalence, enabled selected execution, fallback/stale/missing/rejected owners, malformed guards, backend throw, counters, dirty-range routing, hit-test ownership, diagnostics unchanged, and style-only pre-switch. |

The internal `PartialApplyIntegrationGateChecklist` mirrors this table as a regression guard. All required gates are `Satisfied=true`, so `CanHookUpPartialApply` is true for V1 core. This still does not enable partial apply by default and does not change public API, D3D12, `IDrawingBackend.Execute`, or CLI diagnostics.

Each checklist item carries separate `PreflightEvidence`, `ShadowRuntimeEvidence`, `ProductionOffRuntimeEvidence`, `ProductionRuntimeEvidence`, `NoChangeRegressionEvidence`, `BlockingCondition`, and `RuntimePromotionCondition` fields. Future public/default-on promotion still requires a separate decision; preflight or shadow-only evidence must not be used to expand the runtime surface.
...
The opt-in segmented retained-frame diagnostic harness can also consume real `RenderPipeline.Build` output plus an explicit retained root. This remains a test/diagnostic harness: it is not called by production `RenderPipeline.Build`, does not replace the current `RetainedRenderFrame` read path, and its local shadow result vocabulary is not sent to CLI diagnostics formatters. The retained-frame segment ownership seam is stronger than the feed because it sits beside a real `RetainedRenderFrame` and preserves `TryReadFrame` while maintaining secondary segmented state. The production-off owner feed now delegates to that seam: disabled mode allocates no seam, enabled mode follows batches into `SegmentedRetainedFrameRuntimeOwner`, and the compositor can select those reads only behind the internal default-off selector. Accepted partials advance commands, resource snapshots, retained root metadata, and owner-side hit target metadata together; projection, malformed range, or partial accept failure reports fallback while preserving owner state. Multi-snapshot lifecycle, owner-side clipped/no-hit lookup, handoff counter semantics, missing-owner no-op behavior, same-frame freshness, segment-local dirty-range rules, dirty-range mismatch routing, selected render-source backend execution, and main-compositor internal result reporting are now V1 core evidence. The compositor shadow probe and handoff harness still run beside the compositor; the main compositor selector is internal/default-off and selected-source evidence is limited to guarded fresh accepted frames.
