...
| `SegmentedRetainedFrameRuntimeOwner` | Internal / runtime seam draft | Long-lived owner seam that can hold the segmented owner, accept partial rehearsal, explicit fallback, rebuild, read segments, and dispose without replacing `RetainedRenderFrame`. |
| `SegmentedRetainedFrameProductionOwnerFeed` | Internal / production-off tests only | Default-off wrapper around normal `RenderPipeline.Build`; disabled mode allocates no runtime owner, enabled mode feeds `SegmentedRetainedFrameRuntimeOwner` as secondary state and never makes it the render source. |
...
| Regression coverage | Preflight tests cover planner, projector, root patch, segment table, and segmented reader scaffolds. | Shadow owner, runtime owner seam, compositor shadow probe, diagnostic harness, result vocabulary, disabled no-change sentinel, opt-in end-to-end, fallback, rejection, and adapter tests cover the current shadow path. | Production-off tests cover default-off equivalence, enabled-secondary equivalence, partial rehearsal, fallback, rebuild, empty batch, malformed dirty ranges, empty segments, throw path, clipped hit-test, dirty-range isolation, and diagnostics text no-change. | No partial apply runtime hookup exists to cover. | Focused and full test suites act as the no-change regression guard. | Planner, retained frame, compositor, diagnostics, and hit-test coverage must remain green. |
