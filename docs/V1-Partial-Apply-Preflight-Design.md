...
| `SegmentedRetainedFrameProductionOwnerFeed` | Internal / production-off tests only | Default-off wrapper around normal `RenderPipeline.Build`; disabled mode allocates no runtime owner, enabled mode feeds `SegmentedRetainedFrameRuntimeOwner` as secondary command/resource/root/hit-target state and never makes it the render source. |
...
|------|--------------------|-------------------------|---------------------------------|----------------------------|-------------------------------|----------------|
| Resource resolver ownership | Segmented reader and per-segment adapter prove old/new resolver ownership by command segment. | Diagnostic harness and runtime owner seam feed real batches into shadow owner; compositor shadow probe verifies segmented resolver ownership outside compositor. | Default-off production owner feed can opt in `SegmentedRetainedFrameRuntimeOwner` after normal `Build` and expose segmented reads plus owner-side hit target metadata while render output still comes from the existing retained frame. | Production retained frame still exposes a single resolver boundary. | Backend contract and D3D12 execution stay unchanged. | Runtime retained frame must expose segmented resolver reads before hookup. |
| Resource dispose policy | Segment table covers ownership lifecycle rules. | Shadow owner, runtime owner seam, and diagnostic harness cover partial accept, explicit full fallback, rebuild, disabled mode, invalidate, dispose, replaced old ranges, and malformed reads. | Production-off feed covers disabled no-owner mode plus enabled full apply, accepted partial, fallback full, rebuild, empty batch, malformed dirty ranges, hit target projection failure, and runtime-owner disposal. | Production retained frame does not own multiple retained snapshots. | Existing retained frame resource ownership tests keep same-frame behavior sealed. | Production retained frame must release retained snapshots exactly once across every path. |
| Command range stability | Planner, segment table, and segmented reader cover unstable, invalid, overlapping, and non-contiguous ranges. | Shadow owner reports `ShadowRejected` without command, segment, or retained-root mutation when an applied plan cannot be accepted. | Production-off feed runs planner to hit target projection to root patch to owner accept, confirms projection and malformed dirty range failures preserve owner state before fallback, then explicitly full-applies fallback. | Production runtime still does not replace cross-frame command ranges. | Compositor no-change tests keep current full/guarded partial behavior sealed. | Runtime command replacement must require stable contiguous dirty command ranges. |
| Hit target metadata projection | Projector covers action metadata projection, dirty DFS mismatch, non-dirty drift, key/path mismatch, and nested controls. | Shadow harness consumes projector output while hit-test runtime remains unchanged. | Production-off feed stores secondary hit targets, uses the projector for accepted partial rehearsal, falls back on projection failure, and does not alter production hit-test runtime; compositor probe hardening covers clipped target no-change. | Hit-test runtime still consumes full layout output only. | Hit-test behavior tests keep retained geometry and compositor lookup unchanged. | Runtime projector must reproject action metadata without next layout output. |
...
| Compositor ownership | Dry-run and adapter tests keep segment ownership/execution outside compositor mutation. | Compositor shadow probe executes shadow segmented reads outside compositor and verifies execute order, resolver ownership, and hit-test no-change. | Production-off no-change tests cover compositor counters, backend execute calls, dirty range propagation, hit-test results, diagnostics text, and enabled rendering through the normal full-frame batch. | `DrawingBackendCompositor` owns one retained frame with one resolver boundary. | Compositor no-mutation tests keep counters, retained frame, and backend execution unchanged. | Compositor ownership of multiple retained snapshots must be explicit before hookup. |
| Regression coverage | Preflight tests cover planner, projector, root patch, segment table, and segmented reader scaffolds. | Shadow owner, runtime owner seam, compositor shadow probe, diagnostic harness, result vocabulary, disabled no-change sentinel, opt-in end-to-end, fallback, rejection, and adapter tests cover the current shadow path. | Production-off tests cover default-off equivalence, enabled-secondary equivalence, partial rehearsal, owner-side hit target projection, projection failure fallback, four-piece atomicity, fallback, rebuild, empty batch, malformed dirty ranges, empty segments, throw path, clipped hit-test, dirty-range isolation, and diagnostics text no-change. | No partial apply runtime hookup exists to cover. | Focused and full test suites act as the no-change regression guard. | Planner, retained frame, compositor, diagnostics, and hit-test coverage must remain green. |
...
The opt-in segmented retained-frame diagnostic harness can also consume real `RenderPipeline.Build` output plus an explicit retained root. This remains a test/diagnostic harness: it is not called by production `RenderPipeline.Build`, is not owned by `DrawingBackendCompositor`, does not replace the current `RetainedRenderFrame` read path, and its local shadow result vocabulary is not sent to CLI diagnostics formatters. The production-off owner feed is a stronger wrapper around normal `Build`, but it is still secondary state only: disabled mode allocates no owner, enabled mode follows batches into `SegmentedRetainedFrameRuntimeOwner`, and rendering still comes from the existing production retained frame. Accepted production-off partial rehearsal advances commands, resource snapshots, retained root metadata, and owner-side hit target metadata together; projection or dirty-range failure preserves owner state before explicit fallback. The compositor shadow probe intentionally runs beside the compositor, not inside it.
