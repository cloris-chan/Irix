...
| `DrawingBackendCompositorHandoffSeam` | Internal / tests only | Compositor-adjacent seam; disabled mode does not allocate a candidate harness, enabled mode renders the existing compositor path then invokes the handoff harness as a candidate source and reports `Disabled` / `MissingOwner` / `Executed` / `FallbackFull` / `Rejected` internally. |
...
|------|--------------------|-------------------------|---------------------------------|----------------------------|-------------------------------|----------------|
| Resource resolver ownership | Segmented reader and per-segment adapter prove old/new resolver ownership by command segment. | Diagnostic harness and runtime owner seam feed real batches into shadow owner; compositor shadow probe verifies segmented resolver ownership outside compositor. | `RetainedRenderFrameSegmentOwnership` plus `DrawingBackendCompositorHandoffSeam` can execute secondary segmented reads as a production-adjacent candidate render source while `TryReadFrame` remains unchanged. | Production retained frame still exposes a single resolver boundary. | Backend contract and D3D12 execution stay unchanged. | Runtime retained frame must expose segmented resolver reads before hookup. |
