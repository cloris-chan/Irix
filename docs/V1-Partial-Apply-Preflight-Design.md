...
The opt-in segmented retained-frame diagnostic harness can also consume real `RenderPipeline.Build` output plus an explicit retained root. This remains a test/diagnostic harness: it is not called by production `RenderPipeline.Build`, does not replace the current `RetainedRenderFrame` read path, and its local shadow result vocabulary is not sent to CLI diagnostics formatters. The retained-frame segment ownership seam is stronger than the feed because it sits beside a real `RetainedRenderFrame` and preserves `TryReadFrame` while maintaining secondary segmented state. The production-off owner feed now delegates to that seam: disabled mode allocates no seam, enabled mode follows batches into `SegmentedRetainedFrameRuntimeOwner`, and the compositor can select those reads only behind the internal default-off selector. Accepted production-off partial rehearsal advances commands, resource snapshots, retained root metadata, and owner-side hit target metadata together; projection or dirty-range failure preserves owner state before explicit fallback. Multi-snapshot lifecycle, owner-side clipped/no-hit lookup, handoff counter semantics, missing-owner no-op behavior, same-frame freshness, segment-local dirty-range rules, dirty-range mismatch routing, selected render-source backend execution, and main-compositor internal result reporting are now rehearsed as internal/default-off evidence. The compositor shadow probe and handoff harness still run beside the compositor; the main compositor selector is internal, default-off, and selected-source evidence is limited to fresh accepted frames.
