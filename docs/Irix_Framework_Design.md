...
│      FlatBuffers VirtualNode Patch ◄──► F# Server Core       │
└──────────────────────────────────────────────────────────────┘
```
...
> **命名统一说明：** PoC 已统一采用 C# 风格命名，例如 `VirtualNode`、`VirtualNodePatch`、`Message`、`Command`、`IMessageDispatcher`，避免继续沿用 `VNode`、`Msg` 这类缩写式标识。
...
CompositorThread.Start()
  ...
  └─ 进入渲染循环:
       while (!cancellation.IsCancellationRequested)
         ...
         ├─ 消费 MPSC 队列中的 VirtualNode Patch
...
internal sealed class VulkanCompositor : ICompositor
{
    ...
    public void RenderFrame(ReadOnlySpan<VirtualNodePatch> patches)
    {
...
| R-03 | `VirtualNode` 全量 `ref struct` 化导致实现与调试复杂度过高 | 🟠 中高 | v1 改为"热路径零分配优先"，不把整棵声明树栈上化作为硬要求 |
...
- [ ] 实现 `VirtualNodePatch` 输出与单消费者渲染链路
