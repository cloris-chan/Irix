...
- **Core / Patch 管线不共享可变状态**：MVU Core 与 Compositor 之间的数据通道是 MPSC Channel 的 `IMemoryOwner<T>` 转移，Core 侧不再持有所有权。
- **后端私有同步只允许用于 OS 回调交界处**：例如 Win32 `WM_SIZE` 在 Platform Thread 上记录 D3D12 pending resize，Compositor Thread 在布局前应用；这类状态必须是 backend-private、可合并、不可泄漏到 MVU 数据模型。
...
> **当前实现状态：** 代码中的 `HitTestTarget` 仍是最小过渡形态（`Bounds + ActionId`），已经与 `DrawCommand` 并行传递；`NodeId`、`HitTestBehavior` 与正式 route result 尚未落地。
...
> **当前实现状态：** Windows platform 已能发布 `FocusGained` / `FocusLost` 原始事件，但 `IFocusManager` 及其与 MVU `Model` 的集成尚未落地。
...
public readonly struct VirtualNode
{
    ...
    public ReadOnlySpan<VirtualNodeProperty> Properties { get; }
    public ReadOnlySpan<VirtualNode> Children { get; }
}
...
| Type | Scope | Rule |
|------|-------|------|
| `VirtualNodePropertyListBuilder` | Synchronous authoring/build-view helper | Wraps caller-provided `Span<VirtualNodeProperty>`; small property lists can use `stackalloc`; callers hand off `Written` to the span factory. |
| `VirtualNodeChildrenBuilder` | Synchronous authoring/build-view helper | Keeps an `InlineArray(4)` child buffer and falls back to an array only beyond inline capacity; final node construction freezes children once. |
...
| `VirtualNodeProperty` | node-local override 或 runtime visual state |
| Future stylesheet | 另行设计；不把 preset metrics 塞回 node property |
...
- 递归深比较：逐节点比较 `Kind`、`Key`、`Content`、`Properties`、`Children`，短路于首个差异。
...
> **当前实现状态：** D3D12 resize 路径已落地为 pending resize + 合并式 repaint 请求：`WM_SIZE` 只记录最新尺寸，Compositor 在布局前应用 swapchain resize，并用已应用尺寸驱动 layout。D3D12 fence event 由 renderer 持有 SafeHandle，并使用 auto-reset event，避免 GC 回收底层 handle 或 manual-reset event 长期 signaled 后绕过真实 fence wait。交互运行默认不再串联 `ConsoleCompositor` trace，并将 swapchain scaling 设为非拉伸；D3D12 HWND 启用 external rendering 模式，让 `WM_PAINT` 只 validate 而不执行 GDI 背景擦除，降低快速拖动时旧 backbuffer 拉伸与 GDI paint 竞争造成的视觉抖动。GPU device-lost 检测已具备 fail-fast 标志、诊断字符串与 `SucceededOrMarkDeviceRemoved` 显式 HRESULT 检查；设备丢失后仅标记 `_deviceRemoved` 并跳过后续渲染，不自动重建设备/Swapchain，也不上报连续失败计数。设备丢失自动恢复（重建设备、交换链与所有 GPU 资源）仍为待办。
