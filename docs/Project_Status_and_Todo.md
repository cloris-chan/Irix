...
| Style / animation / composition | D3D12 composition spine now has runtime `NodeKey` animation declarations, retained `CompositionTarget` resolution from normal UI output, resolved `CompositionAnimationPlan` ticks over the retained frame, inverse-transform hit-test remapping for active transform layers, typed composition clock values, formal D3D12 composition backend capability, diagnostics, and a visible PoC demo on the retained UI path. Remaining work is scroll presentation. Fallback compatibility is only for explicit blockers. |
...
| P1 | D3D12-first scroll presentation | Design and implement presented scroll offset as compositor transform under a clip, including scroll-specific target/commit and input remapping rules. Use fallback only when a written blocker prevents the GPU path. |
