...
| Style / animation / composition | Design-only phase is active, but implementation should be GPU-first once a narrow contract is accepted. First composition code should target a D3D12-backed layer path; fallback compatibility is only for explicit blockers. |
...
| P1 | D3D12-first composition spike | First implementation should validate layer identity, immutable composition IR handoff, transform/opacity updates, and diagnostics on the active D3D12 backend. Use the existing renderer as fallback only when a written blocker prevents the GPU path. |
