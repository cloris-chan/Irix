...
| P1 | Direct composition hardening | Keep transform/opacity, fixed-clip scroll, nested/mixed-clip layers, hit testing, marker dispatch, layer content cache, and skip diagnostics on the direct D3D12 path. Layer-cache paint order and marker skip/cancellation queue cleanup now have narrow coverage; add more regression coverage only when new concrete behavior boundaries appear. |
