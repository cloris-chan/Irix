...
| [D3D12-Composition-Spike-v0.md](D3D12-Composition-Spike-v0.md) | Active D3D12 composition implementation gate for transform/opacity ticks over retained draw output. |
...
| P1 | Broaden render-target-backed layer reuse | D3D12 now caches transform-with-content rect/GlyphAtlas text command-order layers into RTV/SRV textures and composites them back with textured quads without replaying stable payloads. Next expand to content-space fixed-clip, image, and vector-capable layer contents. |
| P1 | Scroll presentation fallback diagnostics | Main app wheel input starts compositor scroll presentation ticks, and active presentation is cancelled before viewport/tree/layout/text/max-scroll-changing retained frames render. Next make unsupported or skipped producer paths diagnostic-visible before broader app integration. |
