...
> 📅 **最后验证日期：** 2026-05-16。本文档描述的代码状态以此日期为准。
>
...
| Text/value IR | ✅ 完成：`VirtualNode → LayoutElement → DrawCommandRecorder` 使用 `TextNodeContent + TextBufferSnapshot.ResolveRequired`；text arena 主线冻结 | `TextContentTypes.cs`、`VirtualNodeModels.cs`、`DrawCommandRecorder.cs` |
| Typed style/property metadata | ✅ 完成：Round 14 unified typed property API、metadata effects、animation channel、supported-node spec、source guards 已覆盖 | 本文 `Round 14`、`TypedIdAllocationGuardTests` |
...
- **已验证（2026-05-13）：** D3D12 text cache safety — cache keys 使用 `TextStyle` value equality（非 `ResourceHandle`），跨 resolver 安全。Default-on go/no-go checklist — **GO**：Gate 1-5 全部满足，Counter PoC D3D12 smoke test 通过（多刷新率，无渲染错误，无 crash）。HiDPI / PerMonitorV2 manifest / display scale pipeline 已对齐，Target SDK 为 Windows SDK 26100，runtime minimum 为 Windows 10 1703 / 10.0.15063.0。详见 [Default-On-Partial-Apply-Prep.md](Default-On-Partial-Apply-Prep.md)。
...
- **手测通过（2026-05-13）：** Display scale pipeline — 100% / 150% / 200% DPI 下文字、按钮、hit-test、scroll、resize、partial apply 均正常；运行时改系统缩放后下一幀 relayout 正确，文字/矩形/hit-test 不错位。39 DisplayScale regression tests 覆盖 command rect、clip bounds、hit target、text font size、logical viewport（1.0/1.25/1.5/2.0）。所有 506 tests 通过。
...
**Round 14 = Unified typed style/property metadata + invalidation cleanup**
...
| P1 | Typed id / property API shaping | Typed IDs 与 property metadata 已在 core 完成；后续只做 framework promotion / public API naming | API control boundary prep |
...
- `Irix.Platform.Windows`
  ...
  - Windows-targeted projects 的版本边界集中在 `Directory.Build.props`：`IrixWindowsTargetFramework=net10.0-windows10.0.26100.0`（TargetFramework / target SDK 26100），`IrixWindowsSupportedOSPlatformVersion=10.0.15063.0`（runtime minimum）。15063 是 PerMonitorV2 DPI awareness 与 display scale pipeline 的最低边界；不再声明低于该 runtime floor 的 Windows API 边界。
