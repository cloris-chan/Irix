...
| Glyph shaping | DirectWrite shaped-run probe is wired into the glyph atlas renderer through `IDWriteTextAnalyzer` and pointer-based `GetGlyphs` / `GetGlyphPlacements` calls over resolved spans. It projects DirectWrite glyph index/advance/offset/property output into renderer-owned `ShapedGlyph` scratch and a synchronous `ShapedGlyphRun` span view, then records `shapedProbeRuns` / `shapedProbeGlyphs` for `NonAscii` degraded runs only. Shaped runs are still not drawn by the atlas. |
