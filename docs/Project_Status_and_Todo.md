...
| P1 | Style transition scheduler or multi-owner design | Internal semantic style declarations now feed the existing renderer IR and Poc control-state/transition adapters. Public authoring, Poc-owned runtime ownership, first Counter app/control transition integration, and lifecycle preflight are documented and guard-covered. The next useful style slice is either a narrow transform/opacity completion scheduler or a multi-owner design for concurrent control-state deltas; still no theme/cascade, public transition API, or public processing-layer authoring names. |
