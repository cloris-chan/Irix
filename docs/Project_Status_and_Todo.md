...
| Glyph shaping | DirectWrite shaped-run handling is wired into the glyph atlas renderer through `IDWriteTextAnalyzer`, system `IDWriteFontFallback`, and pointer-based shaping calls over resolved spans. It now runs `AnalyzeScript` / `AnalyzeBidi` into renderer-owned scratch before glyph placement, projects DirectWrite glyph index/advance/offset/property output into renderer-owned `ShapedGlyph`, segment, explicit-line, and per-character advance scratch plus a synchronous `ShapedGlyphRun` span view. Shaped runs with nonzero glyph indices can rasterize/draw through the D3D12 atlas, including explicit CR/LF line breaks, tab control advances, minimal whitespace wrapping, unbreakable wrap words clipped by text scissor, mixed ASCII/CJK runs segmented through DirectWrite fallback font mapping, DirectWrite color-glyph layers for surrogate/variation-selector emoji samples, LTR complex-script runs, single-level RTL `NoWrap` segments, single-level RTL wrapped lines whose glyph segments are all RTL, and mixed BiDi spans split by resolved bidi level with line segment visual ordering. Color bitmap/SVG glyph formats beyond `IDWriteFactory2.TranslateColorGlyphRun` and deeper BiDi shaping cases remain future work. |
