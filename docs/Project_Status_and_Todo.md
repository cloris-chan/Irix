...
| Glyph shaping | DirectWrite shaped-run handling is wired into the glyph atlas renderer through `IDWriteTextAnalyzer`, system `IDWriteFontFallback`, and pointer-based shaping calls over resolved spans. It projects DirectWrite glyph index/advance/offset/property output into renderer-owned `ShapedGlyph` scratch plus segment scratch and a synchronous `ShapedGlyphRun` span view. `NoWrap` shaped runs with nonzero glyph indices can rasterize/draw through the D3D12 atlas, including mixed ASCII/CJK runs segmented through DirectWrite fallback font mapping. Shaped wrapping still degrades explicitly. |
