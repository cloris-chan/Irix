...
| P1 | Style transition lifecycle design | Public authoring, Poc-owned runtime ownership, and first Counter app/control transition integration are now documented and guard-covered. The next useful style slice is a narrow lifecycle/scheduler or multi-owner design for completion/commit, active scroll coexistence, and multi-target control-state deltas; still no theme/cascade or public processing-layer authoring names. |
