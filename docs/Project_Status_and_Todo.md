...
| Retained / partial apply prep | ✅ preflight scaffold 完成：每个 local reason 有 regression test；segmented reader edge guards、resource segment / lifecycle model、hit target projector、retained root metadata patcher、pipeline dry-run、layered integration gate evidence 均为 internal/test-only；cross-frame resources 仍 blocked |
...
- 已记录：[Retained / partial apply prep](V1-Retained-Partial-Apply-Prep.md) 与 [Partial apply preflight design](V1-Partial-Apply-Preflight-Design.md)：retained-input snapshot seam、data-only local planner、planner-only reason coverage、segmented reader edge guards、resource segment / lifecycle model、hit target metadata projector、retained root metadata patcher、pipeline dry-run、layered integration gate checklist 已落地/记录；下一条安全线仍是 internal scaffold，不做 full partial apply。
...
| IRIX-V1-013 | P2 | Retained planner-only hardening / partial apply preflight | Internal scaffold 完成 | [V1-Retained-Partial-Apply-Prep.md](V1-Retained-Partial-Apply-Prep.md) 与 [V1-Partial-Apply-Preflight-Design.md](V1-Partial-Apply-Preflight-Design.md) 已记录 reason coverage、segmented reader edge guards、resource segment / lifecycle model、hit target metadata projector、retained root metadata patcher、pipeline dry-run、layered gates；不启用 StyleOnly fast-path。 |
