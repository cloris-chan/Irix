...
2026-05-20 reusable upload resource fence update: Release build passed; all tests `646` passed; program diagnostics tests `58` passed. Short default GlyphAtlas sync smoke reported `frameSerial=30`, `presentSerial=30`, `syncWaits=2`, avg sync wait `0.009ms`, p95 `0.075ms`, `atlasRuns=90`, and no device removal. The sync wait now represents only a last-submitted-frame fence that had not completed before reusable upload buffers were mapped again; mixed degradation and MixedAtlasFull / MixedAtlasFullReuse stress remained `syncWaits=0`.
