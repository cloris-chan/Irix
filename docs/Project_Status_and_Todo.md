...
| Style / animation / composition | D3D12 composition spine now has runtime `NodeKey` animation declarations, retained `CompositionTarget` resolution from normal UI output, resolved `CompositionAnimationPlan` ticks over the retained frame, typed composition clock values, formal D3D12 composition backend capability, diagnostics, and a visible PoC demo on the retained UI path. Remaining work is hit-test remapping and scroll presentation. Fallback compatibility is only for explicit blockers. |
