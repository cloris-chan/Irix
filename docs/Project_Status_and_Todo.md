...
| P1 | Start live scroll presentation ticks | Wheel input can now retarget from an active compositor scroll presentation. Next, start compositor scroll presentation from the main app scroll path instead of relying on the CPU scroll frame path as the only producer. |
