...
| Style / animation / composition | D3D12 composition spike now has a rendering-layer animation descriptor, compositor-owned transform/opacity tick path over the retained frame, formal D3D12 composition backend capability, diagnostics, and a visible PoC demo. Remaining work is real UI-output layer identity, runtime animation declaration, hit-test remapping, and scroll presentation. Fallback compatibility is only for explicit blockers. |
...
| P1 | Runtime animation declaration | Connect compositor-eligible animation declarations from the UI/runtime boundary to `CompositionAnimationPlan`, keeping logical state in runtime and presentation state in the compositor. |
| P1 | Retained layer identity | Replace demo-owned command ranges with stable layer ids from normal retained UI output, then keep transform/opacity ticks independent of `VirtualNode`, layout, and draw-command rebuild. |
