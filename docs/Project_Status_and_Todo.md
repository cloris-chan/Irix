...
| Glyph shaping | DirectWrite shaped-run handling is wired into the glyph atlas renderer through `IDWriteTextAnalyzer` and pointer-based `GetGlyphs` / `GetGlyphPlacements` calls over resolved spans. It projects DirectWrite glyph index/advance/offset/property output into renderer-owned `ShapedGlyph` scratch and a synchronous `ShapedGlyphRun` span view. Single-face `NoWrap` shaped runs with nonzero glyph indices can now rasterize/draw through the D3D12 atlas; fallback-face/CJK and shaped wrapping still degrade explicitly. |
