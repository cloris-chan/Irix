...
| Glyph shaping | DirectWrite shaped-run handling is wired into the glyph atlas renderer through `IDWriteTextAnalyzer`, system `IDWriteFontFallback`, and pointer-based shaping calls over resolved spans. It projects DirectWrite glyph index/advance/offset/property output into renderer-owned `ShapedGlyph` scratch and a synchronous `ShapedGlyphRun` span view. Single-face `NoWrap` shaped runs with nonzero glyph indices can rasterize/draw through the D3D12 atlas, including pure CJK runs mapped to one system fallback font face. Mixed multi-face fallback segmentation and shaped wrapping still degrade explicitly. |
