...
- **已验证（2026-05-13）：** D3D12 text cache safety — cache keys 使用 `TextStyle` value equality（非 `ResourceHandle`），跨 resolver 安全。Default-on go/no-go checklist — Gate 1/3/5 已满足，Gate 2 架构验证通过，Gate 4 未开始（NO-GO）。详见 [Default-On-Partial-Apply-Prep.md](Default-On-Partial-Apply-Prep.md)。
...
- **下一步：** Default-on partial apply 仍需 Gate 4 最低验证（60Hz spot check）；Post-V1 backlog 已拆为 4 batch，GA hardening first batch 已规划 5 项实现步骤。详见 [Post-V1-MVP-Backlog.md](Post-V1-MVP-Backlog.md)、[GA-Hardening-Plan.md](GA-Hardening-Plan.md)。
