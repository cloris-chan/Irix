...
| Resource lifetime | D3D12 upload map paths unmap in `finally` after a successful map for rectangle vertices, glyph vertices, and atlas uploads. |
...
Mixed fallback v0 update: Release build passed; normal tests `615` passed; D3D12 tests `6` passed; performance tests `6` passed.
Program diagnostics tests `52` passed. Short default GlyphAtlas smoke reported `atlasRuns=90`, `overlayFallbackRuns=0`, `syncWaits=0`.
