...
| Architecture boundary | `Irix.Poc` is an app, diagnostics, and adapter-glue project. It is not the reusable framework home. `D3D12DrawingBackend` has moved to `Irix.Platform.Windows`; `WindowDrawCommandTranslator` needs `TranslatorInput` / `TranslatorOutput` / `ViewportProvider` / `FeedbackSink` boundaries before moving; `WindowBackend` stays as legacy/debug presentation. |
...
| P1 | Framework promotion review | Use [Poc-Promotion-Contracts.md](Poc-Promotion-Contracts.md) as the move contract. Next step is a no-behavior-change translator boundary split; scroll feedback is app/control feedback, not rendering diagnostics or platform feedback. |
