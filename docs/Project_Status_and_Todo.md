...
| P1 | Style transition app integration | Public authoring and Poc-owned runtime ownership preflights are now documented and guard-covered. The next useful style slice is concrete app/control integration for semantic transition decisions, or a later public UI API only after that integration proves ownership; still no theme/cascade/scheduler or public processing-layer authoring names. |
