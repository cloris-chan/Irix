...
| P1 | Render-target-backed layer reuse | D3D12 now caches disjoint composition layer content as backend payloads. Next GPU-side payoff is caching stable layer content into render targets instead of replaying backend payloads for every compositor tick. |
