...
| P1 | Main-app composition integration | Main app wheel input starts compositor scroll presentation ticks; hover/press/release can route through active presentation; chained retarget distance, compositor-thread sample-and-cancel continuity, `EnsureRunning` rapid wheel coalescing, top/bottom boundary wheel clamping, and resize/DPI/max-scroll lifecycle cancellation are diagnostic-covered. Continue hardening queued compositor ticks, cancellation, retarget, hit-test, interaction, and marker behavior without adding a generic fallback compositor. |
