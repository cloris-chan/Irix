...
| P1 | Runtime marker integration and scroll commit/cancel policy | Drain `CompositionAnimationMarkerEvent` values into the UI runtime and map `CompositionRuntimeEventId` to app messages. Define how input retargets, commits, or cancels presented scroll without moving logical scroll ownership into the backend. |
