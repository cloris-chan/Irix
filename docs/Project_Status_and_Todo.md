...
| P1 | Multi-layer composition / nested scroll | Extend the single-layer fixed-clip scroll presentation path to nested/mixed-clip scroll containers before broader runtime integration. Use fallback only when a written blocker prevents the GPU path. |
