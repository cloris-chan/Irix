...
- `VirtualNodeDiffer` 已实现局部 diff：递归深比较 + keyed reconciliation + Update/Add/Remove patches；`default` 树边界处理已完善；241 个测试用例覆盖各场景
...
- ✅ `VirtualNodeDiffer` 已从 `ReplaceRoot` 提升到局部 diff（Update/Add/Remove + keyed reconciliation），并纳入当前 241 个全量测试覆盖
...
- [x] `CompositorLoop` 合并式 render request 行为测试：连续请求只排队一次、渲染中后补一帧、普通 empty diff 不等同 render request；`RequestRenderAndWaitAsync` 与普通 diff `PublishAndWaitRenderAsync` 都在对应 `RenderAsync` 完成后才 complete（241 个测试，全部通过）
...
- [x] Scroll 回归测试：新增覆盖首笔 pending delta 不丢、Windows 向下单刻度 target=54、默认窗口下 Counter view 可滚动、layout max 回传后单刻度仍得到 target=54、快速 100 次 wheel 在 render wait 阻塞期间合并为少量 ScrollFrame、低频 render completion 不堆积旧帧、普通 diff patch render wait、`RequestRenderAndWaitAsync` 保留给无 diff repaint、第二帧 `dt` 包含 render wait、raw wheel 方向、`--diagnose-scroll` synthetic pump 输出；全量 241 个测试通过
...
当前 scroll pump、input ownership、Button visual state 三条交互基础链路已冻结：不继续修改 scroll coalescing / frame pump、ownership 事件规则、button state 派生与颜色优先级，除非有明确回归。短期只允许围绕 style preset、诊断与测试做不改变行为的整理。

### Counter debug UI gating

Counter PoC 默认 UI 隐藏 `ScrollY:` 与 `Input:` 调试文本，只保留计数、操作提示、控件与可滚动内容；`--debug-ui` 通过 `CounterApplication(showDiagnostics: true)` 恢复完整 UI 内诊断文本。CLI 诊断模式保持独立：`--diagnose-scroll` 与 `--diagnose-input` 继续输出完整报告，不依赖 UI 文本是否显示。
...
Style 分层已收敛为三个边界：`LayoutStyle` 只描述布局几何参数（padding、spacing、text/button/rectangle height、button width 估算）；`DrawingStyle` 只描述绘制 token（文本色、矩形色、button 各状态 fill color、文本样式）；`ControlVisualStateResolver` 只负责把 `ButtonVisualState` 按 `Pressed > Hovered > Focused > Normal` 映射到 `DrawingStyle` 的颜色 token。`RenderStylePreset.Default` 是 rendering 默认 preset，`CounterStylePreset.Default` 复用默认 layout/button token，仅覆盖 Counter PoC 的正文文本色。当前不引入 theme system、样式继承或通用控件抽象。

已覆盖测试：默认 preset 的 button height、padding、text height、button width 参数不变；默认 preset 的 normal / focused / hovered / pressed button fill color 不变；Counter PoC preset 保持默认 layout/button token 且只覆盖正文文本色。
