...
| P1 | Wire scroll presentation policy into live input | PoC `ScrollController` now defines commit/cancel/retarget decisions for presented scroll. Next, connect that policy to live compositor scroll presentation rather than only the CPU scroll frame path. |
