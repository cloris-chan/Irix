...
| [D3D12-Composition-Spike-v0.md](D3D12-Composition-Spike-v0.md) | Active D3D12 composition implementation gate for transform/opacity and fixed-clip scroll ticks over retained draw output. |
...
Private execution mode: milestone labels are planning snapshots, not compatibility or release constraints. Prefer direct target-architecture work, migrate internal APIs and tests when boundaries change, and keep secondary paths explicit and diagnostic-visible.
...
| Area | Status |
|------|--------|
| Branch state | Renderer foundation and post-foundation D3D12 composition work are on `master`; the active head has removed the premature viewport-space render-target cache and keeps direct/layer-content composition as the mainline. |
...
| P1 | Composition skip diagnostics | Main app wheel input starts compositor scroll presentation ticks, and active presentation is cancelled before viewport/tree/layout/text/max-scroll-changing retained frames render. Next make unsupported or skipped compositor producer paths diagnostic-visible before broader app integration; do not add a generic fallback compositor. |
| P1 | Direct composition hardening | Keep transform/opacity, fixed-clip scroll, nested/mixed-clip layers, hit testing, marker dispatch, and layer content cache on the direct D3D12 path. Add narrow regression coverage when bugs appear in those semantics. |
