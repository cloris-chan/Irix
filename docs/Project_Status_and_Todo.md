...
| Style / animation / composition | D3D12 composition spike is the active implementation gate. First code validates layer identity, translation, opacity, and diagnostics through the D3D12 backend path; fallback compatibility is only for explicit blockers. |
