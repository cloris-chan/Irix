...
| Style / animation / composition | D3D12 composition spine now has a rendering-layer animation descriptor, compositor-owned transform/opacity tick path over the retained frame, typed composition clock values, retained `NodeKey` composition targets from normal UI output, formal D3D12 composition backend capability, diagnostics, and a visible PoC demo. Remaining work is runtime animation declaration, hit-test remapping, and scroll presentation. Fallback compatibility is only for explicit blockers. |
...
| P1 | Runtime animation declaration | Connect compositor-eligible animation declarations from the UI/runtime boundary to retained `NodeKey` / `CompositionTarget` resolution and `CompositionAnimationPlan`, keeping logical state in runtime and presentation state in the compositor. |
| P1 | Hit-test remapping | Map pointer coordinates through active compositor transforms before dispatch, or define a commit/cancel policy for input during compositor-presented transforms. |
| P1 | D3D12-first scroll presentation | Design and implement presented scroll offset as compositor transform under a clip after hit-test mapping is contracted. Use fallback only when a written blocker prevents the GPU path. |
