...
| P1 | Style transition runtime ownership | Public style/transition authoring preflight is now documented and guard-covered. The next useful style slice is narrow runtime ownership for semantic transition start/cancel/retarget/commit, or a later actual public UI API only after that ownership surface is written; still no theme/cascade/scheduler or public processing-layer authoring names. |
