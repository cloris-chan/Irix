...
| Style / animation / composition | D3D12 composition spine now has runtime `NodeKey` transform declarations, fixed-clip scroll presentation declarations, retained `CompositionTarget` / `ScrollCompositionTarget` resolution from normal UI output, resolved compositor-only ticks over the retained frame, inverse-transform hit-test remapping for active transform and scroll presentation layers, typed composition clock values, declaration-level animation markers with compositor-produced runtime event ids, a runtime marker event pump that maps marker ids to app messages outside the backend, ordered multi-layer `CompositionFrame` execution on D3D12, formal D3D12 composition backend capabilities, diagnostics, and visible PoC/demo coverage on the retained UI path. Remaining work is retained target decomposition for nested/mixed-clip scroll and runtime commit/cancel policy. Fallback compatibility is only for explicit blockers. |
...
| P1 | Nested/mixed-clip scroll decomposition | Multi-layer D3D12 execution is implemented. Next, split retained scroll targets with nested or mixed clips into ordered composition layers before broader runtime integration. Use fallback only when a written blocker prevents the GPU path. |
| P1 | Scroll commit/cancel policy | Runtime marker dispatch first slice is implemented through `CompositionMarkerEventPump` and `--diagnose-composition-marker-runtime`. Define how input retargets, commits, or cancels presented scroll without moving logical scroll ownership into the backend. |
