...
| Style / animation / composition | D3D12 composition spine now has runtime `NodeKey` transform declarations, fixed-clip scroll presentation declarations, retained `CompositionTarget` / `ScrollCompositionTarget` resolution from normal UI output, resolved compositor-only ticks over the retained frame, inverse-transform hit-test remapping for active transform and scroll presentation layers, typed composition clock values, formal D3D12 composition backend capabilities, diagnostics, and a visible PoC demo on the retained UI path. Remaining work is multi-layer/nested clip composition and runtime commit/cancel policy. Fallback compatibility is only for explicit blockers. |
...
| P1 | Scroll runtime commit/cancel policy | Define how input retargets, commits, or cancels presented scroll without moving logical scroll ownership into the backend. |
