...
| P1 | Attribute warm glyph atlas allocation | Latest corrected `--diagnose-text-cache 30` scroll sample is `2793 bytes/frame`; translate sub-breakdown puts the `1631 bytes/frame` translate cost entirely in `RenderPipeline.Build`. Optimize from pipeline attribution next. |
