...
| Color pipeline | Design direction is fixed: future standard Irix `Color` represents an ideal linear BT.2020 / Rec.2020 straight-alpha value, with source color space forgotten after construction. The current implementation remains SDR/sRGB only: compact `StyleColor`, `DrawColor`, and `WindowColor` are active output-stage storage, not the long-term canonical color definition. HDR/scRGB/Rec.2100 HLG/PQ output, system SDR brightness, per-screen mapping, and tone mapping are deferred to a coordinated HDR backend/compositor implementation. |
...
| P1 | Color IR preflight | Introduce a canonical internal linear BT.2020 color value and sRGB conversion tests before changing backend output. Keep the active output path SDR/sRGB and leave HDR/scRGB/Rec.2100 support to the later HDR backend/compositor slice. |
