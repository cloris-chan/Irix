...
| P1 | Main-app composition integration | Main app wheel input starts compositor scroll presentation ticks, and active presentation is cancelled before viewport/tree/layout/text/max-scroll-changing retained frames render. Continue hardening cancellation, retarget, hit-test, and diagnostics behavior without adding a generic fallback compositor. |
