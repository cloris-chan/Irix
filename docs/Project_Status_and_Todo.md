...
- `VirtualNodeDiffer` 已实现局部 diff：递归深比较 + keyed reconciliation + Update/Add/Remove patches；`default` 树边界处理已完善；259 个测试用例覆盖各场景
...
- ✅ `VirtualNodeDiffer` 已从 `ReplaceRoot` 提升到局部 diff（Update/Add/Remove + keyed reconciliation），并纳入当前 259 个全量测试覆盖
...
- [x] `CompositorLoop` 合并式 render request 行为测试：连续请求只排队一次、渲染中后补一帧、普通 empty diff 不等同 render request；`RequestRenderAndWaitAsync` 与普通 diff `PublishAndWaitRenderAsync` 都在对应 `RenderAsync` 完成后才 complete（259 个测试，全部通过）
...
- [x] Scroll 回归测试：新增覆盖首笔 pending delta 不丢、Windows 向下单刻度 target=54、默认窗口下 Counter view 可滚动、layout max 回传后单刻度仍得到 target=54、快速 100 次 wheel 在 render wait 阻塞期间合并为少量 ScrollFrame、低频 render completion 不堆积旧帧、普通 diff patch render wait、`RequestRenderAndWaitAsync` 保留给无 diff repaint、第二帧 `dt` 包含 render wait、raw wheel 方向、`--diagnose-scroll` synthetic pump 输出；全量 259 个测试通过
...
已接受 [ADR-Scissor-Clipping-v0.md](ADR-Scissor-Clipping-v0.md)：D3D12 backend 默认仍是 `ClipMode=Diagnostic`，正常 PoC 不启用 GPU per-command scissor；手测可用 `--enable-scissor` 显式打开 FillRect scissor，启动输出和 `--debug-ui` 都显示当前 clip mode。当前手测现象已记录为 v0 expected：FillRect 会被 D3D12 rasterizer scissor 裁剪，DrawTextRun 仍走 Direct2D/DirectWrite overlay，不会被 D3D12 scissor 裁剪；因此 button 背景可裁剪但文字可能越界，这不代表完整控件裁剪已完成。`DrawCommand.ClipBounds` 已经从 layout 传到 backend；`DrawingScissor.ResolveEffectiveScissor` 已抽为纯函数，覆盖 default clip、viewport 内 clip、超出 viewport 交集、空交集；`DrawingScissor.ToIntegerScissorRect` 明确 floor origin / ceil extent / clamp viewport 策略，避免 float clip 转整数时缩小有效区域。`ClipMode=Scissor` 目前只应用 FillRect：default clip 使用 full viewport，空交集 skip，按连续相同 scissor run 设置 `RSSetScissorRects`；run-length 计数已覆盖连续相同 scissor、不同 scissor 切换、相同 scissor 非连续，以及 Diagnostic/Scissor mode 差异。`--diagnose` 包含三类 scissor smoke：direct（`effectiveClip=(32,32,80,40)`、`textClip=False`、`gpuScissor=True`、`clippedCommands=1`、`emptyIntersectionSkipped=0`、`scissorStateChanges=1`、`deviceRemoved=False`）、pipeline（`source=ScrollContainerRectangle`、`textClip=False`、`clippedCommands=1`、`emptyIntersectionSkipped=0`、`scissorStateChanges=1`、`deviceRemoved=False`、`passed=True`）、empty（`kind=FillRect`、`clippedCommands=1`、`emptyIntersectionSkipped=1`、`scissorStateChanges=0`、`deviceRemoved=False`）。D2D text clip v0 只完成设计：DrawTextRun 后续应按 `ClipBounds` 解析 effective clip，空交集 skip，非 full-viewport clip 在 `DrawTextLayout` 前后 push/pop axis-aligned Direct2D clip；等 text clip 最小闭环和诊断完成后，才考虑把 `--enable-scissor` 变成默认。GPU partial redraw、主题系统、通用控件抽象继续暂缓。
