...
Glyph atlas `Wrap` now has minimal whitespace-based multi-line support in the D3D12 path, and ASCII CR/LF explicit line breaks plus tab spacing are accepted. Simple Latin-1 BMP characters covered by the selected DirectWrite face are now accepted by the atlas classifier instead of being pre-classified as NonAscii. `NoWrap` still clips over-wide line segments; words that cannot fit, complex non-Latin text, combining marks, surrogate pairs/emoji, complex shaping, and over-height line stacks still degrade explicitly without D3D11On12 / D2D fallback.
