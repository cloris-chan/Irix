...
| `ScrollDiagnosticsSnapshot` | scroll service owner; currently PoC scroll pump/controller | `Program` can assemble from `_scrollFramePump` and current `CounterModel.Scroll` | `--diagnose-scroll`, debug UI scroll row, tests | Allowed in v0 because `ScrollFramePump` is still PoC-owned; static access must stay read-only and local to diagnostics. |
| `InputDiagnosticsSnapshot` | input routing/focus owner; currently PoC input ownership state/router | `RunInputDiagnosticModeAsync` can assemble from `InputOwnershipState` and `CounterInputRouter` scripted events | `--diagnose-input`, debug UI input row, tests | Allowed in v0 because input ownership is still PoC-owned; do not expose statics as framework API. |
...
Implemented partial sample: `RenderingPipelineDiagnosticSnapshot`. The current v0 slice covers dirty command ranges, dirty range alignment, backend clipped command count, layout command/clipped counts, and layout rebuild count/reason/classifications. Compositor render/partial/full/empty counters, hit target details, and scroll container details intentionally remain on the existing direct output path.
...
| Pick minimum implementation candidate | `StyleOnlyPatchPlanDiagnosticSnapshot` is implemented first; `ViewportDiagnosticsSnapshot` second; `BackendClipTextDiagnosticSnapshot` third; `RenderingPipelineDiagnosticSnapshot` is partial/minimal. |
