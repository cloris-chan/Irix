...
- A D3D12 render-target-backed layer cache for rect and GlyphAtlas text suffix layers. Stable layer content is recorded into D3D12 textures with RTV/SRV descriptors on cache miss and then composited back through a textured quad on subsequent compositor ticks.
...
- No generalized render-target allocation model for image/vector layers yet; the current D3D12 slice is rect/GlyphAtlas-text and order-preserving for suffix layer ranges.
