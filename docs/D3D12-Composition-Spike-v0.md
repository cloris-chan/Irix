...
- A D3D12 render-target-backed layer cache for rect and GlyphAtlas text layers. Stable layer content is recorded into D3D12 textures with RTV/SRV descriptors on cache miss and then composited back through command-order render segments using textured quads on subsequent compositor ticks.
...
- No generalized render-target allocation model for image/vector layers yet; the current D3D12 slice is rect/GlyphAtlas-text and preserves layer interleaving through command-order render segments.
...
- first presented frame misses the D3D12 render target cache and records retained rect/GlyphAtlas text layer content into an RTV/SRV texture
