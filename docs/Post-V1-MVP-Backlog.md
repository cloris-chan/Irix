...
- BiDi/shaping oracle coverage now pins the current resolved-level visual-order helper with nested examples and a DirectWrite analyzer-level diagnostic (`--diagnose-glyph-atlas-bidi-oracle`) that feeds real `AnalyzeBidi` resolved levels into the same helper. A full DirectWrite-layout pixel or glyph-run oracle remains a P2 production-quality correctness task.
