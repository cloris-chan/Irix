...
| ID | Task | Current status | Blocking condition |
|----|------|---------------|-------------------|
| POST-017 | D3D12-only glyph atlas text renderer | Default-on prototype foundation with overlay renderer removed | Post-GA; `GlyphAtlas` is the only D3D12 PoC text composition path; narrow single-line ASCII runs have local evidence and unsupported cases degrade |
...
The first atlas execution path records a D3D12 glyph pass for basic single-line ASCII runs that fit without wrapping, uses an `R8_UNORM` atlas, and supports leading/center/trailing alignment plus per-run scissor for accepted runs.
...
Full shaping, actual multi-line wrapping, color glyphs, fallback font identity, eviction, command-order-perfect mixed text z-order, and production enablement remain follow-up work.
...
- Default GlyphAtlas degrades unsupported renderable runs while accepted single-line ASCII runs stay on atlas. Initialization and runtime record failure degrade all renderable runs for the frame.
...
- No complex shaping, fallback font identity, color glyphs, SDF/MSDF, or actual multi-line wrapping support in the atlas path.
