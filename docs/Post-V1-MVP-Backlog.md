...
| ID | Task | Current status | Blocking condition |
|----|------|---------------|-------------------|
| POST-017 | D3D12-only glyph atlas text renderer | Default-on prototype foundation with overlay renderer removed; shaped atlas drawing with DirectWrite fallback-face segmentation plus explicit CR/LF/tab/wrap support added | Post-GA; `GlyphAtlas` is the only D3D12 PoC text composition path; simple runs and shaped/fallback runs have local evidence, complex line breaking and color/surrogate glyph cases still degrade |
...
Shaped explicit CR/LF line breaks, tab controls, and minimal whitespace wrapping now remain on the atlas path. Complex script line breaking, color glyphs, surrogate/emoji handling, eviction, command-order-perfect mixed text z-order for still-degraded text, and production enablement remain follow-up work.
