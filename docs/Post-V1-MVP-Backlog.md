...
| ID | Task | Current status | Blocking condition |
|----|------|---------------|-------------------|
| POST-017 | D3D12-only glyph atlas text renderer | Default-on prototype foundation with overlay renderer removed; shaped atlas drawing with DirectWrite fallback-face segmentation and explicit CR/LF line support added | Post-GA; `GlyphAtlas` is the only D3D12 PoC text composition path; simple runs and `NoWrap` shaped/fallback runs have local evidence, shaped wrapping beyond explicit line breaks and color/surrogate glyph cases still degrade |
...
Shaped explicit CR/LF line breaks now remain on the atlas path. Shaped wrapping beyond explicit line breaks, shaped tabs, color glyphs, surrogate/emoji handling, eviction, command-order-perfect mixed text z-order for still-degraded text, and production enablement remain follow-up work.
...
- Color glyphs, SDF/MSDF, shaped tabs, surrogate/emoji shaping, and shaped wrapping beyond explicit CR/LF remain outside the atlas path. Shaped drawing now covers `NoWrap` nonzero glyph runs with DirectWrite fallback-face segmentation and explicit line spans.
