...
| ID | Task | Current status | Blocking condition |
|----|------|---------------|-------------------|
| POST-017 | D3D12-only glyph atlas text renderer | Default-on prototype foundation with overlay renderer removed; shaped atlas drawing with DirectWrite fallback-face segmentation, explicit CR/LF/tab/wrap support, over-height line-stack scissor clipping, DirectWrite script/bidi analysis for complex scripts, single-level RTL `NoWrap` and wrapped lines, mixed BiDi nested resolved-level segment ordering, DirectWrite outline/COLR color-glyph layer drawing, DirectWrite premultiplied BGRA glyph-image-data drawing, WIC-decoded PNG/JPEG/TIFF glyph-image drawing, returned-ppem scaling for bitmap glyph image data, format-specific remaining color guards, and a fixed broad regression matrix added | Post-GA; `GlyphAtlas` is the only D3D12 PoC text composition path; simple runs, shaped/fallback runs, LTR complex-script runs, single-level RTL lines, mixed BiDi resolved-level runs, COLR-style color layer runs, BGRA-only color glyph image data, and encoded bitmap color glyph image data have local coverage, while SVG/COLR paint-tree color glyph rendering and BiDi cases beyond resolved-level segment projection still degrade explicitly |
| POST-011 | Resource cache / stable global handles | Entry/page handles, on-demand 48-page atlas pool, explicit atlas budget/resident-byte diagnostics, page-owned SRV resources, format-scoped cold-page reuse, retained-floor-gated next-record reuse when immediate reuse is unsafe, page usage diagnostics, soak/memory-pressure runner, atlas touch serials, local glyph-atlas regression lane, and entry-eviction design preconditions done | D3D12-specific; full LRU/entry-level eviction remains blocked until retained atlas command ownership and sub-rect free-list implementation ownership are explicit |
...
Current default GlyphAtlas behavior degrades unsupported and initialization/upload/record-failed renderable text runs without invoking overlay. `IDWriteTextAnalyzer` is now created for the atlas path and handles `NonAscii` single-face `NoWrap` shaped runs through pointer-based `GetGlyphs` / `GetGlyphPlacements`, projecting shaped glyph output into local `ShapedGlyph` scratch and a synchronous `ShapedGlyphRun` span view without retaining source strings. Accepted shaped glyphs reuse the existing D3D12 atlas raster/cache/draw path. DirectWrite color-glyph candidates retain the D3D12-only path by translating emoji/VS16 candidate segments through `IDWriteFactory4.TranslateColorGlyphRun` and `IDWriteColorGlyphRunEnumerator1` with only D3D12-renderable layer/bitmap formats requested. Outline/COLR layer runs rasterize into the existing `R8_UNORM` atlas with a separate color-layer glyph atom and draw with per-layer vertex color; BGRA color runs call `IDWriteFontFace4.GetGlyphImageData(PREMULTIPLIED_B8G8R8A8)`, upload raw pixels into `Bgra` atlas pages, and draw through a BGRA pixel shader with source colors preserved and only text alpha applied. PNG/JPEG/TIFF color glyphs call `GetGlyphImageData`, decode through WIC into `32bppPBGRA`, upload into the same `Bgra` atlas pages, and draw through the same BGRA shader without text-brush tinting. Bitmap format combinations prefer BGRA before encoded PNG/TIFF/JPEG. `--diagnose-glyph-atlas-color-formats` probes both `GetGlyphImageFormats` and `TranslateColorGlyphRun` for the installed DirectWrite color font so local evidence can distinguish "font exposes renderable COLR/layer runs only" from "BGRA atlas path regressed." Non-emoji supplementary pairs are not forced through the color route; SVG and COLR paint-tree-only glyphs remain explicit `ColorGlyph` degradation with per-format diagnostics.
...
Shaped explicit CR/LF line breaks, tab controls, minimal whitespace wrapping, unbreakable wrap-word and over-height line-stack scissor clipping, LTR complex-script runs after DirectWrite `AnalyzeScript` / `AnalyzeBidi`, single-level RTL `NoWrap` segments, RTL-base wrapped/mixed lines including leading weak digits before the first RTL strong character, Hebrew/Arabic presentation-form RTL classification, and mixed BiDi nested resolved-level segment ordering now remain on the atlas path. `--diagnose-glyph-atlas-matrix` pins ASCII, Latin Extended, Greek, Cyrillic, CJK, Arabic, Hebrew, mixed BiDi, emoji, wrap, tab, and CRLF in one fixed smoke. SVG/COLR paint-tree color glyph rendering, BiDi cases beyond resolved-level segment projection, eviction, command-order-perfect mixed text z-order for still-degraded text, and production enablement remain follow-up work.
...
- No same-record touched-page eviction. AtlasFull degradation is safe for the current prototype; page reuse is format-scoped and may immediately recycle only cold pages not touched in the current record. If no immediate cold page is available, it schedules tested record-serial- and retained-floor-gated next-record cold-page reset/reuse requests so accepted runs retained from the triggering record cannot sample recycled regions.
...
- Glyph atlas diagnostics report total plus Alpha/Bgra page counts, fixed page budget, page dimensions, total atlas pixel capacity, resident CPU shadow bytes, resident D3D12 upload-buffer bytes, resident D3D12 texture bytes, total and Alpha/Bgra completed and pending next-frame reuse counts, scheduled page reuse requests, hard total and Alpha/Bgra AtlasFull-without-reuse counts, total and Alpha/Bgra used glyph bitmap pixels, total and Alpha/Bgra shelf fragmentation estimates, atlas record serial, total and format-split oldest page age metrics, upload bytes, uploaded glyph count, accepted color layer/bitmap run counts, shaped/raster scratch bytes and reset-scoped resize counts, and split color/complex degradation reasons for future page-size, BGRA pressure, LRU, and unsupported-text policy decisions. The soak runner's policy line reads the page budget from the renderer's `AtlasPageBudget` constant rather than carrying its own literal.
- SDF/MSDF, SVG/COLR paint-tree color glyph rendering and BiDi cases beyond resolved-level segment projection remain outside the atlas path. Surrogate pairs and variation selectors now shape far enough to draw DirectWrite outline/COLR color layers through D3D12 when available, BGRA glyph image data uses the D3D12 `Bgra` atlas path, and PNG/JPEG/TIFF glyph image data decodes through WIC into the same BGRA atlas path; `IDWriteFontFace4` format checks prevent remaining unsupported formats from being misaccepted as layer glyphs. Complex-script candidates run through DirectWrite script/bidi analysis, even-level LTR runs are accepted, odd-level `NoWrap` segments draw from the segment's right edge, RTL-base wrapped/mixed lines draw from the line's right edge even when weak digits precede the first RTL strong character, and mixed-level shaped lines apply nested resolved-level visual ordering before glyph placement. Shaped drawing covers nonzero glyph runs with DirectWrite fallback-face segmentation, explicit line spans, tab control advances, minimal whitespace wrapping when DirectWrite cluster maps are monotonic enough to project per-character advances, unbreakable wrap-word and over-height line-stack scissor clipping, LTR complex-script runs, single-level RTL lines, mixed BiDi resolved-level runs, Factory4 `IDWriteColorGlyphRunEnumerator1` layer/bitmap runs, BGRA glyph image data returned by `GetGlyphImageData`, and WIC-decoded encoded bitmap glyph image data.
- Local color-font natural coverage is machine-readable: default Segoe UI Emoji is currently `LayerOnly` with `naturalBgraSmoke=False`, while the Noto Color Emoji Windows-compatible font-file smoke reports `BitmapRenderableAvailable`, `encodedBitmapRoute=True`, `imageDataRoute=True`, and `decodedBitmapRoute=True` with all eight probes exposing PNG image data and WIC-decoding to `32bppPBGRA`. A public `noto-sbix.ttf` font-file probe also exposes PNG image data, not BGRA/TIFF. Direct BGRA/TIFF natural font coverage remains unavailable locally; the shared encoded bitmap PNG/JPEG/TIFF path has natural PNG coverage plus code-level selector/raster/decode/page-format tests and guards.
- BiDi/shaping oracle coverage now pins the current resolved-level visual-order helper with nested examples, a DirectWrite analyzer-level diagnostic (`--diagnose-glyph-atlas-bidi-oracle`) that feeds real `AnalyzeBidi` resolved levels into the same helper, and a DirectWrite glyph oracle diagnostic (`--diagnose-glyph-atlas-glyph-oracle`) that reports glyph count, glyph indices, advances, offsets, bidi levels, line-break flags, and fallback-font segments without `IDWriteTextLayout` or D2D rendering. A full DirectWrite-layout pixel oracle remains a P2 production-quality correctness task.
...
- Resource cache / stable handles: POST-011 now has generation-safe page-level reuse with separate Alpha/Bgra pending slots plus `--diagnose-glyph-atlas-soak` memory-pressure evidence and `scripts/glyph-atlas-regression.ps1` for the fixed matrix/soak/color-format/oracle lane. `Smoke` remains configured in Windows CI, but GitHub Actions quota is currently exhausted, so local `Smoke` guard summaries are the status source. Run `Smoke` before/after broad changes, `Local` as the 300-frame manual soak after glyph/page/shaping changes, and `Nightly` as a 900-frame manual long run only after page-policy, eviction, or shaping overhauls. Do not add artifact-upload work until Actions quota returns. Continue with resource lifetime hardening and only move to full LRU/entry-level eviction after retained atlas command ownership and sub-rect free-list ownership are explicit.
