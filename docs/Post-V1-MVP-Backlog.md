...
| ID | Task | Current status | Blocking condition |
|----|------|---------------|-------------------|
| POST-017 | D3D12-only glyph atlas text renderer | Default-on prototype foundation with overlay renderer removed; shaped atlas drawing with DirectWrite fallback-face segmentation, explicit CR/LF/tab/wrap support, and DirectWrite color-glyph layer drawing added | Post-GA; `GlyphAtlas` is the only D3D12 PoC text composition path; simple runs, shaped/fallback runs, and COLR-style color layer runs have local evidence, while complex-script candidates and non-layer color glyph formats still degrade explicitly |
...
Shaped explicit CR/LF line breaks, tab controls, minimal whitespace wrapping, and unbreakable wrap-word scissor clipping now remain on the atlas path. Known complex-script ranges are guarded before shaping and degrade explicitly until line breaking/ordering is correctness-complete. Color bitmap/SVG glyph formats beyond DirectWrite color-run layers, eviction, command-order-perfect mixed text z-order for still-degraded text, and production enablement remain follow-up work.
...
- SDF/MSDF, color bitmap/SVG glyph formats beyond DirectWrite color-run layers, and complex-script line breaking/ordering remain outside the atlas path. Surrogate pairs and variation selectors now shape far enough to draw DirectWrite color layers through D3D12 when available; known complex-script ranges are guarded before shaping. Shaped drawing covers nonzero glyph runs with DirectWrite fallback-face segmentation, explicit line spans, tab control advances, minimal whitespace wrapping when DirectWrite cluster maps are monotonic enough to project per-character advances, unbreakable wrap-word scissor clipping, and COLR-style layer runs returned by `TranslateColorGlyphRun`.
...
- Degradation follow-up: AtlasFull, guarded complex-script/color cases, and record-failure contracts are recorded as degradation. Full LRU eviction and D3D12 rendering for currently unsupported text remain future work before widening atlas text coverage.
