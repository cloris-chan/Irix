...
- Local color-font natural coverage is machine-readable: default Segoe UI Emoji is currently `LayerOnly` with `naturalBgraSmoke=False`, while the Noto Color Emoji Windows-compatible font-file smoke reports `BitmapRenderableAvailable`, `encodedBitmapRoute=True`, `imageDataRoute=True`, and `decodedBitmapRoute=True` with all eight probes exposing PNG image data and WIC-decoding to `32bppPBGRA`. Direct BGRA/TIFF natural font coverage remains unobserved locally; the shared encoded bitmap PNG/JPEG/TIFF path now has natural PNG coverage plus code-level selector/raster/decode/page-format tests and guards.
