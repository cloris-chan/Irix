...
- Local color-font natural coverage is machine-readable: default Segoe UI Emoji is currently `LayerOnly` with `naturalBgraSmoke=False`, so a natural BGRA/PNG/JPEG/TIFF smoke requires an installed color font that exposes DirectWrite bitmap glyph image data. Current BGRA/encoded coverage remains through code-level selector/raster/decode/page-format tests and guards.
