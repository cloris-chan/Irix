...
| ID | Task | Current status | Blocking condition |
|----|------|---------------|-------------------|
| POST-017 | D3D12-only glyph atlas text renderer | Default-on prototype foundation with overlay renderer removed; shaped atlas drawing with DirectWrite fallback-face segmentation plus explicit CR/LF/tab/wrap support added | Post-GA; `GlyphAtlas` is the only D3D12 PoC text composition path; simple runs and shaped/fallback runs have local evidence, complex-script candidates and color/surrogate glyph cases still degrade explicitly |
...
- Color glyph drawing, SDF/MSDF, surrogate/emoji rendering, and complex-script line breaking/ordering remain outside the atlas path. Surrogate pairs and variation selectors now shape far enough to probe DirectWrite color layers but still degrade explicitly; known complex-script ranges are guarded before shaping. Shaped drawing covers nonzero glyph runs with DirectWrite fallback-face segmentation, explicit line spans, tab control advances, and minimal whitespace wrapping when DirectWrite cluster maps are monotonic enough to project per-character advances.
