...
- Glyph atlas diagnostics report total plus Alpha/Bgra page counts, fixed page budget, page dimensions, total atlas pixel capacity, resident CPU shadow bytes, resident D3D12 upload-buffer bytes, resident D3D12 texture bytes, completed and pending next-frame reuse counts, scheduled page reuse requests, hard AtlasFull-without-reuse counts, used glyph bitmap pixels, a shelf fragmentation estimate, atlas record serial, oldest/newest page age metrics, upload bytes, uploaded glyph count, accepted color layer/bitmap run counts, and split color/complex degradation reasons for future page-size, BGRA pressure, LRU, and unsupported-text policy decisions.
