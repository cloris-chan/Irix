...
| ID | Task | Current status | Blocking condition |
|----|------|---------------|-------------------|
| POST-017 | D3D12-only glyph atlas text renderer | Default-on prototype foundation with overlay renderer removed | Post-GA; `GlyphAtlas` is the only D3D12 PoC text composition path; narrow ASCII runs, including minimal space wrapping, have local evidence and unsupported cases degrade |
...
- Default GlyphAtlas degrades unsupported renderable runs while accepted ASCII runs stay on atlas. Minimal `Wrap` support breaks at spaces only; initialization and runtime record failure degrade all renderable runs for the frame.
