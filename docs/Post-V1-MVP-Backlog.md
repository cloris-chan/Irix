...
- Color glyphs, SDF/MSDF, surrogate/emoji shaping, and complex-script line breaking/ordering remain outside the atlas path. Surrogate pairs, variation selectors, and known complex-script ranges are guarded before shaping so they degrade explicitly. Shaped drawing now covers nonzero glyph runs with DirectWrite fallback-face segmentation, explicit line spans, tab control advances, and minimal whitespace wrapping when DirectWrite cluster maps are monotonic enough to project per-character advances.
