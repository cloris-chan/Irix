...
- Additional public `noto-sbix.ttf` probing also exposed PNG image data for all eight probes, but still did not expose DirectWrite `PREMULTIPLIED_B8G8R8A8` or TIFF image data.
...
- `scripts/glyph-atlas-regression.ps1 -Mode Smoke -ColorFontFile <path>` is the current environment hook for future BGRA/PNG/TIFF natural coverage probing. Acceptance for this item is either evidence with `atlasBgraPages > 0` in a renderer smoke or an explicit unavailable record like this one when the tested fonts only expose layer/PNG routes.
