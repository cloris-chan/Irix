...
Concurrent multi-owner transition support is now written as a design preflight in [Animation-Composition.md](Animation-Composition.md). The required future shape is runtime-owned owner identity and decision batching, retained target validation against one publication, compositor presentation-set validation, per-owner completion tracking, and diagnostics for accepted/rejected owners. The current implementation remains single-owner and keeps multi-target Counter deltas on abort-and-normal-dispatch until those pieces exist.
