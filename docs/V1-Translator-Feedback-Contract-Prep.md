...
|------|---------------|----------------|
| Style source | Internal `TranslatorRenderPipelineFactory.Default` creates a pipeline from `CounterStylePreset.Default` | Framework promotion would still bake Counter style into a reusable bridge if the default were moved as-is. |
...
| Render-pipeline creation | Private `RenderPipeline` created once through internal `TranslatorRenderPipelineFactory` | Creation is replaceable for tests/PoC composition, but no framework factory contract exists. |
...
| `TranslatorRenderPipelineFactory` | Internal PoC seam that creates `RenderPipeline` with explicit style/options. | Implemented internally; not a framework API. Default still uses Counter style. |
| `RenderPipelineFactory` | Future promoted factory shape if the translator ever leaves `Irix.Poc`. | Must avoid hidden Counter defaults and expose only framework-owned style/options. |
...
|-------------------|-------------|--------|
| Style source | `TranslatorStyleSource` | Clear enough for design; partially prepared by internal factory seam, but still blocked for promotion by default `CounterStylePreset.Default`. |
...
| Render-pipeline creation | `TranslatorRenderPipelineFactory` / `RenderPipelineFactory` | Clear enough for design; internal seam exists, promoted factory contract remains unimplemented. |
...
|------|-------------|----------------|----------|
| Style source | Style is injected or resolved outside the translator. | Partial: internal factory seam exists, but default still resolves to `CounterStylePreset.Default`. | Keep in PoC. |
...
| Render-pipeline creation | Pipeline creation uses explicit style/options/factory. | Partial: internal `TranslatorRenderPipelineFactory` creates the private pipeline once; no promoted contract exists. | Do not move. |
...
- Style source is still sample-shaped by default through `CounterStylePreset.Default`, even though tests can inject an internal factory.
...
- Render-pipeline creation is internally replaceable but still private and PoC-owned.
...
1. Preserve feedback compatibility with tests for `LastScrollFeedback`, `LastMaxScrollY`, and legacy max-scroll callback alignment. Done.
2. Add an internal pipeline creation seam while keeping the default behavior. Done via `TranslatorRenderPipelineFactory`.
3. Define `TranslatorStyleSource` and remove Counter style construction from any promoted bridge shape.
4. Define `TranslatorViewportSource` with physical-pixel units, resize timing, and fallback semantics.
5. Decide `TranslatorRetainedTreeOwner`: embedded bridge ownership or external render service ownership.
6. Promote a `RenderPipelineFactory` or equivalent explicit pipeline creation contract only after style ownership is framework-shaped.
7. Define `FramePreparationHook` semantics or remove the hook from the promoted shape.
8. Define `TranslatorFrameFeedback` with scroll feedback and local layout diagnostics.
9. Only then reconsider moving `WindowDrawCommandTranslator` out of `Irix.Poc`.
...
- Add focused tests proving internal factory/default style equivalence when changing the PoC seam.
...
Current status: internal PoC seam complete for style/pipeline creation, with default behavior preserved. Future work must reopen this line explicitly before adding framework APIs or moving code.
