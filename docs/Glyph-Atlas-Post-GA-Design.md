...
`recordFailurePhase` is reserved for runtime command recording failures after atlas initialization succeeds. Current phases cover generic record failure, DirectWrite runtime lookup/measurement/rasterization failure, vertex-buffer map failure, and atlas-upload map failure. A runtime record failure disables the current atlas renderer instance and degrades renderable text for subsequent frames without marking the D3D12 device lost by itself.
