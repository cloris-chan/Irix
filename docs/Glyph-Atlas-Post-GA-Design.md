...
| Not implemented | Color glyphs, surrogate pair/color emoji handling, shaped wrapping beyond explicit CR/LF line breaks, full LRU/entry-level eviction, and recovery beyond explicit degradation |
...
Removing the remaining limitation requires D3D12 handling for shaped wrapping beyond explicit CR/LF breaks and unsupported glyph/color cases or an accepted product decision that those cases may remain degraded.
...
- Color glyphs, surrogate pair/emoji shaping, and wrapping beyond ASCII explicit breaks / whitespace breaks are still degradation cases.
