...
Color-glyph format diagnostic update: `--diagnose-glyph-atlas-color-formats [ppem]` probes the selected DirectWrite emoji/color font family, defaulting to Segoe UI Emoji, through both `IDWriteFontFace4.GetGlyphImageFormats` and `IDWriteFactory4.TranslateColorGlyphRun`. It reports direct layer, BGRA, PNG/JPEG/TIFF, SVG, and COLR paint-tree candidates, translated color-run count/formats, and the D3D12 bitmap route selector result. This is diagnostic evidence for whether a local machine can naturally produce `atlasBgraPages > 0`; if the installed font exposes DirectWrite-renderable COLR/layer runs but no bitmap glyph image data, the ordinary wrap smoke can remain `atlasBgraPages=0` without implying that the BGRA atlas code path was removed.
Natural color coverage update: the same diagnostic now emits a machine-readable natural coverage line. `status=LayerOnly` with `naturalBgraSmoke=False` is the expected local result when the installed emoji/color font exposes `TranslateColorGlyphRun` outline/COLR layers but no DirectWrite BGRA/PNG/JPEG/TIFF glyph image data. A natural BGRA/encoded smoke requires a font/environment with `bitmapRenderableRoute=True`; until then, BGRA/encoded bitmap code coverage remains through selector, raster/decode, page-format, diagnostics, and source guards.
...
Page diagnostics now expose total plus Alpha/Bgra page counts, the fixed page budget, page dimensions, total atlas pixel capacity, total plus Alpha/Bgra pending/completed page reuse state, scheduled page reuse request count, hard AtlasFull-without-reuse count, used glyph bitmap pixels, a shelf-allocation fragmentation estimate, the atlas record serial, and oldest/newest page age metrics. These are observation fields for deciding page size, multi-page thresholds, BGRA color-glyph pressure, and LRU policy; they do not change current draw or degradation behavior.
