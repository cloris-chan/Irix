...
Color-glyph format diagnostic update: `--diagnose-glyph-atlas-color-formats [ppem]` probes the selected DirectWrite emoji/color font family, defaulting to Segoe UI Emoji, through `IDWriteFontFace4.GetGlyphImageFormats` and reports layer, BGRA, PNG/JPEG/TIFF, SVG, and COLR paint-tree candidates plus the D3D12 bitmap route selector result. This is diagnostic evidence for whether a local machine can naturally produce `atlasBgraPages > 0`; if the installed font exposes only COLR/layer data, the ordinary wrap smoke can remain `atlasBgraPages=0` without implying that the BGRA atlas code path was removed.
