...
| Supported atlas text | ASCII printable characters, simple Latin Extended / Greek / Cyrillic BMP characters covered by the selected DirectWrite face, single-face `NoWrap` shaped runs with nonzero glyph indices, full-run system fallback font shaping, explicit CR/LF line breaks, ASCII tab as four-space advance, no-wrap line segments that fit, minimal `Wrap` runs that can break at whitespace, DirectWrite glyph metrics/raster source |
...
| Shaped text | DirectWrite `IDWriteTextAnalyzer` and system `IDWriteFontFallback` are initialized for the atlas renderer. `NonAscii` runs shape through pinned `ReadOnlySpan<char>` input; selected-face missing glyphs can retry through one DirectWrite fallback font when the whole run maps to that face. Output is projected into renderer-owned `ShapedGlyph` scratch and exposed only as a synchronous `ShapedGlyphRun` span view. Probe/raster failure stays degradation-only and does not disable the atlas. |
...
| Not implemented | Mixed multi-face fallback segmentation, color glyphs, surrogate pair/color emoji handling, shaped wrapping/line breaking, full LRU/entry-level eviction, and recovery beyond explicit degradation |
...
Shaped-run update: the renderer now owns an `IDWriteTextAnalyzer` and `IDWriteFontFallback` alongside the DirectWrite factory/font collection and can use them for `NonAscii` runs. The probe pins resolved frame text as a span, calls the pointer overloads of `GetGlyphs` and `GetGlyphPlacements`, projects glyph index, advance, offset, cluster-start, diacritic, and zero-width flags into local `ShapedGlyph` scratch, and exposes that data through a non-retained `ShapedGlyphRun` span view. Single-face `NoWrap` shaped runs, including pure CJK runs that map to one system fallback face, now rasterize through the existing atlas glyph cache and draw path; source strings are never retained. Mixed ASCII/CJK or otherwise multi-face fallback segmentation and shaped wrapping still degrade explicitly.
...
The mixed fallback diagnostic now covers `ASCII -> CJK fallback-face -> clipped ASCII -> clipped CJK fallback-face` and is expected to remain atlas-only. A separate mixed multi-face diagnostic is still needed before accepting ASCII/CJK text in the same `DrawTextRun`.
Removing the remaining limitation requires D3D12 handling for segmented fallback runs or an accepted product decision that those cases may remain degraded.
...
- Mixed multi-face fallback shaping, color glyphs, and wrapping beyond ASCII explicit breaks / whitespace breaks are still degradation cases.
