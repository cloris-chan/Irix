...
| Not implemented | Color bitmap/SVG glyph formats beyond DirectWrite color-run layers, full BiDi visual reordering and RTL wrapping beyond current single-level `NoWrap` shaped segments, full LRU/entry-level eviction, and recovery beyond explicit degradation |
...
Shaped-run update: the renderer now owns an `IDWriteTextAnalyzer`, `IDWriteFactory2`, and `IDWriteFontFallback` alongside the DirectWrite factory/font collection and can use them for `NonAscii` runs. The probe pins resolved frame text as a span, runs DirectWrite script/bidi analysis into scratch, calls the pointer overloads of `GetGlyphs` and `GetGlyphPlacements`, projects glyph index, advance, offset, cluster-start, diacritic, and zero-width flags into local `ShapedGlyph` scratch, and exposes that data through a non-retained `ShapedGlyphRun` span view. Missing-glyph runs can now be segmented through DirectWrite `MapCharacters`; each shaped segment carries its cached font face, effective em size, and resolved bidi level, so mixed ASCII/CJK and single-level RTL `NoWrap` runs rasterize through the existing atlas glyph cache and draw path without retaining source strings. Shaped text accepts explicit CR/LF line breaks through renderer-owned line scratch, tabs through zero-glyph control segments, minimal whitespace wrapping by projecting monotonic DirectWrite cluster maps into per-character advances, unbreakable wrap words clipped by the existing text scissor, LTR complex-script runs, and uniform odd-level `NoWrap` shaped spans drawn from the segment's right edge. Surrogate pairs and variation selectors now shape far enough to translate `IDWriteColorGlyphRunEnumerator` layers; each layer glyph rasterizes into the existing `R8_UNORM` atlas through a distinct color-layer glyph atom and draws with DirectWrite palette/current-brush vertex color. Full BiDi visual reordering and RTL wrapping remain explicit degradation rather than partial atlas acceptance; diagnostics preserve aggregate `NonAscii` while splitting remaining unsupported cases into `ColorGlyph` and `ComplexScript`.
...
Removing the remaining limitation requires D3D12 handling for full BiDi visual ordering, RTL wrapping, and unsupported glyph/color cases or an accepted product decision that those cases may remain degraded.
...
- Color bitmap/SVG glyph formats beyond DirectWrite color-run layers, full BiDi visual ordering, and RTL wrapping are still degradation cases.
