...
| Not implemented | Color bitmap/SVG glyph formats beyond DirectWrite color-run layers, deeper BiDi shaping cases beyond current resolved-level segment ordering, full LRU/entry-level eviction, and recovery beyond explicit degradation |
...
- Color bitmap/SVG glyph formats beyond DirectWrite color-run layers and deeper BiDi shaping cases beyond current segment-level ordering are still degradation cases.
