...
| Atlas format | `R8_UNORM` alpha atlas pages for alpha masks and outline/COLR color layers; `B8G8R8A8_UNORM` pages for DirectWrite premultiplied BGRA glyph image data and WIC-decoded PNG/JPEG/TIFF glyph images |
...
| Not implemented | SVG and COLR paint-tree color glyph rendering, deeper BiDi shaping cases beyond current resolved-level segment ordering, full LRU/entry-level eviction, and recovery beyond explicit degradation |
...
- SVG and COLR paint-tree color glyph formats plus deeper BiDi shaping cases beyond current segment-level ordering are still degradation cases. DirectWrite premultiplied BGRA glyph image data and WIC-decoded PNG/JPEG/TIFF glyph images are handled by the D3D12 `Bgra` atlas path.
...
- SVG and COLR paint-tree color glyph formats beyond DirectWrite outline/COLR color-run layers, premultiplied BGRA glyph image data, and WIC-decoded PNG/JPEG/TIFF glyph images are not supported by the current atlas path.
