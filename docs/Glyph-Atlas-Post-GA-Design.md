...
| Not implemented | Color bitmap/SVG glyph formats beyond DirectWrite color-run layers, full mixed BiDi visual reordering beyond current segment-level and single-level RTL line handling, full LRU/entry-level eviction, and recovery beyond explicit degradation |
...
Removing the remaining limitation requires D3D12 handling for full mixed BiDi visual ordering and unsupported glyph/color cases or an accepted product decision that those cases may remain degraded.
...
- Color bitmap/SVG glyph formats beyond DirectWrite color-run layers and full mixed BiDi visual ordering are still degradation cases.
