...
| Shaped text | DirectWrite `IDWriteTextAnalyzer` and system `IDWriteFontFallback` are initialized for the atlas renderer. `NonAscii` runs shape through pinned `ReadOnlySpan<char>` input; selected-face missing glyphs can retry through DirectWrite `MapCharacters` segments where each segment carries its own cached font face and effective em size. Output is projected into renderer-owned `ShapedGlyph` and segment scratch and exposed only as a synchronous `ShapedGlyphRun` span view. Probe/raster failure stays degradation-only and does not disable the atlas. |
...
| Not implemented | Color glyphs, surrogate pair/color emoji handling, shaped wrapping/line breaking, full LRU/entry-level eviction, and recovery beyond explicit degradation |
...
Removing the remaining limitation requires D3D12 handling for shaped wrapping and unsupported glyph/color cases or an accepted product decision that those cases may remain degraded.
