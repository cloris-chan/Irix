...
| Supported atlas text | ASCII printable characters, simple Latin Extended / Greek / Cyrillic BMP characters covered by the selected DirectWrite face, first single-face `NoWrap` shaped runs with nonzero glyph indices, explicit CR/LF line breaks, ASCII tab as four-space advance, no-wrap line segments that fit, minimal `Wrap` runs that can break at whitespace, DirectWrite glyph metrics/raster source |
...
| Shaped text | DirectWrite `IDWriteTextAnalyzer` is initialized for the atlas renderer and probes `GetGlyphs` / `GetGlyphPlacements` for `NonAscii` runs through pinned `ReadOnlySpan<char>` input. Output is projected into renderer-owned `ShapedGlyph` scratch and exposed only as a synchronous `ShapedGlyphRun` span view. A first `NoWrap` path rasterizes/draws single-face shaped glyphs through the existing D3D12 atlas. Probe/raster failure stays degradation-only and does not disable the atlas. |
...
| Not implemented | Fallback font identity, color glyphs, surrogate pair/color emoji handling, shaped wrapping/line breaking, full LRU/entry-level eviction, and recovery beyond explicit degradation |
...
Shaped-run update: the renderer now owns an `IDWriteTextAnalyzer` alongside the DirectWrite factory/font collection and can use it for `NonAscii` runs. The probe pins resolved frame text as a span, calls the pointer overloads of `GetGlyphs` and `GetGlyphPlacements`, projects glyph index, advance, offset, cluster-start, diacritic, and zero-width flags into local `ShapedGlyph` scratch, and exposes that data through a non-retained `ShapedGlyphRun` span view. Single-face `NoWrap` shaped runs with nonzero glyph indices now rasterize through the existing atlas glyph cache and draw path; source strings are never retained. CJK/fallback-face cases and shaped wrapping still degrade explicitly.
