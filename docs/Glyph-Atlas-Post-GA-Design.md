...
Shaped-run probe update: the renderer now owns an `IDWriteTextAnalyzer` alongside the DirectWrite factory/font collection and uses it only to probe `NonAscii` degraded runs. The probe pins resolved frame text as a span, calls the pointer overloads of `GetGlyphs` and `GetGlyphPlacements`, stores only scratch arrays and aggregate diagnostics, and never retains source strings. It is not yet an accepted atlas draw path; shaped text still degrades until glyph-run conversion/raster/draw integration is added.
