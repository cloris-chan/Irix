...
Regression lane update: `scripts/glyph-atlas-regression.ps1` is the fixed pre-merge smoke entry for the coverage freeze and is wired into Windows CI as `Glyph atlas regression lane`. Default `-Mode Smoke` runs the matrix, 60-frame soak, color-format natural coverage probe, BiDi oracle, and glyph oracle into `TestResults`; `-Mode Local` extends soak to 300 frames and should be run periodically on the development machine; `-Mode Nightly` extends soak to 900 frames and should be run after larger renderer changes. The summary files include machine-readable `matrix.expected`, `matrix.actual`, `soak.actual`, `bidi-oracle.expected`, `bidi-oracle.actual`, `glyph-oracle.expected`, and `glyph-oracle.actual` lines. The script validates those summary lines before reporting success: matrix actual must keep `degradedRuns=0`, `glyphAtlasInitialized=True`, and `overlaySync=False`; soak must keep `deviceLost=False`, `syncWaits=0`, `hardFullWithoutReuse=0`, `RecordFailed=0`, and `recordFailurePhase=None`; BiDi/glyph oracle expected and actual probe labels/counts must match with `overlayFallback=False`. The lane monitors page policy, resident bytes, fragmentation/reuse counters, degradation policy, color glyph natural coverage availability, and DirectWrite structural oracle output without adding renderer coverage.
