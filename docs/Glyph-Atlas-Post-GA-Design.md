...
| Not implemented | Color bitmap/SVG glyph formats beyond DirectWrite color-run layers, RTL/BiDi complex script ordering beyond current even-level shaped runs, full LRU/entry-level eviction, and recovery beyond explicit degradation |
...
Removing the remaining limitation requires D3D12 handling for RTL/BiDi complex script ordering and unsupported glyph/color cases or an accepted product decision that those cases may remain degraded.
...
- Color bitmap/SVG glyph formats beyond DirectWrite color-run layers and RTL/BiDi complex-script ordering are still degradation cases.
