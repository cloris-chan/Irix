...
| Diagnostics | `atlasPages`, `atlasEvictions`, `atlasUsed`, `atlasFragmented`, `AtlasRuns`, `DegradedRuns`, fallback/degradation frames, unsupported runs, and reason counts |
...
Page diagnostics now expose used glyph bitmap pixels and a shelf-allocation fragmentation estimate. These are observation fields for deciding page size, multi-page thresholds, and LRU policy; they do not change current draw or degradation behavior.
...
- Diagnostics expose `atlasPages`, `atlasEvictions`, `atlasUsed`, `atlasFragmented`, accepted text runs, degradation runs, per-run reasons, sync/present serials, and failure phases.
