...
| Supported atlas text | ASCII printable characters, simple Latin Extended / Greek / Cyrillic BMP characters covered by the selected DirectWrite face, explicit CR/LF line breaks, ASCII tab as four-space advance, no-wrap line segments that fit, minimal `Wrap` runs that can break at whitespace, DirectWrite glyph metrics/raster source |
...
| Shaping probe | DirectWrite `IDWriteTextAnalyzer` is initialized for the atlas renderer and probes `GetGlyphs` / `GetGlyphPlacements` for `NonAscii` degradation runs through pinned `ReadOnlySpan<char>` input. Probe output is projected into renderer-owned `ShapedGlyph` scratch and exposed only as a synchronous `ShapedGlyphRun` span view. Probe success is diagnostic-only today and does not draw shaped runs yet. Probe failure stays degradation-only and does not disable the atlas. |
...
| Not implemented | Drawing DirectWrite shaped glyph runs from the atlas, fallback font identity, color glyphs, surrogate pairs, combining marks, hyphenation/complex wrapping, full LRU/entry-level eviction, and recovery beyond explicit degradation |
...
Shaped-run probe update: the renderer now owns an `IDWriteTextAnalyzer` alongside the DirectWrite factory/font collection and uses it only to probe `NonAscii` degraded runs. The probe pins resolved frame text as a span, calls the pointer overloads of `GetGlyphs` and `GetGlyphPlacements`, projects glyph index, advance, offset, cluster-start, diacritic, and zero-width flags into local `ShapedGlyph` scratch, and exposes that data through a non-retained `ShapedGlyphRun` span view. It stores only scratch arrays and aggregate diagnostics, never source strings. It is not yet an accepted atlas draw path; shaped text still degrades until glyph-run raster/draw integration is added.
