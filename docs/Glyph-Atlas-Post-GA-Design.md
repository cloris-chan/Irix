...
| Atlas format | `R8_UNORM` alpha atlas pages for alpha masks and outline/COLR color layers; `B8G8R8A8_UNORM` pages for DirectWrite premultiplied BGRA glyph image data |
...
| Not implemented | SVG/PNG/JPEG/TIFF and COLR paint-tree color glyph rendering, deeper BiDi shaping cases beyond current resolved-level segment ordering, full LRU/entry-level eviction, and recovery beyond explicit degradation |
...
- SVG/PNG/JPEG/TIFF and COLR paint-tree color glyph formats plus deeper BiDi shaping cases beyond current segment-level ordering are still degradation cases. DirectWrite premultiplied BGRA glyph image data is handled by the D3D12 `Bgra` atlas path.
...
- SVG/PNG/JPEG/TIFF and COLR paint-tree color glyph formats beyond DirectWrite outline/COLR color-run layers and the premultiplied BGRA glyph-image-data path are not supported by the current atlas path.
