...
| Diagnostics | `atlasPages`, `atlasEvictions`, `AtlasRuns`, `DegradedRuns`, fallback/degradation frames, unsupported runs, and reason counts |
...
- Diagnostics expose `atlasPages`, `atlasEvictions`, accepted text runs, degradation runs, per-run reasons, sync/present serials, and failure phases.
