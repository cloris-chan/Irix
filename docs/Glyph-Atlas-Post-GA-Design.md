...
Page diagnostics now expose total plus Alpha/Bgra page counts, the fixed page budget, page dimensions, total atlas pixel capacity, pending/completed page reuse state, scheduled page reuse request count, hard AtlasFull-without-reuse count, used glyph bitmap pixels, a shelf-allocation fragmentation estimate, the atlas record serial, and oldest/newest page age metrics. These are observation fields for deciding page size, multi-page thresholds, BGRA color-glyph pressure, and LRU policy; they do not change current draw or degradation behavior.
