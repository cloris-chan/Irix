...
This preflight is now a Poc-owned value-model plus coordinator, compositor presentation-set validation, owner-table completion preflight, validation-only batch runtime preflight, and rendering-owned presentation-set activation preflight. It adds `StyleTransitionOwnerKey`, `StyleTransitionDecisionBatch`, per-owner validation results, deterministic acceptance/rejection tests, `StyleTransitionRuntimeCoordinator.ValidateBatch`, `StyleTransitionRuntimeCoordinator.ValidateBatchRuntime`, `StyleTransitionBatchRuntimePreflight`, `CompositionAnimationPresentationSetValidator`, `CompositionAnimationPresentationSetActivationPreflight`, `CompositionAnimationPresentationSetPlan`, `DrawingBackendCompositor.ValidateCompositionAnimationPresentationSet`, `DrawingBackendCompositor.PrepareCompositionAnimationPresentationSetActivation`, and owner-keyed `StyleTransitionCompletionTracker` state, but it does not add a public transition API, does not add a generic scheduler, does not move Poc runtime types into `Irix.Rendering`, does not activate multiple compositor owners, and does not change `DrawingBackendCompositor` execution contracts.
...
7. Next: define the active compositor state and tick contract for a validated presentation-set plan while keeping actual multi-owner routing in Counter deferred.
8. Only then allow Counter multi-target control-state deltas to use concurrent transitions instead of the current abort-and-normal-dispatch path.
