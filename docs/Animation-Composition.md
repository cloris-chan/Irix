...
The current implementation has the first boundary for this rule: `CompositionClock` is the internal source behind `CompositionTimestamp.Now()`, while deterministic tests and diagnostics can still inject explicit timestamps through `RenderCompositionAnimationTickAtAsync`, `RenderCompositionAnimationPresentationTickAtAsync`, and `RenderCompositionScrollPresentationTickAtAsync`. `DrawingBackendCompositor`, `CompositorLoop`, `StyleTransitionCompletionPump`, and `ScrollPresentationCoordinator` also accept an internal `ICompositionClockSource` for compositor-owned ticks, delayed tick sampling, automatic transform/opacity or presentation-set completion ticks, and scroll retarget segment starts, so delayed ticks, retargets, and marker completion pumping share the same clock boundary without using a per-output or backend clock. `CompositorLoop` owns work scheduling, delayed ticks, idle waiters, cancellation, and backend-present pacing policy; it does not own a separate animation time domain. D3D12, future Vulkan/Metal backends, native windows, and sync diagnostics may measure input/present/sync timing, but those measurements are not animation timeline authority.
...
Live wheel input probes the next logical target before cancellation, so repeated top/bottom boundary input does not restart a same-target segment. When a real retarget is needed, sampling and hold are serialized on the compositor thread: the old schedule stops and idle waiters complete, but the current visual presentation remains until the replacement retained frame plus declaration installs atomically.
...
Current runtime ownership preflight: `IStyleTransitionRuntimeAdapter`, `StyleTransitionRuntimeCoordinator`, `IStyleTransitionCompositorAdapter`, and `IStyleTransitionRetainedSnapshotProvider` live in `Irix.Poc`. The runtime adapter supplies a decision (`Start`, `Cancel`, `Retarget`, or `Commit`) and receives the result; the coordinator compiles only start/retarget decisions, installs compositor declarations only through the compositor adapter, and falls back before presentation ownership changes when the retained snapshot is missing or the compiler rejects a non-compositor-owned delta. The first concrete app/control integration is Counter-owned: `CounterStyleTransitionBridge` maps a single button control-state delta from `OwnershipSnapshot` into a semantic transform/opacity decision, keeps the narrow single-decision API for single-target deltas while resolving the Counter control-state owner key for completion tracking, and maps Counter multi-target button control-state deltas into a Poc-owned `StyleTransitionDecisionBatch`. It filters batch entries by actual transform/opacity deltas, so focus-only ownership changes remain normal app visual rendering instead of creating no-op compositor owners. Active scroll presentation remains a typed normal-dispatch fallback. Active-scroll fallback and blocked batch activation carry an explicit presentation policy: the Poc path aborts any active style transition presentation and clears tracker ownership instead of leaving stale compositor owners alive after app-owned state is published. `StyleTransitionCompletionTracker` is a Poc-owned marker-driven completion tracker: it tracks `Once` transform/opacity start or retarget decisions with valid animation instances, appends a progress-1 marker, preserves any existing markers, and converts matching `CompositionAnimationMarkerEvent` values into explicit `Commit` decisions that the runtime coordinator must still apply. The tracker now stores active entries in an owner table keyed by `StyleTransitionOwnerKey`, supersedes an older active owner for the same target when a new control-state owner takes over, and prevents one owner completion, cancel, fallback, or abort from committing or clearing another target during preflight; whole-presentation abort clears the owner table when the app falls back. `StyleTransitionCompletionPump` runs transform/opacity compositor ticks on the Poc side, drains marker events from `DrawingBackendCompositor`, and applies or records explicit commit decisions. It also has an active presentation-set tick path: `TickPresentationAndDrainAtAsync` renders validated presentation-set ticks, drains per-layer completion markers, removes completed owners from the owner table, clears their active presentation targets, reports explicit commit results, preserves other active owners, and drops the active presentation plan after the last tracked owner completes. `EnsureRunning` refreshes tick mode from the current compositor plan even when the pump is already running, and the run loop re-samples that plan before each tick so a single-target transition can switch to a newly activated presentation-set batch. The pump lifecycle diagnostics cover no-active, tick-rendered, completion-committed, tick-unavailable, error states, active owner count, and `tickMode` so diagnostics distinguish single-animation ticks from routed presentation-set ticks. Batch activation diagnostics report activation kind/reason, runtime/presentation preflight counts, cleanup result, and cleanup-after state for blocked fallback. These diagnostics are not a public timeline scheduler or runtime API. Loop and alternate timelines are not auto-completed by this slice. `CounterStyleTransitionRuntimeBridge` waits for the app runtime render to publish the retained snapshot before either the single-owner coordinator install or the batch presentation activation path mutates compositor ownership. For the main app path it also retimestamps short control-state transitions after publication, anchors a small initial visible sample, and renders one best-effort initial compositor tick immediately after activation so cross-button pressed presentation is visible before the background completion pump's next scheduled tick. `DrawingBackendCompositor` now has an internal active presentation-set plan and compositor-only tick path for validated transform/opacity plan sets, including per-layer marker playback and multi-layer capability diagnostics. `StyleTransitionRuntimeCoordinator.ActivateBatchPresentation` is the Poc-owned bridge for all-ready start/retarget batches: it reuses the validation-only runtime preflight, attaches completion markers, re-preflights through the rendering-owned activation contract, activates the presentation plan, and records owner-table tracking only after successful activation. `StyleTransitionRuntimeCoordinator.ClearBatchPresentation` is the matching Poc-owned all-ready cancel/commit bridge: it reuses runtime preflight, rejects mixed/blocked/start/retarget/missing-snapshot/no-tracked-clear cases before mutation, clears only matching active presentation targets, drops the whole presentation only when no plan entries remain, and publishes cancel/commit results to the owner table. This is now a narrow Counter multi-target route over the existing concurrent-owner preflight with per-owner compositor partial clear for active presentation plans, not a complete app-level multi-owner runtime. There is still no public transition API, theme/cascade resolver, generic scheduler, arbitrary app-level multi-owner route, or implicit compiler/compositor commit path.

## Concurrent Transition Owner Preflight

True concurrent style transitions require a design boundary before broad code changes. The current Counter path now routes multi-target button control-state deltas through the existing batch/presentation-set preflight, while active scroll presentation and blocked activation still fall back with explicit presentation cleanup. Future implementations must not broaden this beyond the written owner/preflight/completion model until commit, cancel, retarget, marker, and hit-test semantics stay deterministic.

This preflight is now a Poc-owned value-model plus coordinator, compositor presentation-set validation, owner-table completion preflight, validation-only batch runtime preflight, rendering-owned presentation-set activation preflight, internal active presentation-set compositor tick contract, Poc-owned presentation activation bridge, Poc-owned all-ready presentation clear bridge, active presentation-set completion pump preflight, and Counter multi-target routing policy. It adds `StyleTransitionOwnerKey`, `StyleTransitionDecisionBatch`, per-owner validation results, deterministic acceptance/rejection tests, `StyleTransitionRuntimeCoordinator.ValidateBatch`, `StyleTransitionRuntimeCoordinator.ValidateBatchRuntime`, `StyleTransitionRuntimeCoordinator.ActivateBatchPresentation`, `StyleTransitionRuntimeCoordinator.ClearBatchPresentation`, `StyleTransitionBatchRuntimePreflight`, `StyleTransitionBatchPresentationActivation`, `StyleTransitionBatchPresentationActivationResult`, `StyleTransitionBatchPresentationClear`, `StyleTransitionBatchPresentationClearResult`, `IStyleTransitionPresentationActivationCompositorAdapter`, `IStyleTransitionPresentationClearCompositorAdapter`, `CompositionAnimationPresentationSetValidator`, `CompositionAnimationPresentationSetActivationPreflight`, `CompositionAnimationPresentationSetPlan`, `DrawingBackendCompositor.ValidateCompositionAnimationPresentationSet`, `DrawingBackendCompositor.PrepareCompositionAnimationPresentationSetActivation`, `DrawingBackendCompositor.ActivateCompositionAnimationPresentationPlan`, `DrawingBackendCompositor.ClearCompositionAnimationPresentationTargets`, `DrawingBackendCompositor.RenderCompositionAnimationPresentationTickAtAsync`, `StyleTransitionCompletionPump.TickPresentationAndDrainAtAsync`, owner-keyed `StyleTransitionCompletionTracker` state, `CounterStyleTransitionBridge.TryCreateDecisionBatch`, and `CounterStyleTransitionRuntimeBridge.DispatchAndActivateInputTransitionBatchAsync`, but it does not add a public transition API, does not add a generic scheduler, does not move Poc runtime types into `Irix.Rendering`, and does not implement full app-level multi-owner execution.
...
Current preflight implementation: `StyleTransitionDecisionBatchPreflight` validates Poc-owned owner/decision batches against one retained publication and the existing `StyleTransitionCompiler`, then feeds compiled transform/opacity declarations into the rendering-owned presentation-set validator. That validator resolves declarations against the retained command frame, reports duplicate layer ids and overlapping command ranges before presentation state changes, and keeps `PresentationStateChanged=false`. `StyleTransitionRuntimeCoordinator.ValidateBatch` exposes that validation at the Poc coordinator boundary and preserves the existing single-owner apply path. `DrawingBackendCompositor.ValidateCompositionAnimationPresentationSet` exposes the same validation against the compositor's retained frame without activating a plan. `StyleTransitionCompletionTracker` now holds owner-keyed active entries and owner-aware completion/cancel/fallback/abort operations, while the pump has both the legacy single-animation tick path and a narrow presentation-set tick path. `StyleTransitionBatchRuntimePreflight` and `StyleTransitionRuntimeCoordinator.ValidateBatchRuntime` pair validation results with runtime action requirements: start/retarget owners report whether they require a presentation-set activation and completion marker attachment, cancel/commit owners report whether a tracked owner can be cleared, and rejected or mismatched owners are blocked before any presentation state changes. `CompositionAnimationPresentationSetActivationPreflight` resolves fully accepted declaration sets into `CompositionAnimationPresentationSetPlan` data and exposes per-entry resolved plans through `DrawingBackendCompositor.PrepareCompositionAnimationPresentationSetActivation`; mixed or rejected sets produce no aggregate plan. `DrawingBackendCompositor.ActivateCompositionAnimationPresentationPlan` can now make a validated plan set active, clears the legacy single-animation and scroll presentation owners, creates per-plan marker playback state, and `RenderCompositionAnimationPresentationTickAtAsync` executes the evaluated multi-layer `CompositionFrame` only after the retained frame and backend capabilities still match. `StyleTransitionBatchPresentationActivation` and `StyleTransitionRuntimeCoordinator.ActivateBatchPresentation` bridge all-ready start/retarget runtime preflight into that active presentation-set contract: they reject mixed/blocked batches, missing retained snapshots or frames, no presentation owner, and cancel/commit batches before activation; they attach completion markers and publish owner-table tracking only after the compositor accepts the plan. `StyleTransitionBatchPresentationClear` and `StyleTransitionRuntimeCoordinator.ClearBatchPresentation` now bridge all-ready cancel/commit runtime preflight into target-scoped presentation clear: they reject mixed/blocked batches, start/retarget owners, missing retained snapshots, and missing tracked owners before mutation, then remove only matching active presentation plan entries, drop the plan when the last entry is cleared, and clear only the matching owner-table entries. `StyleTransitionCompletionPump.TickPresentationAndDrainAtAsync` can render active presentation-set ticks, drain per-layer completion markers, commit completed owner entries in the tracker, clear completed targets from the active presentation plan, and preserve remaining owners until the owner table becomes empty. `CounterStyleTransitionBridge` now turns only real transform/opacity button control-state deltas into deterministic owner-keyed decisions or batches, and `CounterStyleTransitionRuntimeBridge.DispatchAndActivateInputTransitionBatchAsync` publishes app state before activating batch decisions against the retained snapshot. Blocked activation triggers app-side presentation cleanup rather than leaving the previous single-owner presentation active. This is still not full app-level multi-owner routing across arbitrary controls or properties.
...
4. Done: completion tracking now uses an owner table so one owner completion, cancel, fallback, or abort does not clear another target; a new owner for the same target supersedes the stale active owner.
...
7. Done: active presentation-set compositor state and tick contract can execute a validated `CompositionAnimationPresentationSetPlan` internally, publish per-layer markers after successful ticks, and report multi-layer capability skips.
8. Done: Poc-owned presentation activation bridge moves all-ready start/retarget batches into the active presentation-set compositor contract, attaches completion markers, and records owner-table tracking only after successful activation.
9. Done: Poc-owned batch cancellation/commit bridge clears an all-ready active presentation once and clears matching owner-table entries while blocking mixed, unsupported, or untracked clear batches before mutation.
10. Done: active presentation-set completion pump preflight drains per-layer completion markers, commits completed owners in the owner table, clears completed targets from the active presentation plan, and preserves remaining owners until the last tracked owner completes.
11. Done: Counter multi-target control-state routing now builds a Poc-owned decision batch, publishes app state first, activates the all-ready batch through the presentation-set bridge, and falls back by clearing stale presentation/tracker state when activation is blocked.
12. Next: harden routed multi-target completion and blocked fallback diagnostics, then expand only around concrete main-app regressions.
...
Single-target transform/opacity `Commit` preserves the completed presented layer as a constant compositor plan instead of taking the `Cancel` cleanup path. This keeps hit testing aligned with completed visual state, including the Counter PoC button's pressed downward offset; a lower-edge leave or re-enter should not flicker merely because the completion marker fired. Upper-edge flicker inside the displacement band can still happen because canceling a downward offset can move the original hit bounds back under the pointer.
...
Runtime interrupt handling is explicit: new input may commit, cancel, or retarget presented scroll state before dispatching the next layout frame. Wheel retarget keeps the visible origin continuous but accumulates distance on the logical target, so rapid N-notch input covers the same total distance as N separate one-notch inputs, including the main-app `EnsureRunning` path where new wheel events arrive while the coordinator loop is already active. Retarget sampling and hold are serialized through the compositor loop; app-side code must not read a presented value, clear the presentation, and then allow an older queued compositor tick or an ordinary retained render to flash before the replacement segment is installed. Internally, the compositor loop owns the scroll-presentation lifecycle state for active, held, pending retarget hold, queued-tick, generation, cancellation, and idle-waiter transitions. A sample-and-hold request suppresses old ticks as soon as it is accepted, but keeps the backend presentation sampleable until the queued sample work item finalizes the held state. The drawing backend only executes plan install/tick and retained-frame scroll-presentation refresh/defer requests. Backend active plan exclusivity across single animation, presentation sets, and scroll presentation lives in a private backend helper with marker playback, last-sample, and pending retained scroll-refresh state; it is not a runtime scheduler or public diagnostics contract.
