...
| GPU composition | Define a platform-neutral composition model and drive implementation through D3D12/GPU-backed layer updates. | [GPU-Composition-Architecture-v0.md](GPU-Composition-Architecture-v0.md), [D3D12-Composition-Spike-v0.md](D3D12-Composition-Spike-v0.md) | D3D12-backed compositor tick and hit-test remapping for transform/opacity, fixed-clip scroll presentation, multi-layer frames, main-app scroll presentation, layer content caching, and first-slice transform-with-content render-target-backed reuse exist on retained UI output. |
| Scroll/compositor bridge | Use scroll as the first hybrid case: runtime owns logical target; compositor owns presented offset. | [Animation-Composition-v0.md](Animation-Composition-v0.md), [D3D12-Composition-Spike-v0.md](D3D12-Composition-Spike-v0.md) | Fixed-clip presentation, nested/mixed-clip decomposition, PoC commit/cancel/retarget policy, active-presentation wheel retarget wiring, main app presentation producer, and lifecycle invalidation/cancellation exist. |
...
5. **D3D12-first implementation spike**: current transform/opacity tick and fixed-clip scroll presentation are in place with retained layer identity, runtime `NodeKey` declarations, active hit-test remapping, multi-layer/nested clip composition, scroll commit/cancel policy, main-app scroll presentation production, scroll lifecycle cancellation, D3D12 layer content caching, and transform-with-content rect/GlyphAtlas text command-order render-target-backed layer reuse. Continue by broadening render target content coverage to content-space fixed-clip/image/vector layers and adding explicit fallback diagnostics. Add fallback compatibility only when the D3D12 path exposes an explicit short-term blocker.
