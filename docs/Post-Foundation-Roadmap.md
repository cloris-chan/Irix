...
5. **D3D12-first implementation spike**: current transform/opacity tick and fixed-clip scroll presentation are in place with retained layer identity, runtime `NodeKey` declarations, active hit-test remapping, multi-layer/nested clip composition, scroll commit/cancel policy, main-app scroll presentation production, scroll lifecycle cancellation, and D3D12 layer content caching. Continue by adding explicit composition skip/unsupported-path diagnostics; consider content-space internal offscreen surfaces only after bounds/origin/clip semantics are designed and direct composition still needs them. Add secondary path code only when the D3D12 path exposes an explicit short-term blocker.
