...
5. **D3D12-first implementation spike**: current transform/opacity tick and fixed-clip scroll presentation are in place with retained layer identity, runtime `NodeKey` declarations, active hit-test remapping, multi-layer/nested clip composition, scroll commit/cancel policy, main-app scroll presentation production, scroll lifecycle cancellation, D3D12 layer content caching, and rect/GlyphAtlas text render-target-backed layer reuse. Continue by broadening render target content/order coverage and adding explicit fallback diagnostics. Add fallback compatibility only when the D3D12 path exposes an explicit short-term blocker.
