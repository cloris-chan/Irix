...
5. **D3D12-first implementation spike**: current transform/opacity tick and fixed-clip scroll presentation are in place with retained layer identity, runtime `NodeKey` declarations, and active hit-test remapping. Continue with multi-layer/nested clip composition and scroll commit/cancel policy. Add fallback compatibility only when the D3D12 path exposes an explicit short-term blocker.
