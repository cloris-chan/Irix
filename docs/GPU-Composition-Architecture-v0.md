...
| `RenderTargetCache` | Backend can cache stable layer content into GPU render targets. Implemented by D3D12 for rect-only suffix layers with RTV/SRV textures and textured-quad composition. |
...
| 12 | Render target reuse. | First D3D12 slice implemented for rect-only suffix layers through offscreen RTV/SRV textures and textured-quad composition. Next expand content coverage and ordering. |
