...
| `RenderTargetCache` | Backend can cache stable layer content into GPU render targets. Implemented by D3D12 for rect/GlyphAtlas text command-order layers with RTV/SRV textures and textured-quad composition. |
...
| 12 | Render target reuse. | D3D12 slice implemented for rect/GlyphAtlas text command-order layers through offscreen RTV/SRV textures, command-order render segments, and textured-quad composition. Next expand image/vector content coverage. |
