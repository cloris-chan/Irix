...
| `RenderTargetCache` | Backend can cache stable layer content into GPU render targets. Implemented by D3D12 for transform-with-content rect/GlyphAtlas text command-order layers with RTV/SRV textures and textured-quad composition. Fixed-clip scroll requires content-space targets before it can safely use this path. |
...
| 12 | Render target reuse. | D3D12 slice implemented for transform-with-content rect/GlyphAtlas text command-order layers through offscreen RTV/SRV textures, command-order render segments, and textured-quad composition. Fixed-clip scroll stays on direct/layer-content composition until content-space render target bounds exist. Next expand image/vector content coverage. |
...
- Which layers used render-target-backed cache hits or misses.
