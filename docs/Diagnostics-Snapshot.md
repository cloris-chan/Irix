...
- Material output status is one stable machine-readable line with `outputKind`, `backendCapabilities`, `selectedMaterialKind`, `fallbackReason`, `fallbackApplied`, `commandCount`, `solidColorCommands`, `linearGradientCommands`, `linearGradientSingleRectCommands`, `linearGradientSegmentedCommands`, `linearGradientSegmentRects`, and `fallbackCommands`. D3D12 FillRect may report `backendCapabilities=SolidColor, LinearGradient` and `fallbackReason=None` for clamp-free per-corner linear-gradient SDR rasterization or bounded segmented clamp fallback; the linear-gradient rasterization counters distinguish those internal shapes while unsupported material paths still report fallback without making material authoring public.
