...
| Post-foundation design start | Style, animation, and GPU composition design contracts now exist. The first implementation path is D3D12-backed transform/opacity plus fixed-clip scroll presentation with runtime `NodeKey` declarations, retained target resolution, compositor-owned ticks, marker event publication, runtime marker dispatch, and active hit-test remapping over the retained frame; compatibility fallback remains reserved for explicit blockers. |
...
| POST-042 | GPU composition architecture v0 | `GPU-Composition-Architecture-v0.md` defines IR, capabilities, GPU offload phases, and backend mapping notes. First D3D12 transform/opacity and single fixed-clip scroll presentation capability exists through retained target declarations and hit-test remapping. | Continue D3D12-first: multi-layer/nested clip composition; do not start Vulkan/Metal before the active backend proves those contracts. |
| POST-043 | Scroll compositor design | Single fixed-clip scroll presentation is implemented; logical scroll remains app/control runtime and presented scroll is compositor-owned. | Extend design for nested/mixed-clip scroll and runtime commit/cancel behavior before broad app integration. |
| POST-044 | Composition IR implementation gate | `D3D12-Composition-Spike-v0.md` now covers `CompositionAnimationDeclaration`, `CompositionAnimationPlan`, `CompositionScrollPresentationDeclaration`, `CompositionScrollPresentationPlan`, compositor-owned ticks, transform/fixed-clip scroll hit-test remapping, animation marker events, runtime marker dispatch, formal D3D12 composition capabilities, diagnostic output, `--diagnose-composition-scroll`, `--diagnose-composition-marker-runtime`, and `--composition-demo` on normal retained UI output. | Keep ticks independent of UI rebuild and extend the same path to multi-layer composition. |
| POST-045 | D3D12-first GPU offload spike | Started. | Validate nested/mixed-clip composition on the real D3D12 path. Add fallback code only for a documented blocker found during the spike. |
