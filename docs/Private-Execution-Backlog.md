...
| Post-foundation design start | Style, animation, and GPU composition design contracts now exist. The first implementation path is D3D12-backed transform/opacity plus fixed-clip scroll presentation with runtime `NodeKey` declarations, retained target resolution, compositor-owned ticks, marker event publication, runtime marker dispatch, multi-layer composition frame execution, and active hit-test remapping over the retained frame; compatibility fallback remains reserved for explicit blockers. |
...
| POST-041 | Animation/composition v0 | `Animation-Composition-v0.md` defines UI-runtime, compositor, hybrid, and backend-internal animation classes. Runtime transform/opacity and fixed-clip scroll declarations now resolve retained `NodeKey` targets into compositor ticks, active hit-test remapping, compositor-produced marker events, runtime marker dispatch through app-owned mapping, PoC commit/cancel/retarget decisions for presented scroll interruption, and live wheel retarget from active compositor scroll presentation. | Start live scroll presentation ticks from the main app scroll path before broader app integration. |
| POST-042 | GPU composition architecture v0 | `GPU-Composition-Architecture-v0.md` defines IR, capabilities, GPU offload phases, and backend mapping notes. First D3D12 transform/opacity, fixed-clip scroll presentation, runtime marker dispatch, multi-layer frame execution, and nested/mixed-clip scroll decomposition exist. | Continue D3D12-first: layer caching and scroll commit/cancel policy; do not start Vulkan/Metal before the active backend proves those contracts. |
| POST-043 | Scroll compositor design | Fixed-clip scroll presentation is implemented for single-layer and decomposed nested/mixed-clip retained targets; logical scroll remains app/control runtime and presented scroll is compositor-owned. PoC runtime policy now defines commit, cancel, and retarget behavior, and live wheel input can retarget from active presentation. | Add a main-app producer that starts compositor scroll presentation ticks before broad app integration. |
| POST-044 | Composition IR implementation gate | `D3D12-Composition-Spike-v0.md` now covers `CompositionAnimationDeclaration`, `CompositionAnimationPlan`, `CompositionScrollPresentationDeclaration`, `CompositionScrollPresentationPlan`, compositor-owned ticks, transform/fixed-clip scroll hit-test remapping, animation marker events, runtime marker dispatch, multi-layer frame execution, retained nested/mixed-clip decomposition, formal D3D12 composition capabilities, diagnostic output, `--diagnose-composition-scroll`, `--diagnose-composition-marker-runtime`, `--diagnose-composition-multilayer`, and `--composition-demo` on normal retained UI output. | Keep ticks independent of UI rebuild; next reduce replay cost with layer caching. |
| POST-045 | D3D12-first GPU offload spike | Started. | Validate layer caching on the real D3D12 path. Add fallback code only for a documented blocker found during the spike. |
