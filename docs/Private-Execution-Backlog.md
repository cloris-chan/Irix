...
| Post-foundation design start | Style, animation, and GPU composition design contracts now exist. The first implementation path is D3D12-backed transform/opacity plus fixed-clip scroll presentation with runtime `NodeKey` declarations, retained target resolution, compositor-owned ticks, and active hit-test remapping over the retained frame; compatibility fallback remains reserved for explicit blockers. |
...
| POST-041 | Animation/composition v0 | `Animation-Composition-v0.md` defines UI-runtime, compositor, hybrid, and backend-internal animation classes. Runtime transform/opacity and fixed-clip scroll declarations now resolve retained `NodeKey` targets into compositor ticks and active hit-test remapping. | Define runtime commit/cancel policy before integrating scroll input with presentation ticks. |
