...
| Post-foundation design start | Style, animation, and GPU composition design contracts now exist. The first implementation path is D3D12-backed transform/opacity plus fixed-clip scroll presentation with runtime `NodeKey` declarations, retained target resolution, compositor-owned ticks, marker event publication, runtime marker dispatch, multi-layer composition frame execution, active hit-test remapping over the retained frame, main-app scroll presentation production, lifecycle invalidation/cancellation, D3D12 layer content caching, and first-slice render-target-backed layer caching; compatibility fallback remains reserved for explicit blockers. |
...
| POST-042 | GPU composition architecture v0 | `GPU-Composition-Architecture-v0.md` defines IR, capabilities, GPU offload phases, and backend mapping notes. First D3D12 transform/opacity, fixed-clip scroll presentation, runtime marker dispatch, multi-layer frame execution, nested/mixed-clip scroll decomposition, main-app scroll presentation production, scroll lifecycle cancellation, layer content caching, and rect/GlyphAtlas text render-target-backed layer reuse exist. | Continue D3D12-first: broaden render target content/order coverage next; do not start Vulkan/Metal before the active backend proves those contracts. |
...
| POST-044 | Composition IR implementation gate | `D3D12-Composition-Spike-v0.md` now covers `CompositionAnimationDeclaration`, `CompositionAnimationPlan`, `CompositionScrollPresentationDeclaration`, `CompositionScrollPresentationPlan`, compositor-owned ticks, transform/fixed-clip scroll hit-test remapping, animation marker events, runtime marker dispatch, multi-layer frame execution, retained nested/mixed-clip decomposition, D3D12 layer content cache diagnostics, render target cache diagnostics, formal D3D12 composition capabilities, diagnostic output, `--diagnose-composition-scroll`, `--diagnose-composition-marker-runtime`, `--diagnose-composition-multilayer`, `--diagnose-composition-layer-cache`, `--diagnose-composition-render-target-cache`, and `--composition-demo` on normal retained UI output. | Keep ticks independent of UI rebuild; next broaden render-target-backed reuse into image/vector content and richer ordering. |
| POST-045 | D3D12-first GPU offload spike | D3D12 render-target-backed layer reuse is implemented for rect/GlyphAtlas text suffix layers. | Expand render target reuse to image/vector-capable layers and richer ordering. Add fallback code only for a documented blocker found during the spike. |
