...
| Post-foundation design start | Style, animation, and GPU composition design contracts now exist. The first implementation path is D3D12-backed transform/opacity composition with compositor-owned ticks over the retained frame; compatibility fallback remains reserved for explicit blockers. |
...
| POST-041 | Animation/composition v0 | `Animation-Composition-v0.md` defines UI-runtime, compositor, hybrid, and backend-internal animation classes. First transform/opacity compositor tick exists over the retained frame. | Connect runtime animation declaration to `CompositionAnimationPlan` without moving logical state into the backend. |
| POST-042 | GPU composition architecture v0 | `GPU-Composition-Architecture-v0.md` defines IR, capabilities, GPU offload phases, and backend mapping notes. First D3D12 transform/opacity capability exists. | Continue D3D12-first: stable retained layer identity, then scroll presentation; do not start Vulkan/Metal before the active backend proves those contracts. |
...
| POST-044 | Composition IR implementation gate | `D3D12-Composition-Spike-v0.md` now covers `CompositionAnimationPlan`, compositor-owned ticks, formal D3D12 composition capability, diagnostic output, and `--composition-demo`. | Promote layer identity from demo command ranges to normal retained UI output; keep ticks independent of UI rebuild. |
| POST-045 | D3D12-first GPU offload spike | Started. | Validate retained layer identity, immutable IR handoff, compositor-updated transform/opacity, and diagnostics on the real D3D12 path. Add fallback code only for a documented blocker found during the spike. |
