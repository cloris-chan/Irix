...
| POST-044 | Composition IR implementation gate | Not started. | First code change must be minimal, D3D12-backed, and should not alter current draw command renderer behavior without tests. The existing renderer is fallback; do not build a generic compatibility compositor first. |
| POST-045 | D3D12-first GPU offload spike | Not started. | Validate layer identity, immutable IR handoff, compositor-updated transform/opacity, and diagnostics on the real D3D12 path. Add fallback code only for a documented blocker found during the spike. |
