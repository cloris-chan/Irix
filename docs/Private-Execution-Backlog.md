...
| POST-045 | D3D12-first GPU offload spike | First D3D12 render-target-backed layer reuse is implemented for rect-only suffix layers. | Expand render target reuse to text/image/vector-capable layers and richer ordering. Add fallback code only for a documented blocker found during the spike. |
