...
| POST-044 | Composition IR implementation gate | Started through `D3D12-Composition-Spike-v0.md` and the explicit D3D12 transform/opacity diagnostic path. | Keep the first path minimal, D3D12-backed, and separate from normal draw-command renderer behavior until animation ticking and hit-test mapping are contracted. |
