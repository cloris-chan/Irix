...
| POST-042 | GPU composition architecture v0 | `GPU-Composition-Architecture-v0.md` defines IR, capabilities, GPU offload phases, and backend mapping notes. First D3D12 transform/opacity, single fixed-clip scroll presentation, runtime marker dispatch, and multi-layer frame execution exist. | Continue D3D12-first: retained target decomposition for nested/mixed clips; do not start Vulkan/Metal before the active backend proves those contracts. |
| POST-043 | Scroll compositor design | Single fixed-clip scroll presentation is implemented; logical scroll remains app/control runtime and presented scroll is compositor-owned. | Extend retained target resolution for nested/mixed-clip scroll and runtime commit/cancel behavior before broad app integration. |
| POST-044 | Composition IR implementation gate | `D3D12-Composition-Spike-v0.md` now covers `CompositionAnimationDeclaration`, `CompositionAnimationPlan`, `CompositionScrollPresentationDeclaration`, `CompositionScrollPresentationPlan`, compositor-owned ticks, transform/fixed-clip scroll hit-test remapping, animation marker events, runtime marker dispatch, multi-layer frame execution, formal D3D12 composition capabilities, diagnostic output, `--diagnose-composition-scroll`, `--diagnose-composition-marker-runtime`, `--diagnose-composition-multilayer`, and `--composition-demo` on normal retained UI output. | Keep ticks independent of UI rebuild and generate multi-layer plans from retained nested/mixed-clip targets. |
| POST-045 | D3D12-first GPU offload spike | Started. | Validate retained nested/mixed-clip decomposition on the real D3D12 path. Add fallback code only for a documented blocker found during the spike. |
