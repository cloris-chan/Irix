...
| Post-foundation design start | Style, animation, and GPU composition design contracts now exist as design-only docs. Once the next implementation gate opens, the first composition path should be D3D12/GPU-backed, with compatibility fallback added only for explicit blockers. |
...
| POST-041 | Animation/composition v0 | `Animation-Composition-v0.md` defines UI-runtime, compositor, hybrid, and backend-internal animation classes. | Select a narrow first implementation case; prefer compositor/GPU execution for eligible properties. |
| POST-042 | GPU composition architecture v0 | `GPU-Composition-Architecture-v0.md` defines IR, capabilities, GPU offload phases, and backend mapping notes. | First implementation should be D3D12/GPU-backed; do not start Vulkan/Metal before the active backend proves the contract. |
...
| POST-044 | Composition IR implementation gate | Started through `D3D12-Composition-Spike-v0.md`, the explicit D3D12 transform/opacity diagnostic path, and `--composition-demo`. | Keep the first path minimal, D3D12-backed, and separate from normal draw-command renderer behavior until animation ticking and hit-test mapping are contracted. |
| POST-045 | D3D12-first GPU offload spike | Started. | Validate layer identity, immutable IR handoff, compositor-updated transform/opacity, and diagnostics on the real D3D12 path. Add fallback code only for a documented blocker found during the spike. |
