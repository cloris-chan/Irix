...
| Post-foundation design start | Style, animation, and GPU composition design contracts now exist. The first implementation path is D3D12-backed transform/opacity plus fixed-clip scroll presentation with runtime `NodeKey` declarations, retained target resolution, compositor-owned ticks, marker event publication, runtime marker dispatch, multi-layer composition frame execution, active hit-test remapping over the retained frame, and main-app scroll presentation production; compatibility fallback remains reserved for explicit blockers. |
...
| POST-041 | Animation/composition v0 | `Animation-Composition-v0.md` defines UI-runtime, compositor, hybrid, and backend-internal animation classes. Runtime transform/opacity and fixed-clip scroll declarations now resolve retained `NodeKey` targets into compositor ticks, active hit-test remapping, compositor-produced marker events, runtime marker dispatch through app-owned mapping, PoC commit/cancel/retarget decisions for presented scroll interruption, live wheel retarget from active compositor scroll presentation, and main-app scroll presentation production. | Harden producer cancellation/fallback behavior before broader app integration. |
| POST-042 | GPU composition architecture v0 | `GPU-Composition-Architecture-v0.md` defines IR, capabilities, GPU offload phases, and backend mapping notes. First D3D12 transform/opacity, fixed-clip scroll presentation, runtime marker dispatch, multi-layer frame execution, nested/mixed-clip scroll decomposition, and main-app scroll presentation production exist. | Continue D3D12-first: layer caching and producer hardening; do not start Vulkan/Metal before the active backend proves those contracts. |
| POST-043 | Scroll compositor design | Fixed-clip scroll presentation is implemented for single-layer and decomposed nested/mixed-clip retained targets; logical scroll remains app/control runtime and presented scroll is compositor-owned. PoC runtime policy now defines commit, cancel, and retarget behavior, live wheel input can retarget from active presentation, and the main app path can start compositor scroll presentation ticks. | Harden cancellation/fallback diagnostics before broad app integration. |
