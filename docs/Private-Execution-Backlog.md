...
| Post-foundation design start | Style, animation, and GPU composition design contracts now exist. The first implementation path is D3D12-backed transform/opacity composition with runtime `NodeKey` animation declarations, retained target resolution, compositor-owned ticks, and active-transform hit-test remapping over the retained frame; compatibility fallback remains reserved for explicit blockers. |
...
| POST-044 | Composition IR implementation gate | `D3D12-Composition-Spike-v0.md` now covers `CompositionAnimationDeclaration`, `CompositionAnimationPlan`, compositor-owned ticks, transform-layer hit-test remapping, formal D3D12 composition capability, diagnostic output, and `--composition-demo` on normal retained UI output. | Keep ticks independent of UI rebuild and extend the same path to scroll presentation. |
| POST-045 | D3D12-first GPU offload spike | Started. | Validate presented scroll offset on the real D3D12 path. Add fallback code only for a documented blocker found during the spike. |
