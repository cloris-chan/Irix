...
| POST-041 | Animation/composition v0 | `Animation-Composition-v0.md` defines UI-runtime, compositor, hybrid, and backend-internal animation classes. Runtime transform/opacity and fixed-clip scroll declarations now resolve retained `NodeKey` targets into compositor ticks, active hit-test remapping, compositor-produced marker events, and runtime marker dispatch through app-owned mapping. | Define commit/cancel policy before integrating scroll input with presentation ticks. |
...
| POST-043 | Scroll compositor design | Fixed-clip scroll presentation is implemented for single-layer and decomposed nested/mixed-clip retained targets; logical scroll remains app/control runtime and presented scroll is compositor-owned. | Define runtime commit/cancel behavior before broad app integration. |
