...
| Post-foundation design start | Style, animation, and GPU composition design contracts now exist. The first implementation path is D3D12-backed transform/opacity plus fixed-clip scroll presentation with runtime `NodeKey` declarations, retained target resolution, compositor-owned ticks, marker event publication, runtime marker dispatch, multi-layer composition frame execution, active hit-test remapping over the retained frame, main-app scroll presentation production, lifecycle invalidation/cancellation, and D3D12 layer content caching; compatibility fallback remains reserved for explicit blockers. |
...
| POST-042 | GPU composition architecture v0 | `GPU-Composition-Architecture-v0.md` defines IR, capabilities, GPU offload phases, and backend mapping notes. First D3D12 transform/opacity, fixed-clip scroll presentation, runtime marker dispatch, multi-layer frame execution, nested/mixed-clip scroll decomposition, main-app scroll presentation production, scroll lifecycle cancellation, and layer content caching exist. | Continue D3D12-first: render-target-backed layer reuse next; do not start Vulkan/Metal before the active backend proves those contracts. |
